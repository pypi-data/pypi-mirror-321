...
def fused_rwkv7_kernel(
    ...
    state_ptr, output_ptr, state_output_ptr,
    scale: tl.constexpr,
    N:tl.constexpr, 
    V:tl.constexpr, 
    L: tl.constexpr, 
...
class FusedRecurrentRWKV7Function(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, w, a, b, scale, state, output_final_state:bool=True, training: bool = True):
        ...
        B, H, L, N = q.shape
        V = v.shape[-1]
        assert N == V, "fused_rwkv7 only support N=V"

        output = torch.empty_like(v)
        final_state = q.new_empty(B, H, N, N) if output_final_state else None
...
def fused_recurrent_rwkv7(
    ...
    Args:
        ...
        scale (Optional[int]):
            Scale factor for the RWKV7 attention scores.
            If not provided, it will default to `1 / sqrt(K)`. Default: `None`.
        initial_state (Optional[torch.Tensor]):
            Initial state of shape `(B, H, K, V)`. Default: `None`.
        output_final_state (Optional[bool]):
            Whether to output the final state of shape `(B, H, K, V)`. Default: `False`.
        `fused_rwkv7` already has `-tl.exp(w)`, so there is no need to use it again.
    """
    if scale == -1.0:
        scale = r.shape[-1] ** -0.5
    set_torch_device(r)
