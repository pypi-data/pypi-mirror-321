...
def _setup_callbacks(what):
    ...
    _callbacks = {
        ...
        'kinetic energy per particle': lambda x: x.system.kinetic_energy(True),
        'total energy per particle': lambda x: x.system.total_energy(True, cache=True),
        'potential_energy_per_particle': lambda x: x.system.potential_energy(True),
        'kinetic_energy_per_particle': lambda x: x.system.kinetic_energy(True),
        'total_energy_per_particle': lambda x: x.system.total_energy(True, cache=True),
        'temperature': lambda x: x.system.temperature,
        'density': lambda x: x.system.density,
        'pressure': lambda x: x.system.pressure,
        'rmsd': lambda x: x.rmsd,
        'cm velocity': lambda x: x.system.center_of_mass('velocity'),
        'cm velocity norm': lambda x: numpy.sum(x.system.center_of_mass('velocity')**2)**0.5,
        'cm_velocity': lambda x: x.system.center_of_mass('velocity'),
        'cm_velocity_norm': lambda x: numpy.sum(x.system.center_of_mass('velocity')**2)**0.5,
    }
...
def write(sim, what, suffix=None, path=None):
    ...
    - a string from the following list:
      ...
      - kinetic_energy_per_particle
      - total_energy_per_particle
      - temperature
      - density
      - pressure
      - rmsd
