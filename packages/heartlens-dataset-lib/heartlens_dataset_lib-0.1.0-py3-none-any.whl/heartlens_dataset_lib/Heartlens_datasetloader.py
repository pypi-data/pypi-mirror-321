...
if __name__ == "__main__":
    # Specify the path to the Zarr dataset
    zarr_path = "dataset/uk_heartlens_v1"  # Update this path to your Zarr dataset location
    batch_size = 16  # Define the batch size
    rank = 0  # Define the rank (used in distributed training)
    shuffle = True  # Specify whether to shuffle the dataset

    # Initialize the HeartlensDatasetLoader
    dataset_loader = HeartlensDatasetLoader(
        batch_size=batch_size,
        rank=rank,
        shuffle=shuffle,
        zarr_path=zarr_path,
        is_DDP=False  # Set to False if not using Distributed Data Parallel (DDP)
    )

    # Retrieve the first batch of data
    first_batch = next(iter(dataset_loader.data_loader))
    images, bag_ids, indices = first_batch

    # Visualize the images using matplotlib
    num_images_to_show = min(len(images), 8)  # Display up to 8 images
    ...
    for i in range(num_images_to_show):
        ...
        # Reverse normalization: scale from [-1, 1] back to [0, 255]
        img = ((img + 1) / 2) * 255  
        img = img.astype(np.uint8)

        # Add the image to the grid (2x4)
        plt.subplot(2, 4, i + 1)  
        plt.imshow(img, cmap="gray")  # Display in grayscale
        plt.title(f"Bag ID: {bag_ids[i].item()}")  # Set the title with Bag ID
