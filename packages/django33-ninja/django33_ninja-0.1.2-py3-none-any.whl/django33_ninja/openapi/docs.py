...
if TYPE_CHECKING:
    ...
    from django33_ninja import NinjaAPI  # pragma: no cover

ABS_TPL_PATH = Path(__file__).parent.parent / "templates/django33_ninja/"
...
class Swagger(DocsBase):
    template = "django33_ninja/swagger.html"
...
class Redoc(DocsBase):
    template = "django33_ninja/redoc.html"
...
def render_template(
    ...
    """
    I do not really want django33_ninja to be required in INSTALLED_APPS to ease installation
    so it automatically detects - if django33_ninja is in INSTALLED_APPS - then we render with django.shortcuts.render
    otherwise - rendering custom html with swagger js from cdn
    """
    if "django33_ninja" in settings.INSTALLED_APPS:
...
def _render_cdn_template(
    ...
    "this is helper to find and render html template when django33_ninja is not in INSTALLED_APPS"
    from django33.template import RequestContext, Template
