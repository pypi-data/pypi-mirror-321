...
###############################################################################
# Suppose we have the following data and we want to fit it with the model:
...
def fitting(xs, ys):
    ...
    out = minimize(residual, fit_params, args=(xs,), kws={'data': ys})

    return out


out = fitting(x, data)
fit = residual(out.params, x.mantissa)
###############################################################################
