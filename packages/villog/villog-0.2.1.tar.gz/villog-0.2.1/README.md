...
villsql_client: VillSQL = VillSQL(login_data = login_dict)

example_get_table: Table = villsql_client.get_table("EXAMPLE_TABLE",
                                                    raw_filter="EXAMPLE_TABLE.col_1 = "example",
                                                    order_by = ["EXAMPLE_TABLE.col_1","ASC",
                                                                "ECAMPLE_TABLE.col_2","DESC"],
                                                                # kwargs:
                                                                COL_2 = 1)

example_get_table.set_filter(column_names = ["col_1, "col_2"])

print("COLUMNS:")
for column in example_get_table.columns():
    print(column, end = "\t")
print("\nROWS:)
for row in example_get_table.rows:
    for elem in row:
        print(elem, end = "\t")
"""
Output:
