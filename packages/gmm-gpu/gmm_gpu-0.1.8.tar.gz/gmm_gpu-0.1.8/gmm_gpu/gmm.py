...
class GMM:
    ...
    def fit(self, X):
        ...
        self.covs = [self._get_covs(X, self.means[k], r[:, :, k], N_actual[:, k])
                     for k in range(self._n_components)]
        self._cholesky_covs = [torch.linalg.cholesky(cov) for cov in self.covs]
        ...
        while iteration < self._max_iter and not converged.all():
            ...
            for k in range(self._n_components):
                new_r[~converged, :, k] = _estimate_gaussian_prob(
                    ...
                    self._cholesky_covs[k][~converged],
                    self._dtype
            ...
            for k in range(self._n_components):
                ...
                self._cholesky_covs[k][~converged] = torch.linalg.cholesky(self.covs[k][~converged])
...
def _estimate_gaussian_prob(X, mean, chol_cov, dtype):
    ...
    solve = torch.linalg.solve_triangular(chol_cov, #.float(),
                                          diff.permute(0, 2, 1).float(),
                                          upper=False).permute(0, 2, 1)  # (B, N, D)
