...
class AgentSocietyWorker:
    ...
    def _register_agent_and_get_queue_name(self) -> Optional[str]:
        for _ in range(10):
            try:
                ...
                print("Could not get queue name, retrying after wait!")
            except Exception as e:
                print(f"Something went wrong registring: {e}")
    ...
    def _register_agent(self) -> Optional[str]:
        ...
        if result.status_code <= 299:
            print(f"STATUS {result.status_code}: {result.json()}")
            return result.json()["queue_name"]
        return None

    def _delete_mismatching_agent(self):
        print("Found old agent metadata under this name, deleting old agent for re-creation.")
    ...
    def listen_to_queue(self):
        ...
        for _ in range(5):
            try:
                ...
                print(f"Something went wrong connecting to the queue: {e}, retrying in 3 seconds")
            time.sleep(3)
        print("Connection has failed. Retries exhausted.")