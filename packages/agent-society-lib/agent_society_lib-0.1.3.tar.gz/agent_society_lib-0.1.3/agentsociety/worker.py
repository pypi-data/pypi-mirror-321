...
from chat.history import History, HistoryDelta
from chat.artifact import TaskArtifacts, TaskArtifact
from chat.box.free_flow_graph import FreeFlowSequence
...
class AgentSocietyWorker:
    ...
    def __init__(self, connection_params: pika.ConnectionParameters, user_id: str, agent_name: str, host: str, free_flow: FreeFlowSequence) -> None:
    ...
    def get_queue_name(self) -> str:
        for _ in range(10):
            result = requests.get(self.host, json={"name": self.agent_name}, headers={"user-id": self.user_id})
        
            if result.status_code <= 299:
                return result.json()["queue_name"]
            else:
                print("agent does not exist trying to create it")
                result = requests.post(self.host, json={"name": self.agent_name}, headers={"user-id": self.user_id})
                if result.status_code <= 299:
                    time.sleep(1)
                    return result.json()["queue_name"]
            time.sleep(1)
    ...
    def _send_messages_reply_success(self, chn: Channel, method: pika.spec.Basic.Deliver, delta: HistoryDelta, task_id: str):
        ...
        print("Sent messages response to dispatcher")
    ...
    def _send_reply_failed(self, chn: Channel, method: pika.spec.Basic.Deliver, task_id: str):
        ...
        chn.basic_publish(exchange='dispatcher', routing_key='', body=json.dumps(body))
        chn.basic_ack(method.delivery_tag)
        print("Sent response to dispatcher")
    ...
    def _queue_callback(self, chn: Channel, method: pika.spec.Basic.Deliver, props: pika.spec.BasicProperties, body: bytes):
        ...
        try:
            ...
            print(f"Could not convert python object into History: {e}\n{traceback.format_exc()}")
    ...
    def listen_to_queue(self):
        ...
        self.channel.basic_consume(queue_name, on_message_callback=self._queue_callback, auto_ack=False)
