...
from Relukko.skip_relukko import SKIP_RELUKKO
...
def test_relukko_with_skip_robot(mock_env_relukko_skip):
    with mock_env_relukko_skip.context() as m:
        def mock_get_all_relukkos(*args, **kwargs):
            return [ SKIP_RELUKKO ]

        m.setattr(Relukko, "get_all_relukkos", mock_get_all_relukkos)
