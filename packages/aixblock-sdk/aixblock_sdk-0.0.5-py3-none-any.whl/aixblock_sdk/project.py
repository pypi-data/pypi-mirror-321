...
class Project(Client):
    ...
    def get_checkpoint_ids(self, checkpoint_s3=True):
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/checkpoint_model_marketplace?project_id={self.id}&checkpoint_s3={checkpoint_s3}'
        )
        return [cp['id'] for cp in response.json()]
    
    def get_checkpoint_info(self, checkpoint_s3=True):
        """ Get list of checkpoint ID from project ID
        ...
        return [{"id": cp["id"], "version": cp.get("version"), "checkpoint_storage_name": cp.get("checkpoint_storage_name")} for cp in response.json()]

    def upload_checkpoint(self, checkpoint, version, checkpoint_id=None, path_file="", type_checkpoint="ml_checkpoint", send_mail=False):
        """ Upload checkpoint
        ...
        if isinstance(checkpoint, (str, Path)):
            ...
            with open(checkpoint, mode='rb') as f:
                if checkpoint_id:
                    ...
                    response = self.make_request(
                        method='POST',
                        url=f'/api/checkpoint_model_marketplace/upload/',
                        files={'file': f},
                        data={
                            'project_id': self.id,
                            "type_checkpoint": type_checkpoint,
    ...
    def download_checkpoint(self, checkpoint_id, save_path=None, project_id=None):
        """ Download checkpoint
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/checkpoint_model_marketplace/download/{checkpoint_id}?project_id={project_id}'
        )
        filename = None
        # path = "."
        ...
        if response.status_code == 200:
            # filename = response.headers.get('filename')
            filename = response.headers.get('X-Checkpoint-Name')
            if not save_path:
                data_zip_dir = os.path.join(clone_dir, f"{filename}.zip")
            else:
                data_zip_dir = os.path.join(save_path, f"{filename}.zip")
            # with open(os.path.join(path, filename), 'wb') as f:
            #     for chunk in response:
            #         f.write(chunk)
    ...
    def get_dataset_ids(self):
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/dataset_model_marketplace?project_id={self.id}'
        )
    ...
    def get_dataset_info(self):
        ...
        return [{"id": ds["id"], "version": ds.get("version"), "dataset_storage_name": ds.get("dataset_storage_name")} for ds in response.json()]

    def upload_dataset(self, dataset, version, dataset_id=None, type_dataset="hugging-face", username=None, token=None, url=None, version_dataset=None, workspace=None, path_file=None):
        """ Upload dataset
        ...
        if type_dataset == "local":
            if isinstance(dataset, (str, Path)):
                ...
                with open(dataset, mode='rb') as f:
                    if dataset_id:
                        ...
                        response = self.make_request(
                            method='POST',
                            url=f'/api/import/upload-dataset',
                            files={'file': f},
                            data={
                                'project_id': self.id,
            ...
            if type_dataset in ["kaggle", "hugging-face", "roboflow", "git"]:
                response = self.make_request(
                        method='POST',
                        url=f'/api/import/upload-dataset',
                        data={
    ...
    def download_dataset(self, dataset_id, save_path=None, project_id=None):
        """ Download dataset
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/dataset_model_marketplace/download/{dataset_id}?project_id={project_id}'
        )
        filename = None
        # path = "."
        clone_dir = os.path.join(os.getcwd())
        if response.status_code == 200:
            # filename = response.headers.get('filename')
            filename = response.headers.get('X-Dataset-Name')
            if not save_path:
                data_zip_dir = os.path.join(clone_dir, f"{filename}.zip")
            else:
                data_zip_dir = os.path.join(save_path, f"{filename}.zip")
    ...
    def upload_raw_file(self, file_path):
        """ Upload raw file
        ...
        if isinstance(file_path, (str, Path)):
            ...
            if not os.path.isfile(file_path):
                raise AIxBlockException(f'Not found dataset file {file_path}')
            
            with open(file_path, mode='rb') as f:
                response = self.make_request(
                    method='POST',
                    url=f'/api/import/upload-raw-file',
                    files={'file': f},
                    data={
                        'project_id': self.id
                    }
            ...
            raise TypeError(f'Not supported type provided as "dataset" argument: {type(file_path)}')
            
        return response.json()
    
    def download_raw_file(self, save_path=None, project_id=None):
        """ Download checkpoint
        ...
        response = self.make_request(
            method='GET',
            url=f'/api/projects/download-raw-file?project_id={project_id}'
        )
        filename = None
        # path = "."
        clone_dir = os.path.join(os.getcwd())
        if response.status_code == 200:
            # filename = response.headers.get('filename')
            filename = response.headers.get('X-Rawfile-Name')
            if not save_path:
                data_zip_dir = os.path.join(clone_dir, f"{filename}.zip")
            else:
                data_zip_dir = os.path.join(save_path, f"{filename}.zip")
