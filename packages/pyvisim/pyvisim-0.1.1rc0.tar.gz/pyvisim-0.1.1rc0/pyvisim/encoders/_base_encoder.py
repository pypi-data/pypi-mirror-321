...
class ImageEncoderBase(SimilarityMetric):
    ...
    def clustering_model(self, clustering_model):
        if self._feature_extractor.output_dim != self._clustering_model.n_features_in_:
            raise RuntimeError(f"Feature Extractor outputs shape {self.feature_extractor.output_dim}, "
                               f"But clustering model accepts input dim {self._clustering_model.n_features_in_}")
    ...
    def learn(self, images: Iterable[np.ndarray], /,*, n_clusters: int, reduced_dim: bool = False, reduce_factor: int=2, **kwargs) -> None:
        """
        ...
        :param reduced_dim: Whether to apply PCA for dimensionality reduction
