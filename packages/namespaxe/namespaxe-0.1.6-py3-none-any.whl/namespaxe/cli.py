...
class NamespaxeCLI:
    ...
    def list_resources(self, resource_type, wide=False, clean=False):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if clean:
                    # Print the raw JSON data if --clean is passed
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('status'):
                    ...
                    if clean:
                        click.echo(json.dumps(description_data, indent=4))  # Print raw data without tabulation
                    else:
    ...
    def show_help(self):
        ...
        Commands:
            ...
            install-k8s-config  - Install Kubernetes config for a specific resource.
                                Usage: namespaxe install-k8s-config <resource> <resource_name>
                                Example: namespaxe install-k8s-config ns <namespace-name>
