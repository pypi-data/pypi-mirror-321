...
import dateparser
from obsei_module.obsei.source.module_url_test import GoogleNews
...
class GoogleNewsSource(BaseSource):
    ...
    def lookup(self, config: GoogleNewsConfig, **kwargs: Any) -> List[TextPayload]:
        ...
        while more_data_exist and before_time > last_after_time:
            ...
            for article in articles:
                published_date = (
                    None
                    if article["datetime"] is None or (isinstance(article["datetime"], float) and math.isnan(article["datetime"]))
