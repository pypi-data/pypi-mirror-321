...
def download_assets(ctx, id, url, object_key, destination, threads, include_empty_folders, download_proxy, recursive):
    ...
    try:
        ...
        download_assets_exec(ctx, filtered_assets, id_details, destination, threads, download_proxy)

    except Exception as e:           
        click.echo(json.dumps({"error": f"Error downloading asset: {e}"}))
        sys.exit(1)
        
def download_assets_exec(ctx, assets, id_details, destination, threads, download_proxy):
    ...
    for batch_num in range(0, len(assets), 100):
        ...
        download_batch(ctx, batch, threads, download_proxy, is_folder_asset, id_details, destination, num_file_assets, batch_num)
...
def download_batch(ctx, batch, threads, download_proxy, is_folder_asset, id_details, destination, num_file_assets, batch_num):
