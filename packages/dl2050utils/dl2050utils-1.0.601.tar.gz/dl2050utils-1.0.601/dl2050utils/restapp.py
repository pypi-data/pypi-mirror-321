...
class App():
    ...
    async def ulists_insert(self, request):
        ...
        l = LexIdx()
        if id2 is None:
            seq = await self.db.execute(f"select max(lexseq) from ulists where email='{email}' and ulist='{ulist}'")
    ...
    async def ulists_update(self, request):
        ...
        if id1 is not None or id2 is not None:
            l = LexIdx()
            if id2 is None:
                res = await self.db.execute(f"select max(lexseq) from ulists where email='{email}' and ulist='{ulist}'")
                lseq = oget(res,[0,'max'])
                lseq = l.next(lseq)
            else:
    ...
    async def ulists_delete(self, request):
        ...
        args = enforce_required_args(self.LOG, data, ['ulist', 'id'], label='REST', label2=label2)
        ulist,id_ = itemgetter('ulist','id')(args)
        await self.db.execute(f"delete from ulists where email='{email}' and ulist='{ulist}' and id='{id_}'")
        return rest_ok()
