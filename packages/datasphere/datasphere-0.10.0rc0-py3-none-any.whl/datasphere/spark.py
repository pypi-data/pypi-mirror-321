...
class SparkWrapper:
    ...
    def sc(self) -> Optional['SparkContext']:
        return getattr(self._session, 'sparkContext', None)
...
def parse_spark_config(config: Any) -> SparkConfig:
    ...
    try:
        ...
        raise ValueError(
            f"Unknown mode '{mode}'. Allowed values: {[mode.name for mode in ConnectionMode]}"
        ) from error

    spark_session_params = config.get('spark_session_params')
    if spark_session_params is None:
        raise ValueError('Spark session params are not specified')
    elif not (
        isinstance(spark_session_params, dict)
        and all(isinstance(k, str) and isinstance(v, str) for k, v in spark_session_params.items())
    ):
    ...
    return SparkConfig(
        ...
        session_params=spark_session_params,
    )
