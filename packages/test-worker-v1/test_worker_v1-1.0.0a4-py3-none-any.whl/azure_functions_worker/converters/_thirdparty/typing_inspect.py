...
def is_callable_type(tp):
    ...
    """
    if NEW_TYPING:
        return (tp is Callable or isinstance(tp, _GenericAlias)
                and tp.__origin__ is collections.abc.Callable
                or isinstance(tp, type) and issubclass(tp, Generic)
...
def is_tuple_type(tp):
    ...
    """
    if NEW_TYPING:
        return (tp is Tuple or isinstance(tp, _GenericAlias)
...
def is_classvar(tp):
    ...
    """
    if NEW_TYPING:
        return (tp is ClassVar
                or isinstance(tp, _GenericAlias) and tp.__origin__ is ClassVar)
...
def get_parameters(tp):
    ...
    if NEW_TYPING:
        ...
        return ()
    if (
        is_generic_type(tp) or is_union_type(tp)
            or is_callable_type(tp) or is_tuple_type(tp)
    ):
