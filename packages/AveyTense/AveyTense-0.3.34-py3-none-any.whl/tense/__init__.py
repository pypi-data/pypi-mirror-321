...
_SizeableItemGetter = _tc.PickSequence[_T]
...
class _ProbabilityLength(_tc.Enum):
...
class TenseOptions(_util.AbstractFinal):
    ...
    enableFurryDeclarations = False
    """
    \\@since 0.3.34
    
    When this setting is `True`, normally `Tense.owoify()` and `Tense.uwuify()` will be available at runtime. \\
    Using these, when this option is `False`, throw an error. Default value is `False`
...
class Tense(
    ...
    def __init__(self):
        ...
        if TenseOptions.initializationMessage:
            print(f"\33[1;90m{e}\33[1;36m INITIALIZATION\33[0m Class '{__class__.__name__}' was successfully initalized. Line {_inspect.currentframe().f_back.f_lineno}")
        ...
        for i in abroad(value1, value2, modifier):
            ...
            if each is None:
                ...
                if not isinstance(tmp, (int, float)):
                    error = ValueError("parameter 'each' returns invalid type or has invalid parameter type (expected 'int' or 'float'), has too much parameters, or is not a callable object. Use 'lambda' expression, like 'lambda x: Tense.cbrt(x)'.")
                    raise error
    ...
    @classmethod
    def abroadHex(self, value1: _AbroadValue1[_T1], /, value2: _AbroadValue2[_T2] = None, modifier: _AbroadModifier[_T3] = None, mode: _AbroadHexMode = ABROAD_HEX_INCLUDE):
        """
        \\@since 0.3.25
        ```ts
        // created 10.07.2024
    ...
    if False:
        ...
        def isDict(self, v: object, /, ktype: type[_KT] = _Any, vtype: type[_VT] = _Any) -> _tc.TypeIs[dict[_KT, _VT]]:
            ...
            """
            # notice not all classes support isinstance() checking, hence this notation
            if type(v) is dict:
                
                if reckon(v) == 0:
                    
                    if ktype is _Any and vtype is _Any:
                        return True
                ...
                _params = _get_all_params(self.isDict)
                
                if not isinstance(ktype, type):
                    error = TypeError("passed value to parameter '{}' must be a type, not an object".format(_params[0]))
                    raise error
                
                if not isinstance(vtype, type):
                    error = TypeError("passed value to parameter '{}' must be a type, not an object".format(_params[1]))
                    raise error
                
                _placeholder = True
                # fact worth explaining: even if 'type[object]' is deduced, these are still on their type TypeVar counterparts
                _vk = (ktype, list(v.keys()))
                _vv = (vtype, list(v.values()))
                ...
                for _v in (_vk, _vv):
                    ...
                    for e in _v[1]:
                        ...
                        try:
                            # not all types can be inspected with isinstance()
                            _placeholder = _placeholder and isinstance(e, _v[0])
                            
                        except:
                            # the 'is' keyword usage is unnecessary, but some types tend to use '==' operator differently
                            _placeholder = _placeholder and type(e) is _v[0]
    ...
    def reverse(self, v):
        ...
        # note i dont really rely on reversed builtin xd
        if isinstance(v, str):
    ...
    @classmethod
    @_tc.overload
    def bisect(self, a: _tc.SizeableItemGetter[_T_richComparable], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _BisectMode = BISECT_RIGHT, *, key: None = None) -> int:
        """\\@since 0.3.26rc2"""
        ...
    
    @classmethod
    @_tc.overload
    def bisect(self, a: _tc.SizeableItemGetter[_T], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _BisectMode = BISECT_RIGHT, *, key: _cal[[_T], _T_richComparable]) -> int:
        """\\@since 0.3.26rc2"""
        ...
    ...
    if False:
        @classmethod
        def insort(self, seq: _tc.MutableSequence[_T], item: _T, low: int = 0, high: _opt[int] = None, mode: _InsortMode = INSORT_RIGHT, key: _opt[_cal[[_T], _T_richComparable]] = None):
        ...
        @classmethod
        @_tc.overload
        def insort(self, a: _tc.MutableSequence[_T_richComparable], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _InsortMode = INSORT_RIGHT, *, key: None = None) -> None:
            """\\@since 0.3.26rc2"""
            ...
            
        @classmethod
        @_tc.overload
        def insort(self, a: _tc.MutableSequence[_T], x: _T_richComparable, /, l: int = 0, h: _opt[int] = None, dir: _InsortMode = INSORT_RIGHT, *, key: _cal[[_T], _T_richComparable]) -> None:
            """\\@since 0.3.26rc2"""
            ...
    ...
    def until(self, desiredString: _SizeableItemGetter[str], /, message: _opt[str] = None, caseInsensitive: bool = True):
        ...
        while c:
            ...
            if not caseInsensitive:
                c = s != _d if self.isString(_d) else s not in _d
        return self
    ...
    count = _itertools.count # do not initialize
    """\\@since 0.3.34"""
    
    
    if TenseOptions.enableFurryDeclarations: # since 0.3.34
        
        @classmethod
        def __local_owoify_template(self, string: str, /) -> str:
            "\\@since 0.3.26b1"
            s: str = string
            s = _re.sub(r"\s{2}", " UrU ", s, flags = _re.M)
            s = _re.sub(r"XD", "UrU", s, flags = _re.M | _re.I)
            s = _re.sub(r":D", "UrU", s, flags = _re.M)
            s = _re.sub(r"lenny face", "OwO", s, flags = _re.M | _re.I)
            s = _re.sub(r":O", "OwO", s, flags = _re.M | _re.I)
            s = _re.sub(r":\)", ":3", s, flags = _re.M)
            s = _re.sub(r"\?", " uwu?", s, flags = _re.M) # ? is a metachar
            s = _re.sub(r"!", " owo!", s, flags = _re.M)
            s = _re.sub(r"; ", "~ ", s, flags = _re.M)
            s = _re.sub(r", ", "~ ", s, flags = _re.M)
            s = _re.sub(r"you are", "chu is", s, flags = _re.M)
            s = _re.sub(r"You are", "chu is".capitalize(), s, flags = _re.M)
            s = _re.sub(r"You Are", "chu is".title(), s, flags = _re.M)
            s = _re.sub(r"YOU ARE", "chu is".upper(), s, flags = _re.M)
            s = _re.sub(r"wat's this", "OwO what's this", s, flags = _re.M)
            s = _re.sub(r"Wat's [Tt]his", "OwO What's this", s, flags = _re.M)
            s = _re.sub(r"WAT'S THIS", "OwO what's this".upper(), s, flags = _re.M)
            s = _re.sub(r"old person", "greymuzzle", s, flags = _re.M)
            s = _re.sub(r"Old [Pp]erson", "greymuzzle".capitalize(), s, flags = _re.M)
            s = _re.sub(r"OLD PERSON", "greymuzzle".upper(), s, flags = _re.M)
            s = _re.sub(r"forgive me father, I have sinned", "sowwy daddy~ I have been naughty", s, flags = _re.M)
            s = _re.sub(r"Forgive me father, I have sinned", "sowwy daddy~ I have been naughty".capitalize(), s, flags = _re.M)
            s = _re.sub(r"FORGIVE ME FATHER, I HAVE SINNED", "sowwy daddy~ I have been naughty".upper(), s, flags = _re.M)
            s = _re.sub(r"your ", "ur ", s, flags = _re.M)
            s = _re.sub(r"Your ", "Ur ", s, flags = _re.M)
            s = _re.sub(r"YOUR ", "UR ", s, flags = _re.M)
            s = _re.sub(r" your", " ur", s, flags = _re.M)
            s = _re.sub(r" Your", " Ur", s, flags = _re.M)
            s = _re.sub(r" YOUR", " UR", s, flags = _re.M)
            s = _re.sub(r"(^your)| your", "ur", s, flags = _re.M)
            s = _re.sub(r"(^Your)| Your", "Ur", s, flags = _re.M)
            s = _re.sub(r"(^YOUR)| YOUR", "UR", s, flags = _re.M)
            s = _re.sub(r"you", "chu", s, flags = _re.M)
            s = _re.sub(r"You", "Chu", s, flags = _re.M)
            s = _re.sub(r"YOU", "CHU", s, flags = _re.M)
            s = _re.sub(r"with ", "wif ", s, flags = _re.M)
            s = _re.sub(r"With ", "Wif ", s, flags = _re.M)
            s = _re.sub(r"wITH ", "wIF ", s, flags = _re.M)
            s = _re.sub(r"what", "wat", s, flags = _re.M)
            s = _re.sub(r"What", "Wat", s, flags = _re.M)
            s = _re.sub(r"WHAT", "WAT", s, flags = _re.M)
            s = _re.sub(r"toe", "toe bean", s, flags = _re.M)
            s = _re.sub(r"Toe", "Toe Bean", s, flags = _re.M)
            s = _re.sub(r"TOE", "TOE BEAN", s, flags = _re.M)
            s = _re.sub(r"this", "dis", s, flags = _re.M)
            s = _re.sub(r"This", "Dis", s, flags = _re.M)
            s = _re.sub(r"THIS", "DIS", s, flags = _re.M)
            s = _re.sub(r"(?!hell\w+)hell", "hecc", s, flags = _re.M)
            s = _re.sub(r"(?!Hell\w+)Hell", "Hecc", s, flags = _re.M)
            s = _re.sub(r"(?!HELL\w+)HELL", "HECC", s, flags = _re.M)
            s = _re.sub(r"the ", "teh ", s, flags = _re.M)
            s = _re.sub(r"^the$", "teh", s, flags = _re.M)
            s = _re.sub(r"The ", "Teh ", s, flags = _re.M)
            s = _re.sub(r"^The$", "Teh", s, flags = _re.M)
            s = _re.sub(r"THE ", "TEH ", s, flags = _re.M)
            s = _re.sub(r"^THE$", "TEH", s, flags = _re.M)
            s = _re.sub(r"tare", "tail", s, flags = _re.M)
            s = _re.sub(r"Tare", "Tail", s, flags = _re.M)
            s = _re.sub(r"TARE", "TAIL", s, flags = _re.M)
            s = _re.sub(r"straight", "gay", s, flags = _re.M)
            s = _re.sub(r"Straight", "Gay", s, flags = _re.M)
            s = _re.sub(r"STRAIGHT", "GAY", s, flags = _re.M)
            s = _re.sub(r"source", "sauce", s, flags = _re.M)
            s = _re.sub(r"Source", "Sauce", s, flags = _re.M)
            s = _re.sub(r"SOURCE", "SAUCE", s, flags = _re.M)
            s = _re.sub(r"(?!slut\w+)slut", "fox", s, flags = _re.M)
            s = _re.sub(r"(?!Slut\w+)Slut", "Fox", s, flags = _re.M)
            s = _re.sub(r"(?!SLUT\w+)SLUT", "FOX", s, flags = _re.M)
            s = _re.sub(r"shout", "awoo", s, flags = _re.M)
            s = _re.sub(r"Shout", "Awoo", s, flags = _re.M)
            s = _re.sub(r"SHOUT", "AWOO", s, flags = _re.M)
            s = _re.sub(r"roar", "rawr", s, flags = _re.M)
            s = _re.sub(r"Roar", "Rawr", s, flags = _re.M)
            s = _re.sub(r"ROAR", "RAWR", s, flags = _re.M)
            s = _re.sub(r"pawlice department", "paw patrol", s, flags = _re.M)
            s = _re.sub(r"Paw[Ll]ice [Dd]epartment", "Paw Patrol", s, flags = _re.M)
            s = _re.sub(r"PAWLICE DEPARTMENT", "PAW PATROL", s, flags = _re.M)
            s = _re.sub(r"police", "pawlice", s, flags = _re.M)
            s = _re.sub(r"Police", "Pawlice", s, flags = _re.M)
            s = _re.sub(r"POLICE", "PAWLICE", s, flags = _re.M)
            s = _re.sub(r"pervert", "furvert", s, flags = _re.M)
            s = _re.sub(r"Pervert", "Furvert", s, flags = _re.M)
            s = _re.sub(r"PERVERT", "FURVERT", s, flags = _re.M)
            s = _re.sub(r"persona", "fursona", s, flags = _re.M)
            s = _re.sub(r"Persona", "Fursona", s, flags = _re.M)
            s = _re.sub(r"PERSONA", "FURSONA", s, flags = _re.M)
            s = _re.sub(r"perfect", "purrfect", s, flags = _re.M)
            s = _re.sub(r"Perfect", "Purrfect", s, flags = _re.M)
            s = _re.sub(r"PERFECT", "PURRFECT", s, flags = _re.M)
            s = _re.sub(r"(?!not\w+)not", "nawt", s, flags = _re.M)
            s = _re.sub(r"(?!Not\w+)Not", "Nawt", s, flags = _re.M)
            s = _re.sub(r"(?!NOT\w+)NOT", "NAWT", s, flags = _re.M)
            s = _re.sub(r"naughty", "nawt", s, flags = _re.M)
            s = _re.sub(r"Naughty", "Nawt", s, flags = _re.M)
            s = _re.sub(r"NAUGHTY", "NAWT", s, flags = _re.M)
            s = _re.sub(r"name", "nyame", s, flags = _re.M)
            s = _re.sub(r"Name", "Nyame", s, flags = _re.M)
            s = _re.sub(r"NAME", "NYAME", s, flags = _re.M)
            s = _re.sub(r"mouth", "maw", s, flags = _re.M)
            s = _re.sub(r"Mouth", "Maw", s, flags = _re.M)
            s = _re.sub(r"MOUTH", "MAW", s, flags = _re.M)
            s = _re.sub(r"love", "luv", s, flags = _re.M)
            s = _re.sub(r"Love", "Luv", s, flags = _re.M)
            s = _re.sub(r"LOVE", "LUV", s, flags = _re.M)
            s = _re.sub(r"lol", "waw", s, flags = _re.M)
            s = _re.sub(r"Lol", "Waw", s, flags = _re.M)
            s = _re.sub(r"LOL", "WAW", s, flags = _re.M)
            s = _re.sub(r"lmao", "hehe~", s, flags = _re.M)
            s = _re.sub(r"Lmao", "Hehe~", s, flags = _re.M)
            s = _re.sub(r"LMAO", "HEHE~", s, flags = _re.M)
            s = _re.sub(r"kiss", "lick", s, flags = _re.M)
            s = _re.sub(r"Kiss", "Lick", s, flags = _re.M)
            s = _re.sub(r"KISS", "LICK", s, flags = _re.M)
            s = _re.sub(r"lmao", "hehe~", s, flags = _re.M)
            s = _re.sub(r"Lmao", "Hehe~", s, flags = _re.M)
            s = _re.sub(r"LMAO", "HEHE~", s, flags = _re.M)
            s = _re.sub(r"hyena", "yeen", s, flags = _re.M)
            s = _re.sub(r"Hyena", "Yeen", s, flags = _re.M)
            s = _re.sub(r"HYENA", "YEEN", s, flags = _re.M)
            s = _re.sub(r"^hi$", "hai", s, flags = _re.M)
            s = _re.sub(r" hi ", " hai~ ", s, flags = _re.M)
            s = _re.sub(r"hi(,| )", "hai~ ", s, flags = _re.M)
            s = _re.sub(r"hi!", "hai!", s, flags = _re.M)
            s = _re.sub(r"hi\?", "hai?", s, flags = _re.M)
            ...
            s = _re.sub(r"HI!", "HAI!", s, flags = _re.M)
            s = _re.sub(r"HI\?", "HAI?", s, flags = _re.M)
            s = _re.sub(r"(?!handy)hand", "paw", s, flags = _re.M)
            s = _re.sub(r"(?!Handy)Hand", "Paw", s, flags = _re.M)
            s = _re.sub(r"(?!HANDY)HAND", "PAW", s, flags = _re.M)
            s = _re.sub(r"handy", "pawi", s, flags = _re.M)
            s = _re.sub(r"Handy", "Pawi", s, flags = _re.M)
            s = _re.sub(r"HANDY", "PAWI", s, flags = _re.M)
            s = _re.sub(r"for", "fur", s, flags = _re.M)
            s = _re.sub(r"For", "Fur", s, flags = _re.M)
            s = _re.sub(r"FOR", "FUR", s, flags = _re.M)
            s = _re.sub(r"foot", "footpaw", s, flags = _re.M)
            s = _re.sub(r"Foot", "Footpaw", s, flags = _re.M)
            s = _re.sub(r"FOOT", "FOOTPAW", s, flags = _re.M)
            s = _re.sub(r"father", "daddy", s, flags = _re.M)
            s = _re.sub(r"Father", "Daddy", s, flags = _re.M)
            s = _re.sub(r"FATHER", "DADDY", s, flags = _re.M)
            s = _re.sub(r"fuck", "fluff", s, flags = _re.M)
            s = _re.sub(r"Fuck", "Fluff", s, flags = _re.M)
            s = _re.sub(r"FUCK", "FLUFF", s, flags = _re.M)
            s = _re.sub(r"dragon", "derg", s, flags = _re.M)
            s = _re.sub(r"Dragon", "Derg", s, flags = _re.M)
            s = _re.sub(r"DRAGON", "DERG", s, flags = _re.M)
            s = _re.sub(r"(?!doggy)dog", "good boi", s, flags = _re.M)
            s = _re.sub(r"(?!Doggy)Dog", "Good boi", s, flags = _re.M)
            s = _re.sub(r"(?!DOGGY)DOG", "GOOD BOI", s, flags = _re.M)
            s = _re.sub(r"disease", "pathOwOgen", s, flags = _re.M)
            s = _re.sub(r"Disease", "PathOwOgen", s, flags = _re.M)
            s = _re.sub(r"DISEASE", "PATHOWOGEN", s, flags = _re.M)
            s = _re.sub(r"cyborg|robot|computer", "protogen", s, flags = _re.M)
            s = _re.sub(r"Cyborg|Robot|Computer", "Protogen", s, flags = _re.M)
            s = _re.sub(r"CYBORG|ROBOT|COMPUTER", "PROTOGEN", s, flags = _re.M)
            s = _re.sub(r"(?!children)child", "cub", s, flags = _re.M)
            s = _re.sub(r"(?!Children)Child", "Cub", s, flags = _re.M)
            s = _re.sub(r"(?!CHILDREN)CHILD", "CUB", s, flags = _re.M)
            s = _re.sub(r"(?!cheese[ds])cheese", "sergal", s, flags = _re.M)
            s = _re.sub(r"(?!Cheese[ds])Cheese", "Sergal", s, flags = _re.M)
            s = _re.sub(r"(?!CHEESE[DS])CHEESE", "SERGAL", s, flags = _re.M)
            s = _re.sub(r"celebrity", "popufur", s, flags = _re.M)
            s = _re.sub(r"Celebrity", "Popufur", s, flags = _re.M)
            s = _re.sub(r"CELEBRITY", "POPUFUR", s, flags = _re.M)
            s = _re.sub(r"bye", "bai", s, flags = _re.M)
            s = _re.sub(r"Bye", "Bai", s, flags = _re.M)
            s = _re.sub(r"BYE", "BAI", s, flags = _re.M)
            s = _re.sub(r"butthole", "tailhole", s, flags = _re.M)
            s = _re.sub(r"Butthole", "Tailhole", s, flags = _re.M)
            s = _re.sub(r"BUTTHOLE", "TAILHOLE", s, flags = _re.M)
            s = _re.sub(r"bulge", "bulgy-wulgy", s, flags = _re.M)
            s = _re.sub(r"Bulge", "Bulgy-wulgy", s, flags = _re.M)
            s = _re.sub(r"BULGE", "BULGY-WULGY", s, flags = _re.M)
            s = _re.sub(r"bite", "nom", s, flags = _re.M)
            s = _re.sub(r"Bite", "Nom", s, flags = _re.M)
            s = _re.sub(r"BITE", "NOM", s, flags = _re.M)
            s = _re.sub(r"awful", "pawful", s, flags = _re.M)
            s = _re.sub(r"Awful", "Pawful", s, flags = _re.M)
            s = _re.sub(r"AWFUL", "PAWFUL", s, flags = _re.M)
            s = _re.sub(r"awesome", "pawsome", s, flags = _re.M)
            s = _re.sub(r"Awesome", "Pawsome", s, flags = _re.M)
            s = _re.sub(r"AWESOME", "PAWSOME", s, flags = _re.M)
            s = _re.sub(r"(?!ahh(h)+)ahh", "murr", s, flags = _re.M)
            s = _re.sub(r"(?!Ahh[Hh]+)Ahh", "Murr", s, flags = _re.M)
            s = _re.sub(r"(?!AHH(H)+)AHH", "MURR", s, flags = _re.M)
            s = _re.sub(r"(?![Gg]reymuzzle|[Tt]ail(hole)?|[Pp]aw [Pp]atrol|[Pp]awlice|luv|lick|[Ff]luff|[Ss]ergal|[Pp]awful)l", "w", s, flags = _re.M)
            s = _re.sub(r"(?!GREYMUZZLE|TAIL(HOLE)?|PAW PATROL|PAWLICE|L(uv|UV)|L(ick|ICK)|FLUFF|SERGAL|PAWFUL)L", "W", s, flags = _re.M)
            s = _re.sub(r"(?![Gg]reymuzzle|ur|[Rr]awr|[Ff]ur(sona|vert)?|[Pp]urrfect|[Vv]ore|[Dd]erg|[Pp]rotogen|[Ss]ergal|[Pp]opufur|[Mm]urr)r", "w", s, flags = _re.M)
            s = _re.sub(r"(?!GREYMUZZLE|UR|RAWR|FUR(SONA|VERT)?|PURRFECT|VORE|DERG|PROTOGEN|SERGAL|POPUFUR|MURR)R", "W", s, flags = _re.M)
            # above: 0.3.26a3, below: 0.3.26b1
            s = _re.sub(r"gweymuzzwe", "greymuzzle", s, flags = _re.M)
            s = _re.sub(r"Gweymuzzwe", "Greymuzzle", s, flags = _re.M)
            s = _re.sub(r"GWEYMUZZWE", "GREYMUZZLE", s, flags = _re.M)
            s = _re.sub(r"taiwhowe", "tailhole", s, flags = _re.M)
            s = _re.sub(r"Taiwhowe", "Tailhole", s, flags = _re.M)
            s = _re.sub(r"TAIWHOWE", "TAILHOLE", s, flags = _re.M)
            s = _re.sub(r"paw patwow", "paw patrol", s, flags = _re.M)
            s = _re.sub(r"Paw Patwow", "Paw Patrol", s, flags = _re.M)
            s = _re.sub(r"PAW PATWOW", "PAW PATROL", s, flags = _re.M)
            s = _re.sub(r"pawwice", "pawlice", s, flags = _re.M)
            s = _re.sub(r"Pawwice", "Pawlice", s, flags = _re.M)
            s = _re.sub(r"PAWWICE", "PAWLICE", s, flags = _re.M)
            s = _re.sub(r"wuv", "luv", s, flags = _re.M)
            s = _re.sub(r"Wuv", "Luv", s, flags = _re.M)
            s = _re.sub(r"WUV", "LUV", s, flags = _re.M)
            s = _re.sub(r"wick", "lick", s, flags = _re.M)
            s = _re.sub(r"Wick", "Lick", s, flags = _re.M)
            s = _re.sub(r"WICK", "LICK", s, flags = _re.M)
            s = _re.sub(r"fwuff", "fluff", s, flags = _re.M)
            s = _re.sub(r"Fwuff", "Fluff", s, flags = _re.M)
            s = _re.sub(r"FWUFF", "FLUFF", s, flags = _re.M)
            s = _re.sub(r"sewgaw", "sergal", s, flags = _re.M)
            s = _re.sub(r"Sewgaw", "Sergal", s, flags = _re.M)
            s = _re.sub(r"SEWGAW", "SERGAL", s, flags = _re.M)
            s = _re.sub(r"pawfuw", "pawful", s, flags = _re.M)
            s = _re.sub(r"Pawfuw", "Pawful", s, flags = _re.M)
            s = _re.sub(r"PAWFUW", "PAWFUL", s, flags = _re.M)
            s = _re.sub(r"(?!uwu)uw", "ur", s, flags = _re.M)
            s = _re.sub(r"(?!Uwu)Uw", "Ur", s, flags = _re.M)
            s = _re.sub(r"(?!UWU)UW", "UR", s, flags = _re.M)
            s = _re.sub(r"waww", "rawr", s, flags = _re.M)
            s = _re.sub(r"Waww", "Rawr", s, flags = _re.M)
            s = _re.sub(r"WAWW", "RAWR", s, flags = _re.M)
            s = _re.sub(r"fuw", "fur", s, flags = _re.M)
            s = _re.sub(r"Fuw", "Fur", s, flags = _re.M)
            s = _re.sub(r"FUW", "FUR", s, flags = _re.M)
            s = _re.sub(r"furvewt", "furvert", s, flags = _re.M)
            s = _re.sub(r"Furvewt", "Furvert", s, flags = _re.M)
            s = _re.sub(r"FURVEWT", "FURVERT", s, flags = _re.M)
            s = _re.sub(r"puwwfect", "purrfect", s, flags = _re.M)
            s = _re.sub(r"Puwwfect", "Purrfect", s, flags = _re.M)
            s = _re.sub(r"PUWWFECT", "PURRFECT", s, flags = _re.M)
            s = _re.sub(r"vowe", "vore", s, flags = _re.M)
            s = _re.sub(r"Vowe", "Vore", s, flags = _re.M)
            s = _re.sub(r"VOWE", "VORE", s, flags = _re.M)
            s = _re.sub(r"dewg", "derg", s, flags = _re.M)
            s = _re.sub(r"Dewg", "Derg", s, flags = _re.M)
            s = _re.sub(r"DEWG", "DERG", s, flags = _re.M)
            s = _re.sub(r"pwotogen", "protogen", s, flags = _re.M)
            s = _re.sub(r"Pwotogen", "Protogen", s, flags = _re.M)
            s = _re.sub(r"PWOTOGEN", "PROTOGEN", s, flags = _re.M)
            s = _re.sub(r"popufuw", "popufur", s, flags = _re.M)
            s = _re.sub(r"Popufuw", "Popufur", s, flags = _re.M)
            s = _re.sub(r"POPUFUW", "POPUFUR", s, flags = _re.M)
            s = _re.sub(r"muww", "murr", s, flags = _re.M)
            s = _re.sub(r"Muww", "Murr", s, flags = _re.M)
            s = _re.sub(r"MUWW", "MURR", s, flags = _re.M)
            # end 0.3.26b1; start 0.3.26rc2
            s = _re.sub(r"furwy", "fuwwy", s, flags = _re.M)
            s = _re.sub(r"Furwy", "Fuwwy", s, flags = _re.M)
            s = _re.sub(r"FURWY", "FUWWY", s, flags = _re.M)
            s = _re.sub(r"UrU", "UwU", s, flags = _re.M)
            s = _re.sub(r"Uru", "Uwu", s, flags = _re.M)
            s = _re.sub(r"uru", "uwu", s, flags = _re.M)
            s = _re.sub(r"URU", "UWU", s, flags = _re.M)
            s = _re.sub(r"femboy", "femboi", s, flags = _re.M)
            s = _re.sub(r"Femboy", "Femboi", s, flags = _re.M)
            s = _re.sub(r"FEMBOY", "FEMBOI", s, flags = _re.M)
            s = _re.sub(r":<", "x3", s, flags = _re.M)
            # end 0.3.26rc2; start 0.3.26
            s = _re.sub(r"ding", "beep", s, flags = _re.M)
            s = _re.sub(r"Ding", "Beep", s, flags = _re.M)
            s = _re.sub(r"DING", "BEEP", s, flags = _re.M)
            s = _re.sub(r"shourd", "shouwd", s, flags = _re.M)
            s = _re.sub(r"Shourd", "Shouwd", s, flags = _re.M)
            s = _re.sub(r"SHOURD", "SHOUWD", s, flags = _re.M)
            s = _re.sub(r"course", "couwse", s, flags = _re.M)
            s = _re.sub(r"Course", "Couwse", s, flags = _re.M)
            s = _re.sub(r"COURSE", "COUWSE", s, flags = _re.M)
            return s
    ...
    def aeify(self, s: str, /):
        ...
        """
        _s = s
        _s = _re.sub(r"[ae]", "\u00E6", _s, flags = _re.M)
        _s = _re.sub(r"[AE]", "\u00C6", _s, flags = _re.M)
        return _s
    ...
    def oeify(self, s: str, /):
        ...
        """
        _s = s
        _s = _re.sub(r"[oe]", "\u0153", _s, flags = _re.M)
        _s = _re.sub(r"[OE]", "\u0152", _s, flags = _re.M)
        return _s
...
class FinalVar:
    ...
    ```py
    class FinalVar: ...
    from tense[.types_collection] import FinalVar
    ```
...
class ClassLike:
    ...
    def __new__(cls, f: _tc.Callable[_P, _T]):
        """
        \\@since 0.3.27a3 \\
        \\@lifetime ≥ 0.3.27a3
        ```
...
class ChangeVar(_tc.UnaryOperable, _tc.Comparable, _tc.AdditionReassignable, _tc.SubtractionReassignable):
    ...
    def setModifier(self, m: int):
        ...
        """
        _params = [k for k in self.setModifier.__annotations__]
