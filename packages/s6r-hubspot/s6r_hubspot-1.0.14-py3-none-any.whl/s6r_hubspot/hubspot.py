...
class HubspotConnection(HubDb, CustomObject):
    ...
    def send_request_response_with_paging(
            self, model, url, params, function_name, max_retries, retry_delay, cc=0, **kwargs):
        ...
        while retries < max_retries:
            ...
            if ('status' in data and data['status'] == 'COMPLETE') or ('total' in data):
                ...
                if after and cc < 6:
                    cc += 1
                    return getattr(self, function_name)(model, after=after, cc=cc, **kwargs)
                elif after:
    ...
    def search(self, model, after=0, max_retries=3, retry_delay=1, cc=0, **kwargs):
        """
