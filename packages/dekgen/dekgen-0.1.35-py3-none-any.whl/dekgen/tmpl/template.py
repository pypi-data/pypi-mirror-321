    ...
    def render_string(self, template_file, close_tpl=False):
        if close_tpl:
            ...
            try:
                ...
                raise RenderException(f'Render error: {template_file}: {e.args} : {self.variables}')
    ...
    def render_dir(self, path_target, path_template, force_close_tpl=False, open_ignore_override=True):
        ...
        for root, _, files in os.walk(path_template):
            ...
            for f in files:
                ...
                if open_ignore_override and os.path.exists(target_file) and match_ignore_override(fp):
                    continue
                if is_bin:
                    write_file(target_file, sb=self.variables[self.render(read_text(fp)).strip()])
                else:
                    self.render_file(target_file, fp, force_close_tpl or match_ignore_tpl(fp))
