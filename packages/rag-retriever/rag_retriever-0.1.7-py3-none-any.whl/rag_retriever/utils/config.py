...
def ensure_user_directories() -> None:
    ...
    try:
        ...
        if not config_dir.exists():
            ...
            if not config_dir.exists():
                raise RuntimeError(f"Failed to create config directory at {config_dir}")
            logger.debug("Created config directory: %s", config_dir)

        if not data_dir.exists():
            data_dir.mkdir(parents=True, exist_ok=True)
            if not data_dir.exists():
                raise RuntimeError(f"Failed to create data directory at {data_dir}")
        ...
        try:
            ...
            raise RuntimeError(
                f"Config directory {config_dir} is not writable. Error: {e}"
            )

        logger.debug(
            "Created and verified user directories: %s, %s", config_dir, data_dir
        )
    except Exception as e:
        raise RuntimeError(
            f"Failed to create or verify directories. Error: {e}\n"
            "Please ensure you have permissions to create these directories."
        )
