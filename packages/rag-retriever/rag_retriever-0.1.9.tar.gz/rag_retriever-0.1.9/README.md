...
On Windows, you must use PowerShell as Administrator for installation and setup:

1. Open PowerShell as Administrator (right-click PowerShell and select "Run as Administrator")

   > **Note**: Administrator rights are required to create directories in `%PROGRAMDATA%`
...
5. Initialize the configuration (still in Administrator PowerShell):

```powershell
rag-retriever --init
```

Note: Keep PowerShell running as Administrator for any commands that modify configuration or data.
...
- A configuration file at `~/Library/Application Support/rag-retriever/config.yaml` (Unix/Mac) or `%PROGRAMDATA%\rag-retriever\config\config.yaml` (Windows)
- A `.env` file in the same directory for your OpenAI API key

### Setting up your API Key

Add your OpenAI API key to your config.yaml file:
...
Note: While environment variables can be used as a fallback, using the config file is recommended.
...
- Unix/Mac: `~/Library/Application Support/rag-retriever/chromadb/`
- Windows: `%PROGRAMDATA%\rag-retriever\data\chromadb\`
...
- Unix/Mac: `~/Library/Application Support/rag-retriever/config.yaml`
- Windows: `%PROGRAMDATA%\rag-retriever\config\config.yaml`
...
# On Unix/Mac:
rm -rf ~/Library/Application\ Support/rag-retriever
# On Windows (run in PowerShell):
Remove-Item -Recurse -Force "$env:PROGRAMDATA\rag-retriever"
```
...
1. Configure your Confluence credentials in your config file (see Configuration Files section above):
