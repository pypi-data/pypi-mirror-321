...
def get_config_dir() -> Path:
    """Get system-wide config directory path."""
    if os.name == "nt":  # Windows
        programdata = os.environ.get("PROGRAMDATA")
        if not programdata:
            raise RuntimeError("PROGRAMDATA environment variable not found")
        logger.debug("Using Windows PROGRAMDATA path: %s", programdata)
        return Path(programdata) / "rag-retriever" / "config"
    # Keep existing behavior for non-Windows
    logger.debug("Using Unix/Mac user config path")
    return Path(user_config_dir("rag-retriever", appauthor=False))


def get_data_dir() -> Path:
    """Get system-wide data directory path."""
    if os.name == "nt":  # Windows
        programdata = os.environ.get("PROGRAMDATA")
        if not programdata:
            raise RuntimeError("PROGRAMDATA environment variable not found")
        return Path(programdata) / "rag-retriever" / "data"
    # Keep existing behavior for non-Windows
...
def ensure_user_directories() -> None:
    ...
    def verify_directory(path: Path, retries: int = 3) -> None:
        """Create and verify a directory with retries."""
        for attempt in range(retries):
            try:
                ...
                # Verify existence
                if not path.exists():
                    raise RuntimeError(f"Directory not created: {path}")

                # Verify write access
                test_file = path / f"write_test_{attempt}.txt"
                test_file.write_text("test")
                test_file.unlink()

                logger.info(f"Verified directory at: {path}")
                return

            except Exception as e:
                if attempt == retries - 1:
                    raise RuntimeError(f"Failed to create/verify directory {path}: {e}")
                time.sleep(0.5)  # Wait before retry

    config_dir = get_config_dir()
    data_dir = get_data_dir()

    verify_directory(config_dir)
