...
def upload_export(
    ...
    with ui.spinner("Requesting upload") as spinner:
        upload = project_api.get_upload_link(project_id, name, format)
...
@click.option(
    ...
    default="json",
    type=click.Choice(["json", "json_with_png"], case_sensitive=False),
)
@click.pass_context
def check(ctx, name, project, format):
    ...
    try:
        ...
        check_id = upload_export(project_api, project_id, export, name, format)
