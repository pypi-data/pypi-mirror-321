...
def GeneTree(args):
    ...
    # prepare commands
    mafft_cmd       = 'mafft-einsi --thread %s --quiet %s > %s/%s'              % (num_threads, seq_file, op_dir, msa_file)
    trim_cmd        = 'java -jar %s -i %s/%s -m %s -t AA -h %s -of %s/%s'       % (pwd_bmge_jar, op_dir, msa_file, bmge_trim_model, bmge_entropy_score_cutoff, op_dir, msa_file_bmge)

    msa_for_iqtree = msa_file
    if trim_with_bmge is True:
        msa_for_iqtree = msa_file_bmge

    iqtree_cmd      = '%s -m LG+G+I -bb 1000 --wbtl -nt %s -s %s/%s -pre %s'    % (iqtree_exe, num_threads, op_dir, msa_for_iqtree, sep_file_base)
