...
def get_nsi_damages_pixel(
    ...
    flooding_upsampled = flooding.interp(
        x=np.linspace(flooding.x.min(), flooding.x.max(), flooding.sizes['x'] * resolution_upsample),
        y=np.linspace(flooding.y.min(), flooding.y.max(), flooding.sizes['y'] * resolution_upsample),
        method='nearest'
    )
