...
from appdynamics.lib import change_permissions_recursively
...
def make_virtual_bindeps_package():
    ...
    lib_dir = os.path.join(config.AGENT_BASE_DIR, 'lib')
    dest_root = os.path.join(lib_dir, prefix, 'site-packages')
    ...
    if tombstone_version == 'unknown' or tombstone_version != agent_version:
        ...
        try:
            make_virtual_bindeps_destination(dest, mode=0o744)
            zmq_target = 'zmq-' + abi
    ...
    # Changing permission for root directory recursively to 744
    change_permissions_recursively(lib_dir, mode=0o744)
...
def make_virtual_bindeps_destination(dest, mode=0o777):
    try:
