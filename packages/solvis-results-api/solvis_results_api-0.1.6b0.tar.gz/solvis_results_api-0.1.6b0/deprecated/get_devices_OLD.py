...
class GetDevices:
    def get_devices(self, user: str, password: str, env='sistema') -> pd.DataFrame:
        """Get surveyed units
        ...
        def request_api():
            ...
            response_survey = self.session.get(
                f'https://{env}.solvis.net.br/api/v1/devices' f'?page={str(page)}',
                headers=headers,
            )
        ...
        while True:
            ...
            if response.status_code != 200:
                ...
                if answers:
                    df_devices = pd.json_normalize(answers)
                    self.df_temp = pd.concat(
                        [self.df_temp, df_devices], ignore_index=True
                    )
                    ...
                    print(f'Total de devices: {total}')
                    break
