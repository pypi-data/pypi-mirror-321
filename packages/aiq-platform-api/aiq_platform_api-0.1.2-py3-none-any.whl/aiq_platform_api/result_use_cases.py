...
from .common_utils import AttackIQRestClient, AttackIQLogger, ResultsUtils
from .env import ATTACKIQ_API_TOKEN, ATTACKIQ_PLATFORM_URL

logger = AttackIQLogger.get_logger(__name__)


def list_results(client: AttackIQRestClient, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None,
        limit: Optional[int] = None, ):
    logger.info(f"Listing results (limit: {limit if limit else 'None'})...")
    count = 0
    results = ResultsUtils.get_results(client, start_date=start_date, end_date=end_date, )
