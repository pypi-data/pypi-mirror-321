...
from .common_utils import AttackIQRestClient, AttackIQLogger, PhaseResultsUtils
from .env import ATTACKIQ_API_TOKEN, ATTACKIQ_PLATFORM_URL

logger = AttackIQLogger.get_logger(__name__)


def list_phase_results(client: AttackIQRestClient, assessment_id: str, project_run_id: Optional[str] = None,
        result_summary_id: Optional[str] = None, limit: Optional[int] = 10, ):
    logger.info("Listing phase results ...")
    count = 0
    phase_results = PhaseResultsUtils.get_phase_results(client, assessment_id=assessment_id,
        project_run_id=project_run_id, result_summary_id=result_summary_id, )
