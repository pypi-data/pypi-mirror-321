...
def get_assessment_results(client: AttackIQRestClient, assessment_id: str, limit: Optional[int] = None):
...
def wait_for_assessment_completion(client: AttackIQRestClient, assessment_id: str, run_id: Optional[str] = None,
    ...
    while time.time() - start_time < timeout:
        if not AssessmentUtils.is_assessment_running(client, assessment_id):
            ...
            logger.info(f"Assessment {assessment_id} (Run ID: {run_id}) completed in {elapsed_time} seconds.")
            return True

        if check_interval > 1:
            logger.info(f"Assessment {assessment_id} is still running. Waiting {check_interval} seconds...")
        time.sleep(check_interval)

    logger.warning(f"Assessment {assessment_id} (Run ID: {run_id}) did not complete within {timeout} seconds.")
    return False


def list_assets_in_assessment(client: AttackIQRestClient, project_id: str, limit: Optional[int] = None):
    logger.info(f"Listing assets for project ID: {project_id} (limit: {limit if limit else 'None'})...")
...
def main():
    ...
    if assessment_id:
        ...
        if run_id:
            wait_for_assessment_completion(client, assessment_id, run_id, timeout=20, check_interval=1)
    else:
        logger.warning(
