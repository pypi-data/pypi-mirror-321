...
from .common_utils import (AttackIQRestClient, AttackIQLogger, PhaseLogsUtils, AssessmentUtils, )
from .env import ATTACKIQ_API_TOKEN, ATTACKIQ_PLATFORM_URL

logger = AttackIQLogger.get_logger(__name__)


def list_phase_logs(client: AttackIQRestClient, scenario_job_id: str, limit: Optional[int] = 10, ):
    logger.info(f"Listing phase logs for scenario job ID: {scenario_job_id}...")
    count = 0
    logs = PhaseLogsUtils.get_phase_logs(client, scenario_job_id=scenario_job_id, )
...
def get_assessment_results(client: AttackIQRestClient, assessment_id: str, limit: int = 10):
