...
class ACIProcessorQueueControllerABC(metaclass=ABCMeta):
    ...
    def __vacuumTablesWrapped(self):
        ...
        for Declarative_ in self._VacuumDeclaratives:
            ...
            rawConn = self._dbSessionCreator.bind.connect()
            rawConn.set_isolation_level("AUTOCOMMIT")
            ...
            try:
                ...
                # from 13.2.1. Read Committed Isolation Level
                # https://www.postgresql.org/docs/15/transaction-iso.html
                rawConn._set_isolation_level(IsolationLevel.READ_COMMITTED)
                rawConn.close()
                cursor.close()
