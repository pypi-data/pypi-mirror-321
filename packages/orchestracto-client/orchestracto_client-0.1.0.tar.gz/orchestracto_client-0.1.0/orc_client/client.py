...
class OrcClient:
    ...
    def get_runs(
        ...
        if start_dt is not None:
            params["start_dt"] = start_dt.strftime("%Y-%m-%dT%H:%M:%S")

        if end_dt is not None:
            params["end_dt"] = end_dt.strftime("%Y-%m-%dT%H:%M:%S")
    ...
    def update_workflow(self, workflow_path: str, workflow: dict[str, Any] | None = None):
        req_data = {
            "workflow_path": workflow_path,
        }
        if workflow is not None:
