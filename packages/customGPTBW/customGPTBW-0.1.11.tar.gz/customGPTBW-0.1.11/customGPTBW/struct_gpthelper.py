...
class Dataset():
    ...
    def mask_qu_multiple(self,tokenizer ,conversations ,len=1024,batch_size=8):
        ...
        for conversation in tqdm(conversations, total=len(conversations), desc="Processing conversations"):
        ...
        dataset = self.create_tf_dataset(inputs=tf.concat(all_input_ids, axis=0),attention_mask=tf.concat(all_attention_masks, axis=0),labels=tf.concat(all_labels, axis=0)).batch(batch_size).shuffle(buffer_size=75000)
