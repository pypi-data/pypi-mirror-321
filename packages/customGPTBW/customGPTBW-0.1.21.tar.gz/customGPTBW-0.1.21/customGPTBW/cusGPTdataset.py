...
class Dataset():
    ...
    def __call__(self,file_name,tokenizer,first_n=None,start_from=0,name="WICKED4950/MentalHeathEsther",local=False,answer_only=True,batch_size=8,maskq=False,toe_len=512):
        ...
        try:
            ...
            if maskq:
                self.datasetgptraw(answer_only=answer_only)
                self.mask_qu_multiple(conversations=self.gpt_sentence,tokenizer=tokenizer,len_=toe_len,batch_size=batch_size)
            else:
                self.dataset_gpt(tokenizer=tokenizer,max_length=toe_len,batch_size=batch_size)
