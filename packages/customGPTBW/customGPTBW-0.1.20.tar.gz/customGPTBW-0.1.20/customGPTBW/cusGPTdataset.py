...
class Dataset():
    ...
    def mask_qu_multiple(self,tokenizer ,conversations ,len_=1024,batch_size=8):
        all_input_ids = []
        all_attention_masks = []
        all_labels = []
        ...
        for index in tqdm(range(tokenized_output["input_ids"].shape[0]), desc="Processing conversations", unit="conversation"):
            ...
            for idx, token in enumerate(words):
                if attention_mask[idx_] == 0:
                    break
                if words[idx_] == "<|SOH|>":  # If it's a question start token, mask the subsequent tokens
                    try:
                        while words[idx_] != "<|SOB|>":
                            labels[idx_ + 1] = -100
                            idx += 1
                            idx_ = idx + idx_
                    except:
    ...
    def __call__(self,file_name,tokenizer,first_n=None,start_from=0,name="WICKED4950/MentalHeathEsther",local=False,answer_only=True,batch_size=8,maskq=False):
        ...
        try:
            ...
            if maskq:
                self.datasetgptraw(answer_only=answer_only)
                self.mask_qu_multiple(conversations=self.gpt_sentence,tokenizer=tokenizer,len_=512,batch_size=batch_size)
            else:
