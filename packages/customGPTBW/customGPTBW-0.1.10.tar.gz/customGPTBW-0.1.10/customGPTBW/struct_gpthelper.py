...
class Dataset():
    ...
    def mask_qu_multiple(self, tokenizer, conversations, len=1024, batch_size=8):
        ...
        # Tokenizing all conversations outside the loop
        tokenized_outputs = tokenizer(
            ...
            is_split_into_words=True  # Make sure to split the input text properly
        )

        # Iterate over tokenized conversations
        for i, conversation in enumerate(conversations):
            input_ids = tokenized_outputs.input_ids[i]
            attention_mask = tokenized_outputs.attention_mask[i]
            ...
            words = tokenizer.convert_ids_to_tokens(input_ids.numpy())
