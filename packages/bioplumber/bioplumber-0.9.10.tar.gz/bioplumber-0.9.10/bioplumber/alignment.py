...
def convert_sam_bam_(
    ...
    if container=="none":
        ...
        outdir=Path(outdir).absolute()
        outdir_parent=Path(outdir).parent
        command = f"singularity exec -B {sam_file}:{sam_file} -B {outdir_parent}:{outdir_parent} {config.singularity_container} samtools view -bS {sam_file}| samtools sort -o {outdir}"
