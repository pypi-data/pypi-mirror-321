...
def plot_grid_comparison(grid1: Grid | str | Path, grid2: Grid | str | Path, outfile: str | Path, custom_shapefile=None, add_netherlands_shapefile=False, title1=None, title2=None, suptitle=None):
    ...
    grid2_resampled.gridresample(grid2use=grid1)
    diff_grid = grid1 - grid2_resampled

    # convert to xarrays
    grid1 = grids_to_xarray([grid1], labels=["grid1"])
    grid2 = grids_to_xarray([grid2], labels=["grid2"])
    grid2_resampled = grids_to_xarray([grid2_resampled], labels=["grid2_resampled"])
    diff_grid = grids_to_xarray([diff_grid], labels=["grid difference"])
    ...
    plot_grid(grid1.isel(grid=0), axes=grid_axes[0], cmap=cmaps[0], vmin=vmin, vmax=vmax, zoom=True, custom_shapefile=custom_shapefile, add_netherlands_shapefile=add_netherlands_shapefile)
    plot_grid(grid2.isel(grid=0), axes=grid_axes[1], cmap=cmaps[1], vmin=vmin, vmax=vmax, zoom=True, custom_shapefile=custom_shapefile, add_netherlands_shapefile=add_netherlands_shapefile)
    plot_grid(diff_grid.isel(grid=0), axes=grid_axes[2], cmap=cmaps[2], vmin=-max_abs_val, vmax=max_abs_val, zoom=True, custom_shapefile=custom_shapefile, add_netherlands_shapefile=add_netherlands_shapefile)

    # make histograms
    vmins = [vmin, vmin, -max_abs_val]
    vmaxs = [vmax, vmax, max_abs_val]
    for i in range(3):
        data = grid_list[i].isel(grid=0).data
    ...
    add_background_hist(hist_axes[0], grid2_resampled.isel(grid=0).data)
    hist_axes[0].set_title(f"Grid 1 dx: {grid1.x.data[1] - grid1.x.data[0]}m, dy: {grid1.y.data[1] - grid1.y.data[0]}m")
    add_background_hist(hist_axes[1], grid1.isel(grid=0).data)
