...
class ControllerMeta(type):
    def __new__(cls, name, bases, attrs):
        ...
        def create_http_endpoint(
            ...
            # If the method has @_requires_auth, inject a credentials param
            if getattr(bound_method, "_requires_auth", False):
                credentials_param = inspect.Parameter(
                    name="credentials",
                    kind=inspect.Parameter.POSITIONAL_OR_KEYWORD,
                    default=security,  # Security(...) object
                    annotation=HTTPAuthorizationCredentials
                )
                # Insert the credentials param at the end of the param list
                # new_sig = new_sig.replace(
                #     parameters=[*params, credentials_param]
                # )
            ...
            for http_method in methods:
                getattr(router, http_method.lower())(path, response_model=None)(endpoint)
