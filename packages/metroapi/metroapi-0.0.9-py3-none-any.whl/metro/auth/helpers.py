...
async def get_current_user_from_token(
    ...
    user = UserBase.verify_auth_token(token)
    if not user:
        raise UnauthorizedError("Invalid authentication token")
...
def dummy_requires_auth(function = None):
    print("dummy requires auth")
    pass
...
def requires_auth(function=None):
    ...
    async def wrapper(*args, **kwargs):
        ...
        print("being used as decorator (in decorator)")
        ...
        # if being used in controller context:
        if hasattr(args[0], '__class__') and isinstance(args[0], Controller):
            controller = args[0]
            req = kwargs.get('request')
            creds = kwargs.get('credentials')

            curr_user = get_current_user_from_token(credentials=creds)
            ...
            req = next((arg for arg in args if isinstance(arg, Request)), kwargs.get('request'))
            if not req:
                raise ValueError("Request object not found in arguments")

            curr_user = get_current_user(req)
            kwargs['user'] = curr_user
