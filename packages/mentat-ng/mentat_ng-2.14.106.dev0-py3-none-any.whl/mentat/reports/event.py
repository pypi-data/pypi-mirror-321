...
class EventReporter(BaseReporter):
    ...
    def _get_categories(self, events: dict[str, list[mentat.idea.internal.Idea]]) -> set[str]:
        """
        Extracts and returns a set of all categories from the given IDEA events.

        :param dict events: A dictionary where keys are event types ('regular', 'relapsed') and values are lists of IDEA events.
        :return: A set of unique categories from the events.
        :rtype: set[str]
        """
        all_categories = set()

        for t in ["regular", "relapsed"]:
            event_group = events.get(t, [])
            for event in event_group:
                all_categories.update(event.get_categories())

        return all_categories

    def _get_event_classes(self, events: dict[str, list[mentat.idea.internal.Idea]], target: bool = False) -> set[str]:
        """
        Extracts and returns a set of all event classes from the given IDEA events.
        Optionally, it can extract target classes if specified.

        :param dict events: A dictionary where keys are event types ('regular', 'relapsed') and values are lists of IDEA events.
        :param bool target: If True, extract target classes; otherwise, extract regular classes.
        :return: A set of unique event classes from the events.
        :rtype: set[str]
        """
        all_classes = set()
        ...
        for t in ["regular", "relapsed"]:
            event_group = events.get(t, [])
            for event in event_group:
                cls = event.get_target_class() if target else event.get_class()
                if cls is not None:
                    all_classes.add(str(cls))
    ...
    def report_target(self, result, events, group_chain, fallback_groups, settings, severity, time_l, time_h, original_group_only, template_vars = None, testdata = False):
        ...
        if final_group_list:
            additional_headers = {
                "categories": self._get_categories(events),
                "event_classes": self._get_event_classes(events, target=True),
            }
            self._mail_report(report, self.settings_dict[final_group_list[0]], final_group_list, fallback_groups,
                              result, template_vars, additional_headers=additional_headers)
    ...
    def report_summary(self, result, events, group_chain, fallback_groups, settings, severity, time_l, time_h, original_group_only, template_vars = None, testdata = False):
        ...
        if final_group_list:
            additional_headers = {
                "categories": self._get_categories(events),
                "event_classes": self._get_event_classes(events),
            }
            self._mail_report(report, self.settings_dict[final_group_list[0]], final_group_list, fallback_groups, result, template_vars, additional_headers=additional_headers)
        # Commit all changes on report object to database.
        self.sqlservice.session.commit()
    ...
    def report_extra(self, parent_rep, result, events, group_chain, fallback_groups, settings, severity, time_l, time_h, template_vars = None, testdata = False):
        ...
        for src in sorted(sources):
            ...
            if final_group_list:
                ...
                self._mail_report(report, self.settings_dict[final_group_list[0]], final_group_list, fallback_groups, result, template_vars, src, additional_headers)
