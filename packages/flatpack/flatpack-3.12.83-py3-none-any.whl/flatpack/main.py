...
def add_package_to_toml(
    ...
    """Add a package to the specified section in flatpack.toml, maintaining alphabetical order.

    Args:
        toml_path (Path): Path to the flatpack.toml file
        package_type (str): Either 'python' or 'unix'
        package_name (str): Name of the package to add
        version (str): Version of the package, defaults to "*"
    """
    try:
        ...
        if package_type not in config["packages"]:
            config["packages"][package_type] = {}

        config["packages"][package_type][package_name] = version

        config["packages"][package_type] = dict(
            sorted(config["packages"][package_type].items()))

        with open(toml_path, "w") as f:
            toml.dump(config, f)

        logger.info(
            f"Added {package_type} package: {package_name} = {version}")

    except Exception as e:
        logger.error(f"Failed to add package to toml: {str(e)}")
        raise
...
def fpk_unbox(
    ...
    if overwrite and target_dir.exists():
        try:
            ...
            logger.info(f"Removed existing contents of {target_dir}")
        except Exception as e:
            logger.error(f"Error removing existing directory contents: {e}")
            return False
