...
def read_text(filepath, default=DEFAULT_VALUE, encoding='utf-8', lines=False):
...
def read_lines(filepath, default=DEFAULT_VALUE, encoding='utf-8', skip_empty=False, strip=True, trace=False):
    for i, line in enumerate(read_text(filepath, default=default, encoding=encoding, lines=True)):
        if strip:
            line = line.strip()
