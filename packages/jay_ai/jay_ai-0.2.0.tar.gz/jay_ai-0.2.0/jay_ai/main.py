...
def run(agent, agent_id, host, port):
    ...
    os.environ["VOICE_DEV__INTERNAL_AGENT_API_KEY"] = api_key
    ...
    # Allows the playground to fetch the VOICE_DEV_AGENT_ID specified by the user in their .env file
...
def playground(agent_id, host, port):
    ...
    # Allows the playground to fetch the VOICE_DEV_AGENT_ID specified by the user
    @app.get("/jay/fetch_agent_id")
    def fetch_agent_id():
        return JSONResponse(
            content={
...
def deploy(agent, agent_id):
    ...
    if local_package_path:
        ...
        shutil.copytree(str(jay_src), f"{tmp_jay_dir}/jay", dirs_exist_ok=True)
    ...
    with importlib.resources.path("jay", "Dockerfile") as dockerfile_path:
        # --------------------------------------------------------------------------
        # 1. BUILD DOCKER IMAGE
        # --------------------------------------------------------------------------
        build_cmd = [
