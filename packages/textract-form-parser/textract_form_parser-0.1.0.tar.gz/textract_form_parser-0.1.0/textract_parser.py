...
def setup_textract_logger(verbose: bool = False, log_dir: str = 'logs') -> logging.Logger:
    """
...
def get_geometric_confidence(key_bbox: Dict, value_bbox: Dict) -> float:
    ...
    if value_bbox["Left"] > key_bbox["Left"]:
        horizontal_distance = value_bbox["Left"] - (key_bbox["Left"] + key_bbox["Width"])
        if horizontal_distance < 0.1:  # Threshold for horizontal proximity
            is_aligned = True
            geometric_score = 100.0 - (horizontal_distance * 1000)  # Reduce score based on distance
    
    # Check vertical alignment (value below key)
    if not is_aligned and abs(value_bbox["Top"] - key_bbox["Top"]) < 0.05:  # Threshold for vertical alignment
        is_aligned = True
...
def get_text_by_geometry(target_bbox: BoundingBox, blocks: List[Dict], 
    ...
    for block in blocks:
        if block['BlockType'] == 'LINE' or (block['BlockType'] == 'WORD' and 'Text' in block):
...
def find_value_by_geometry(key_block: Dict, blocks: List[Dict]) -> Optional[Dict]:
    ...
    right_texts = [t for t in get_text_by_geometry(key_bbox, blocks) 
                  if t['position'] == 'RIGHT']
    if right_texts:
        return right_texts[0]
    
    # Then look below
    below_texts = [t for t in get_text_by_geometry(key_bbox, blocks) 
                  if t['position'] == 'BELOW']
    if below_texts:
...
def find_answer_by_geometry(query_block: Dict, blocks: List[Dict]) -> Optional[Dict]:
    ...
    for block in blocks:
        if 'Geometry' in block and block['BlockType'] in ['WORD', 'LINE']:
            block_bbox = get_bounding_box(block['Geometry'])
            if (abs(block_bbox.left - answer_bbox.left) < 0.001 and
                abs(block_bbox.top - answer_bbox.top) < 0.001 and
                abs(block_bbox.width - answer_bbox.width) < 0.001 and
                abs(block_bbox.height - answer_bbox.height) < 0.001):
...
def find_blocks_with_matching_geometry(target_geometry: Dict, blocks: List[Dict]) -> List[Dict]:
    ...
    for block in blocks:
        if 'Geometry' in block:
            ...
            if (block_bbox['Width'] == target_bbox['Width'] and
                block_bbox['Height'] == target_bbox['Height'] and
                block_bbox['Left'] == target_bbox['Left'] and
                block_bbox['Top'] == target_bbox['Top']):
                matching_blocks.append({
                    'block_type': block['BlockType'],
                    'confidence': block.get('Confidence', 0.0),
                    'text': block.get('Text', ''),
                    ...
                    if cell_block:
                        if cell_block['BlockType'] in ['CELL', 'MERGED_CELL']:
                                ...
                                for child_rel in cell_block['Relationships']:
                                    if child_rel['Type'] == 'CHILD':
                                        for child_id in child_rel['Ids']:
                                            child_block = next((b for b in blocks if b['Id'] == child_id), None)
                                            if child_block and child_block['BlockType'] == 'SELECTION_ELEMENT':
                            ...
                            table_cells.append(TableCell(
                                ...
                                selected=selection_status == 'SELECTED' if selection_status else None
                            ))
...
def print_table(table_data: List[List[TableCell]]):
    ...
    for col in range(len(table_data[0])):
        width = max(
            ...
            (12 if cell.is_merged else 0) +  # Space for merge markers
            15  # Space for confidence score
            for row in table_data for cell in [row[col]]
        ) + 2
    ...
    filled_cells = sum(1 for row in table_data for cell in row 
                      if cell.text.strip() and not cell.is_merged)
    empty_cells = sum(1 for row in table_data for cell in row 
                     if not cell.text.strip() and not cell.is_merged)
    merged_cells = sum(1 for row in table_data for cell in row 
                      if cell.is_merged and cell.row_span > 0)
    merged_spans = sum(1 for row in table_data for cell in row 
                      if cell.is_merged and cell.row_span == 0)
    
    valid_cells = [cell for row in table_data for cell in row 
                  if cell.text.strip() and not cell.is_merged]
    avg_confidence = (sum(cell.confidence for cell in valid_cells) / len(valid_cells)) if valid_cells else 0

    logger.info("\nTable Summary:")
    logger.info(f"Total Cells: {total_cells}")
    logger.info(f"├─ Filled Cells: {filled_cells}")
    logger.info(f"├─ Empty Cells: {empty_cells}")
    logger.info(f"├─ Merged Cells: {merged_cells} (spanning {merged_spans} additional cells)")
...
def format_readable_output(blocks: List[Dict]) -> DocumentContent:
    ...
    try:
        ...
        for block in blocks:
            if block['BlockType'] == 'QUERY':
                        ...
                        if rel['Type'] == 'ANSWER':
                            for answer_id in rel['Ids']:
                                ...
                                if answer_block:
                                    ...
                                    matching_blocks = find_blocks_with_matching_geometry(
                ...
                if answer_from_rel:
                    ...
                    original_confidence = answer_block.get('Confidence', 0.0)  # This will be 37.00
                    ...
                    accumulated_score = sum(b['confidence'] for b in matching_blocks_info) / len(matching_blocks_info) if matching_blocks_info else 0.0
                    ...
                    for rel in block['Relationships']:
                        if rel['Type'] == 'VALUE':
                            for value_id in rel['Ids']:
                                ...
                                if value_block:
                                    value_text = get_text_from_block(value_block, blocks).strip()
                                    # Find all blocks with matching geometry
                                    matching_blocks = find_blocks_with_matching_geometry(
                                        value_block['Geometry'], blocks)
                                    
                                    # Find LINE block containing this key-value pair
                                    line_block = find_line_containing_text(f"{key_text}: {value_text}", blocks)
                                    if line_block:
                                        matching_blocks.append(line_block)
                                    
                                    original_confidence = value_block.get('Confidence', 0.0)
                                    accumulated_score = sum(b['confidence'] for b in matching_blocks) / len(matching_blocks) if matching_blocks else 0.0
                                    ...
                                    form_fields.append({
                                        ...
                                        'source': 'relationship',
                                        'matching_blocks': matching_blocks,
                                        'line_context': line_block['text'] if line_block else None
                                    })
        ...
        return DocumentContent(queries=queries, form_fields=form_fields, 
...
def print_key_value_table(form_fields: List[Dict]):
    ...
    value_width = max(
        max(len(field['value'] if field['value'].strip() else "<NO VALUE GIVEN>") 
            for field in form_fields) + 15,  # Extra space for confidence
        20  # Minimum width
    )
...
def print_query_table(queries: List[Dict]):
    ...
    answer_width = max(len(qa['answer']) for qa in queries) + 15  # Extra space for confidence
    ...
    logger.info("\nQUERIES AND ANSWERS:")
    logger.info(separator)
    logger.info(f"| {'Question':<{question_width-1}}| {'Answer':<{answer_width-1}}| {'Alias':<{alias_width-1}}|")
    logger.info(separator)
    ...
    for qa in queries:
        ...
        logger.info(f"| {question:<{question_width-1}}| {answer_text:<{answer_width-1}}| {alias:<{alias_width-1}}|")
        logger.info(separator)
...
def print_section_structure(sections: Dict[str, List[Dict]], blocks: List[Dict]):
    ...
    layout_types = {'LAYOUT', 'LAYOUT_HEADER', 'LAYOUT_TITLE', 'LAYOUT_SECTION_HEADER', 'LAYOUT_TEXT', 'LAYOUT_FOOTER'}
    ...
    for section_name, section_blocks in sections.items():
        ...
        if any(t in type_blocks for t in form_types):
            ...
            for block_type in form_types:
                if block_type in type_blocks:
                    logger.info(f"│  ├─ {block_type}: {len(type_blocks[block_type])} blocks")
        ...
        other_types = set(type_blocks.keys()) - layout_types - form_types - table_types - page_types
        if other_types:
        ...
        if block_type in layout_types:
            ...
            if content:
                ...
                block_data = {
                    ...
                    } if "Geometry" in block else None
                }
...
def analyze_key_value_pairs(form_fields: List[Dict], blocks: List[Dict] = None) -> Dict[str, Any]:
    ...
    for field in form_fields:
        ...
        if blocks and field.get("geometry"):  # Only process geometry if blocks are provided
            key_block = {
                "Geometry": {
                    "BoundingBox": field["geometry"]["key_bbox"]
                }
            }
            value_block = {
                "Geometry": {
                    "BoundingBox": field["geometry"]["value_bbox"]
                }
            }

            field_data["key_nearby_text"] = find_text_by_geometry(key_block, blocks, max_distance=0.05)
            field_data["value_nearby_text"] = find_text_by_geometry(value_block, blocks, max_distance=0.05)
        ...
        for block in blocks:
            if block["BlockType"] == "TABLE":
                ...
                for text_block in blocks:
                    if text_block["BlockType"] in ["LINE", "LAYOUT_TITLE", "LAYOUT_SECTION_HEADER"]:
                        ...
                        horizontal_overlap = (
                            text_bbox["Left"] < (table_left + table_bbox["Width"]) and
                            (text_bbox["Left"] + text_bbox["Width"]) > table_left
                        )
                        
                        if 0 <= vertical_distance < 0.05 and horizontal_overlap:  # Adjust threshold as needed
                ...
                if potential_titles:
                    closest_title = min(potential_titles, key=lambda x: x["distance_to_table"])
                    table_titles.append({
                        "table_id": block["Id"],
    ...
    try:
        ...
        if not page_found:
            return {
                ...
                "total_pages": sum(1 for block in blocks if block["BlockType"] == "PAGE")
            }
...
def find_selection_element_label(element_block: Dict, blocks: List[Dict], 
    ...
    for block in blocks:
        if block["BlockType"] in ["WORD", "LINE"] and "Text" in block:
            ...
            distance = ((text_center["x"] - element_center["x"]) ** 2 + 
                       (text_center["y"] - element_center["y"]) ** 2) ** 0.5
...
def generate_html_report(analysis_results: Dict, output_file: str = "textract_report.html") -> None:
    """
            ...
            if blocks:
                ...
                for block in blocks:
                    ...
                    if block.get('confidence'):
                        detailed_info.append(f"<span class='confidence'>({block['confidence']:.1f}%)</span>")
                    ...
                    content.append(f"<li>{block['content']} {''.join(detailed_info)}</li>")
                content.append("</ul>")
        return "\n".join(content)
    ...
    def format_form_fields_section(form_data: Dict) -> str:
        ...
        content = ["<table>", "<tr><th>Key</th><th>Value</th><th>Confidence</th><th class='detailed-info'>Details</th></tr>"]
        ...
        content.append(f"<p>Average Confidence: {summary['average_confidence']:.1f}%</p>")
        content.append("</div>")
    ...
    def format_queries_section(queries_data: Dict) -> str:
        ...
        content = ["<table>", "<tr><th>Question</th><th>Answer</th><th>Confidence</th></tr>"]
        ...
        for i, table in enumerate(tables_data, 1):
            ...
            for row in table['data']:
                ...
                for cell in row:
                    ...
                    detailed_info = f"<td class='detailed-info'>({cell['confidence']:.1f}%)</td>"
    ...
    try:
        ...
        form_fields_content = format_form_fields_section(analysis_results.get('form_fields', {}))
...
def find_text_by_geometry(block: Dict, blocks: List[Dict], max_distance: float = 0.05) -> List[Dict[str, Any]]:
    """
    ...
    for other_block in blocks:
        if other_block['BlockType'] in ['LINE', 'WORD'] and 'Text' in other_block:
            ...
            if source_polygon and other_polygon:
                polygon_overlap = calculate_polygon_overlap(source_polygon, other_polygon)
    ...
    return sorted(nearby_texts, key=lambda x: (x["polygon_overlap"] == 0, x["distance"]))
...
def calculate_polygon_overlap(poly1: List[Dict[str, float]], poly2: List[Dict[str, float]]) -> float:
    """
...
def create_concise_results(analysis_results: Dict) -> Dict:
    ...
    if "layout" in analysis_results:
        concise["layout"] = {
            k: [{"content": b["content"], "confidence": b["confidence"]} 
                for b in v]
