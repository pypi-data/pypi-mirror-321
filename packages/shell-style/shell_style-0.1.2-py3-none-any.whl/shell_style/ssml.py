...
from . import (rgb_to_ansi, hex_to_ansi)
from re import sub
...
def ssml_to_ansi(text: str) -> str:
    ...
    ssml_to_ansi = {
        ...
        "<@heading>": Styles.BOLD + Styles.UNDERLINE
    }
    ...
    text = sub(
        r"<@fg_rgb\((\d+)[,\s]+(\d+)[,\s]+(\d+)\)>",
        lambda m: rgb_to_ansi(int(m.group(1)), int(m.group(2)), int(m.group(3)), "fg"),
        text
    )
    
    text = sub(
        r"<@bg_rgb\((\d+)[,\s]+(\d+)[,\s]+(\d+)\)>",
        lambda m: rgb_to_ansi(int(m.group(1)), int(m.group(2)), int(m.group(3)), "bg"),
        text
    )

    text = sub(
        r"<@fg_hex\(#?([0-9a-fA-F]{6})\)>",
        lambda m: hex_to_ansi(f"#{m.group(1)}", "fg"),
        text
    )
    
    text = sub(
        r"<@bg_hex\(#?([0-9a-fA-F]{6})\)>",
        lambda m: hex_to_ansi(f"#{m.group(1)}", "bg"),
        text
    )
