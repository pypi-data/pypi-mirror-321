...
import json
import numpy as np
from cs410_arena.game.go.adversarial_search_problem import AdversarialSearchProblem, GameState
import copy
from  cs410_arena.game.go.go_utils import create_go_game
...
class GoProblem(AdversarialSearchProblem[GoState, Action]):
    def __init__(self, size=DEFAULT_SIZE, state=None, player_to_move=0):
        ...
        """
        if state is None:
            game_state = create_go_game(size)
        else:
            game_state = state
