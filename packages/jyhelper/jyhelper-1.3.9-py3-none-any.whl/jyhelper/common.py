...
class common:
    ...
    @staticmethod
    def print_table(data, table_name=None):
        if not bool(data):
            data = [['No Data']]
        """打印输出整齐的表格"""
        def getRowWidth(string):
            """
            此函数使用正则表达式计算字符串 s 的长度，其中汉字及中文标点算 2 个长度，英文及英文标点算 1 个长度
            """
            string = str(string)
            # 去掉颜色的标识
        ...
        if bool(table_name):
            ...
            print('|'+titleEmpty*' '+table_name+(titleWidth-2-getRowWidth(table_name)-titleEmpty)*' '+'|')
        # 打印
        i = 0
        dataLen = len(data)
        for row in data:
