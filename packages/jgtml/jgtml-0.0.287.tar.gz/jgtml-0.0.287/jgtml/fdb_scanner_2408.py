...
def main():
  ...
  for i in instruments:
    ...
    for t in timeframes_to_parse:
      ...
      if dfsrc is None:
        try:
          dfsrc=pd.read_csv(cache_filepath,index_col=0,parse_dates=True,date_parser=lambda x: pd.to_datetime(x, format=DATE_FORMAT, utc=True),dtype=DTYPE_DEFINITIONS)
        except:
