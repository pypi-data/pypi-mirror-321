...
def update_ssh_config_ip(instance_name, new_ip_address):
    """Update instance IP in SSH config atomically."""
    ...
    for line in config_lines:
        if line.strip().startswith("Host "):
            in_target_host = (line.strip() == f"Host {instance_name}")
            
        if in_target_host and line.strip().startswith("HostName") and not updated:
