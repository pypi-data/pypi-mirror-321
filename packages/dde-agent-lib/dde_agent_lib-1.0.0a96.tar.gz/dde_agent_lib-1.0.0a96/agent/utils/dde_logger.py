    ...
    def _init_statis_logger(cls):
        ...
        class JsonFormatter(Formatter):
            def format(self, record):
                log_record = {
                    "message": getattr(record, 'content', 'empty'),
                    "source": getattr(record, 'source', 'default'),
                    "env": getattr(record, 'env', 'default'),
                    "traffic_type": getattr(record, 'traffic_type', 'normal'),
                    "api_type": getattr(record, 'api_type', 'common_api'),
                    "service_instance": getattr(record, 'service_instance', 'default'),
                    "service_group": getattr(record, 'service_group', 'default')
                }
...
def statis_log(traffic_type, api_type, service_instance, service_group, *contents):
    splitter = " |#| "
    contents = map(str, contents)
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")  # 获取当前时间并格式化
    log_content = f"{current_time}{splitter}{splitter.join(contents)}"
    extra_fields = {'content':log_content, 'source':source, 'env':env,  'traffic_type': traffic_type, 'api_type': api_type, 'service_instance':service_instance, 'service_group':service_group}
    dde_statis_logger.info(log_content, extra = extra_fields)
