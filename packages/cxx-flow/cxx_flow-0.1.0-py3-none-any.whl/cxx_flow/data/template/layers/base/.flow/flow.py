...
def activate_virtual_env():
    ...
    with cd(os.path.dirname(__file__)):
        ...
        has_venv = bindir is not None and os.path.isfile(os.path.join(bindir, python_exec))

        if not has_venv:
            venv("--upgrade-deps", ".venv")

        os.environ['PATH'] = f"{os.path.abspath(bindir)}{os.pathsep}{os.environ['PATH']}"
...
def bootstrap_cxx_flow():
    ...
    # TODO: use pypi to get the package
