...
async def test_find_contact_email(llm, context):
    ...
    agent = Agent(
        task='Go to https://lumivor.com/ and find out the contact email',
        llm=llm,
        browser_context=context,
    )
...
async def test_agent_finds_installation_command(llm, context):
    """Test agent's ability to find the pip installation command for lumivor on the web"""
    agent = Agent(
        task='Find the pip installation command for the lumivor repo',
        llm=llm,
        browser_context=context,
    )
    ...
    extracted_content = history.extracted_content()
    install_command = 'pip install lumivor'
    for content in extracted_content:
