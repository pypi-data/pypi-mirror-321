...
def create_layer_columns(df, layer_names):
    """
    Aggregates retinal layer data into summary statistics (mean values) 
    for "in", "out", and overall categories.

    This function computes the mean for specific regions ("in", "out") 
    of each layer and creates new columns in the DataFrame. Additionally, 
    it calculates the average across "in" and "out" for each layer.

    Args:
        df (pd.DataFrame): Input DataFrame containing retinal layer data.
        layer_names (list of str): List of layer names to aggregate.

    Returns:
        pd.DataFrame: Updated DataFrame with new aggregated columns.
    """
    for layer in layer_names:
        # Generate patterns for "in" and "out"
        in_columns = df.filter(like=f"{layer}_mean_in_").filter(regex="(inf|sup|nas|tem)$").columns
        out_columns = df.filter(like=f"{layer}_mean_out_").filter(regex="(inf|sup|nas|tem)$").columns

        # Calculate the mean for "in" and "out" locations
        if not in_columns.empty:
            df[f"{layer}_in"] = df[in_columns].mean(axis=1)
        if not out_columns.empty:
            df[f"{layer}_out"] = df[out_columns].mean(axis=1)

        # Calculate the overall mean of "in" and "out"
        in_out_cols = [col for col in [f"{layer}_in", f"{layer}_out"] if col in df.columns]
        if in_out_cols:
            df[layer] = df[in_out_cols].mean(axis=1)
...
def impute_missing_data(
    ...
    optionally processing only rows with missing data in chunks.

    Args:
        df (pd.DataFrame): The DataFrame to impute.
        columns_to_impute (list): List of columns to impute.
        categorical_columns (list, optional): List of categorical columns.
        max_iter (int, optional): Maximum number of imputation iterations.
        random_state (int, optional): Random seed for reproducibility.
        sample_posterior (bool, optional): Whether to sample from the posterior distribution.
        min_value (float, optional): Minimum possible imputed value.
        max_value (float, optional): Maximum possible imputed value.
        estimator (object, optional): Custom estimator for IterativeImputer.
        verbose (bool, optional): If True, prints information about imputation progress.
        chunk_size (int, optional): Number of rows per chunk for imputation.

    Returns:
        pd.DataFrame: The DataFrame with imputed values.
    """
    # Identify columns with missing values
    cols_with_missing = [col for col in columns_to_impute if df[col].isnull().any()]
    if not cols_with_missing:
        if verbose:
            print("No missing values to impute.")
        return df  # Return the original DataFrame if there's nothing to impute

    # Log missing columns and counts
    if verbose:
        print(f"Columns with missing data: {cols_with_missing}")
        missing_before = df[cols_with_missing].isnull().sum().sum()
        print(f"Total missing values before imputation: {missing_before}")

    # Create a copy of only the relevant columns for imputation
    df_to_impute = df[cols_with_missing].copy()
    ...
    if categorical_columns:
        for col in categorical_columns:
            if col in df_to_impute.columns:
                ...
                df_to_impute[[col]] = encoder.fit_transform(df_to_impute[[col]])
    ...
    # Initialize df_imputed as None for handling later
    df_imputed = None

    # Handle chunking
    if chunk_size:
        chunks = [df_to_impute.iloc[i:i + chunk_size] for i in range(0, df_to_impute.shape[0], chunk_size)]
        imputed_chunks = []

        for chunk in chunks:
            if verbose:
                print(f"Imputing chunk with {chunk.shape[0]} rows...")
            imputed_chunk_array = imputer.fit_transform(chunk)
            imputed_chunk = pd.DataFrame(imputed_chunk_array, columns=chunk.columns, index=chunk.index)
            imputed_chunks.append(imputed_chunk)

        # Concatenate all imputed chunks
        df_imputed = pd.concat(imputed_chunks)
    else:
        # Impute the entire DataFrame at once
        imputed_array = imputer.fit_transform(df_to_impute)
        df_imputed = pd.DataFrame(imputed_array, columns=df_to_impute.columns, index=df_to_impute.index)

    # Round and inverse transform categorical columns
    for col in categorical_columns:
        if col in df_imputed.columns:
            try:
                ...
                if verbose:
                    print(f"Error during inverse_transform for column '{col}': {e}. Skipping this column.")

    # Update the original DataFrame with imputed values
    df.update(df_imputed)

    if verbose:
        missing_after = df[columns_to_impute].isnull().sum().sum()
        print(f"Total missing values before imputation: {missing_before}")
