...
def impute_missing_data(
    ...
    Parameters:
        ...
        estimator (object, optional): A custom estimator to use for imputation. Defaults to BayesianRidge.
        verbose (bool, optional): If True, prints the number of missing values before and after imputation.
            Defaults to True.
        chunk_size (int, optional): Number of rows to process in each chunk. If None, processes all rows at once.
            Defaults to None.
    ...
    for col in categorical_columns:
        if col in df_imputed.columns:  # Ensure the column exists in the imputed DataFrame
            try:
                # Round the imputed values to the nearest integer for categorical data
                df_imputed[col] = np.round(df_imputed[col]).astype(int)
                # Apply inverse transform to restore original categorical values
                df_imputed[[col]] = encoders[col].inverse_transform(df_imputed[[col]])
            except ValueError as e:
                print(f"Error applying inverse_transform for column '{col}': {e}")
                print("Skipping inverse_transform for this column.")
        else:
            print(f"Column '{col}' not found in the imputed DataFrame. Skipping.")

    # Identify rows with missing data
    rows_with_missing = df[columns_to_impute].isnull().any(axis=1)
    if not rows_with_missing.any():
        if verbose:
            print("No missing values to impute.")
        return df

    df_to_impute = df.loc[rows_with_missing, columns_to_impute]
    ...
    imputer = IterativeImputer(
        estimator=estimator,  # Use the custom estimator
        max_iter=max_iter,
    ...
    if chunk_size:
        ...
        # Impute the rows with missing values at once
        imputed_array = imputer.fit_transform(df_to_impute)
        df_imputed = pd.DataFrame(imputed_array, columns=columns_to_impute, index=df_to_impute.index)
    ...
    # Replace the original rows with imputed data
