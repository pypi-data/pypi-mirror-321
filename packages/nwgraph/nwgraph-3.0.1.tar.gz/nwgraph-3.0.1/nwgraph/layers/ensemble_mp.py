...
from nwgraph.message_passing import PathwayListMessagePassing, MessagePassingType, NodeStatesType
...
class EnsembleMp(PathwayListMessagePassing):
    ...
    @property
    @overrides
    def pathway_list_models(self) -> nn.ModuleDict:
        return self._pathway_models

    @overrides
    def message_pass(self) -> MessagePassingType:
        """output = {"node1": [msg1, msg2], "node2": [msg3] ... }"""
        node_messages = {node: [] for node in self.graph.nodes}
        for edge_name, edge_model in self.pathway_list_models.items():
            edge = self.graph.name_to_edge[edge_name]
            if edge.input_node not in self.node_states:
                if self.all_edges_must_have_data:
                    raise KeyError(f"All edges must have data. Not found for edge '{edge}'")
                continue

            if (edge_input_node_state := self.node_states[edge.input_node]) is None:
                continue

            msg = edge_model(edge_input_node_state)
            node_messages[edge.output_node].append(msg)
        # Only put nodes in the messages dict if any messages were exchanged this step.
        node_messages = {node.name: messages for node, messages in node_messages.items() if len(messages) > 0}
