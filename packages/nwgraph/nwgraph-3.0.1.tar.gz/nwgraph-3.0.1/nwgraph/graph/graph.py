...
from .internal import graph_subgraph, get_n_nodes, build_node_names, build_node_types, build_node_args, check_graph, \
    build_edge_indexes
...
class Graph:
    ...
    TODO: node states -- perhaps this is part of MP class.
    ...
    def __init__(self, edge_indexes: list[EdgeIndex], n_nodes: int | None = None,
        ...
        self._node_states: tr.Tensor = tr.FloatTensor([tr.nan] * _n_nodes)
        self._n_nodes_set = 0
    ...
    @node_states.setter
    def node_states(self, node_states: Any):
        """we can put anything in this graph setter. MP setters will have different restrictions"""
    ...
    def n_nodes_set(self) -> int:
        """returns the number of nodes with a state set (non nan)"""
        return self._n_nodes_set

    @n_nodes_set.setter
    def n_nodes_set(self, n_nodes_set: int):
        self._n_nodes_set = n_nodes_set
    ...
    def add_nodes(self, node_names: list[str] | list[int], node_types: list[Type[Node]], node_args: list[NodeArgs]):
        ...
        for node_name, node_type, node_arg in zip(node_names, node_types, node_args):
            ...
            self._node_states = tr.cat([self._node_states, tr.FloatTensor([tr.nan])])
    ...
    def remove_nodes(self, node_names: list[str] | list[int]):
        ...
        for node_name in node_names:
            ...
            self._node_states = tr.cat([self._node_states[:node_ix], self._node_states[node_ix + 1:]])
    ...
    def add_edges(self, edge_indexes: list[EdgeIndex], edge_types: EdgeTypes | None = None):
        ...
        for edge_index, edge_type in zip(edge_indexes, edge_types):
            if edge_index in self.edge_indexes:
                raise ValueError(f"Edge {edge_index} already exists in the graph.")
            if edge_index[0] not in self.node_names:
                raise ValueError(f"Node {edge_index[0]} doesn't exist in the graph.")
            if edge_index[1] not in self.node_names:
                raise ValueError(f"Node {edge_index[1]} doesn't exist in the graph.")
    ...
    def readout(self) -> tr.Tensor | Dict[str, list[tr.Tensor]]:
    ...
    def reset_all_states(self):
        ...
        logger.debug2("Resetting all node states.")
        self._node_states = tr.FloatTensor([tr.nan] * len(self.nodes))
    ...
    def __repr__(self) -> str:
        return f"{parsed_str_type(self)} (N:{len(self.nodes)}, E:{len(self.edges)}, S:{self.n_nodes_set} " \
               f"{list(self.node_states.shape[1:]) if isinstance(self.node_states, tr.Tensor) else '[list]'})"
