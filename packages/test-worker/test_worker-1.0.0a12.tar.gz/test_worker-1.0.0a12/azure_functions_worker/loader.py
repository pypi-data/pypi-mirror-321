...
def index_function_app(function_path: str):
    ...
    from azure.functions import FunctionApp, ExternalHttpFunctionApp, Blueprint
    app: Optional[Union[FunctionApp, ExternalHttpFunctionApp, Blueprint]] = None
    for i in imported_module.__dir__():
        if isinstance(getattr(imported_module, i, None), FunctionApp)\
                or isinstance(getattr(imported_module, i, None), ExternalHttpFunctionApp)\
                or isinstance(getattr(imported_module, i, None), Blueprint):
            if not app:
