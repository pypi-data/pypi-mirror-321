...
class WebSocketClient:
    ...
    async def handle_rtn_back_results(self, data):
        ...
        df.to_csv(path + 'positions.csv', index=False)
        ...
        # 将 DataFrame 写入 CSV 文件
        df.to_csv(path + 'daily_found_results.csv', index=False)

        trade_lists = data[0]["tradeLists"]
        df = pd.DataFrame(trade_lists)

        # 将 DataFrame 写入 CSV 文件
        df.to_csv(path + 'trade_lists.csv', index=False)

        order_lists = data[0]["orderLists"]
        df = pd.DataFrame(order_lists)

        # 将 DataFrame 写入 CSV 文件
        df.to_csv(path + 'order_lists.csv', index=False)
        ...
        # 保存文本信息图表为图片
        plt.tight_layout()
        plt.savefig(path + 'text_info.png')  # 保存为 text_info.png
        plt.close()  # 关闭图形以释放资源
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(path + 'chart.png')  # 保存为 chart.png
        plt.close()  # 关闭图形以释放资源

        # 拼接两张图片
        text_img = Image.open(path + 'text_info.png')
        chart_img = Image.open(path + 'chart.png')
        ...
        # 保存拼接后的图像
        combined_img.save(path + 'combined_output.png')
