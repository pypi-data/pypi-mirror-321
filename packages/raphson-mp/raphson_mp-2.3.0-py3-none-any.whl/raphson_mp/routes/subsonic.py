...
def SubsonicAlbumID3(conn: Connection, album: str, artist: str | None) -> dict[str, Any]:
    arbitrary_path, artist, total_count, total_duration, created, year = conn.execute(
        """
        SELECT path, album_artist, COUNT(*), SUM(duration), MIN(mtime), MIN(year)
        FROM track
        WHERE album = ?
        """,
        (album,),
...
def SubsonicArrayPlaylist(conn: Connection) -> dict[str, Any]:
    # https://opensubsonic.netlify.app/docs/responses/playlists
    return {
        "playlist": [SubsonicPlaylist(conn, name) for name, in conn.execute("SELECT path FROM playlist").fetchall()]
    }
...
async def getAlbumList2(request: web.Request, conn: Connection, _user: User):
    ...
    if t == "random":
        ...
        query += """
        FROM track JOIN scanner_log ON track.path = scanner_log.track
        WHERE album IS NOT NULL
        GROUP BY album
...
async def getPlaylists(request: web.Request, conn: Connection, _user: User):
    # https://opensubsonic.netlify.app/docs/endpoints/getplaylists/
    return response(request, {"playlists": SubsonicArrayPlaylist(conn)})
