...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (const elem of document.getElementsByClassName('window-overlay')) {
        const id = elem.id;
        const window = elem.querySelector(".window");

        elem.addEventListener('click', event => {
            if (event.clientX == 0 || event.clientY == 0) {
                console.debug('window: close: ignoring click at zero X or Y');
                return;
            }

            if (!windows.isOpen(id)) {
                console.debug('window: close: ignoring click, window is not open');
                return;
            }
            ...
            if (event.clientX < rect.left
                ...
                console.debug('window: close:', event.clientX, event.clientY, rect.left, rect.right, rect.top, rect.bottom);
                windows.close(id);
            }
        });
    }
