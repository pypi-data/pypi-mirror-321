...
class TablePrinter:
    ...
    def add_column(self, name: str, column_align: str = ALIGN_CENTER, row_align: str = ALIGN_AUTO):
        ...
        assert column_align != TablePrinter.ALIGN_AUTO, 'column align cannot be ALIGN_AUTO'
        self.columns.append(TablePrinter.Column(name, len(name), column_align, row_align))
    ...
    def print(self):
        ...
        """
        print(' | '.join([f'{column.name:{column.column_align}{column.width}}' for column in self.columns]))
        print('-+-'.join(['-' * column.width for column in self.columns]))
        for row in self.rows:
            print(' | '.join(
                [f'{cell:{self.columns[i].row_align}{self.columns[i].width}}' for i, cell in enumerate(row)]))
