...
class GrindrHTTPClient:
    ...
    def __init__(
        ...
        self._request_dump_directory: str = web_kwargs.get('request_dump_directory') if web_kwargs else None
    ...
    def add_log_to_file(self, log_data: GrindrHTTPLog) -> None:
        ...
        request_dir = Path(self._request_dump_directory)

        # Create the session directory if it doesn't exist
        if not request_dir.exists():
            request_dir.mkdir()

        request_fp = request_dir.joinpath(f'./{self._session_profile_id or 'anonymous'}.json')
    ...
    async def request(
        ...
        # Make the request
        self._request_id += 1
        response: curl_cffi.requests.Response = await (client or self._session).request(
            method=method,
            url=url,
            params=url_params,
            headers=headers,
        ...
        self.add_log_to_file({
            "request": {
                ...
                "params": url_params,
                "headers": headers,
                "kwargs": {k: v for k, v in kwargs.items() if k is not 'content'}  # Exclude binary content
            },
            "response": {
