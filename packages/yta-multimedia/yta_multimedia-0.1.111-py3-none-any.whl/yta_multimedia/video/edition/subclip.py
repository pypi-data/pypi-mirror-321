...
class SubClip:
    ...
    def speed_factor(self, value: SubClipAttributeModifier):
        _validate_is_video_attribute_modifier_instance(value)
        _validate_attribute_modifier(value, 'speed_factor', WHITE_BALANCE_LIMIT, self.number_of_frames)
    ...
    def apply_speed_factor(self):
        ...
        def _custom_speed_modification(self: SubClip):
            ...
            # TODO: This method should be called if 'speed_factor' 
            # exist and is an array of values
        ...
        frames, frame_indexes = _custom_speed_modification(self)

        # TODO: Avoid the use of this MPVideo class please
        from yta_multimedia.video import MPVideo
        from moviepy import ImageSequenceClip, concatenate_audioclips

        # Build the audio base
        new_audio_clips = []
        for frame_index in frame_indexes:
            frame_time = MPVideo.frame_index_to_frame_time(frame_index, self.fps)
            # TODO: This doesn't work, I will try concatenating and
            # subclipping
            #new_audio_frames.append(video.audio.get_frame(frame_time))
            #new_audio_frames.append(video.audio.with_subclip(frame_time, frame_time + 1 / video.fps + 0.000001).to_soundarray(fps = video.audio.fps))
            #new_audio_clips.append(self.video.audio.with_subclip(frame_time, frame_time + 1 / self.fps + 0.000001))
            new_audio_clips.append(self.video.audio.with_subclip(frame_time, frame_time + 1 / self.fps))

        new_audio_clips = concatenate_audioclips(new_audio_clips)

        # Build the video base
        output_video = ImageSequenceClip(frames, fps = self.fps)
        output_video = output_video.with_audio(new_audio_clips)
