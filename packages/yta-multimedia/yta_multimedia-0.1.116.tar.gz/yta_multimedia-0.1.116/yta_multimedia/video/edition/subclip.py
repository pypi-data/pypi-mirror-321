...
class SubClip:
    ...
    # Color attributes
    # Zoom and movement
    _zoom: Union[float, list[float]] = None
    """
    A single resize factor o an array of values for
    the resizing process.
    """
    x_movement: Union[float, list[float]] = None
    y_movement: Union[float, list[float]] = None
    rotation: Union[float, list[float]] = None
    ...
    def __init__(self, video: Clip):
        ...
        self.resizes = [1 for _ in range(video.n_frames)]
        self.rotations = [0 for _ in range(video.n_frames)]
        self.positions = [('center', 'center') for _ in range(video.n_frames)]
    ...
    def zoom(self, value: SubClipAttributeModifier):
        """
        Sets the global zoom to this clip. This zoom value is
        a percentage of the zoom we want.
        
        - zoom=100 means no zoom
        - zoom=50 means zoom out a 50%, so the video size will
        be the half
        - zoom=200 means zoom in a 100%, so the video size will
        be the double
        """
        _validate_is_video_attribute_modifier_instance(value)
        ...
        # We accept zoom as a value between 0 and 100 but
    ...
    @x_movement.setter
    def x_movement(self, value: SubClipAttributeModifier):
        _validate_is_video_attribute_modifier_instance(value)
        # TODO: Validate value between limits

        self._x_movement = value.modifier if value.is_single_value else value.get_values(self.number_of_frames)
    ...
    @y_movement.setter
    def y_movement(self, value: SubClipAttributeModifier):
        _validate_is_video_attribute_modifier_instance(value)
        # TODO: Validate value between limits

        self._y_movement = value.modifier if value.is_single_value else value.get_values(self.number_of_frames)
    ...
    def rotation(self, value):
        ...
        self._rotation = value.modifier if value.is_single_value else value.get_values(self.number_of_frames)
    ...
    def set_zoom(self, start: int, end: Union[int, None], rate_function: mRateFunction = mRateFunction.LINEAR):
        """
        Set a new zoom that will be modifier frame by frame.
        ...
        frames.
        """
        self._set_attribute('zoom', start, end, rate_function)

    def set_x_movement(self, start: int, end: Union[int, None], rate_function: mRateFunction = mRateFunction.LINEAR):
        """
        Set a new x movement that will be modifier frame by frame.
        ...
        frames.
        """
        self._set_attribute('x_movement', start, end, rate_function)

    def set_y_movement(self, start: int, end: Union[int, None], rate_function: mRateFunction = mRateFunction.LINEAR):
        """
        Set a new y movement that will be modifier frame by frame.
        ...
        frames.
        """
        self._set_attribute('y_movement', start, end, rate_function)

    def set_rotation(self, start: int, end: Union[int, None], rate_function: mRateFunction = mRateFunction.LINEAR):
        """
        Set a new rotation that will be modifier frame by frame.
        ...
        frames.
        """
        self._set_attribute('rotation', start, end, rate_function)
        ...
        def _get_frame_ts_applying_speed_factors(self: SubClip):
            ...
            while current_frame_index < self.number_of_frames:
                ...
                if current_speed_factor < 1:
                    ...
                    resizes.extend([self.resizes[current_frame_index]] * times_to_append)
                    positions.extend([self.positions[current_frame_index]] * times_to_append)
                    rotations.extend([self.rotations[current_frame_index]] * times_to_append)
                    ...
                    # Any other array that also has to be recalculated
                    resizes.append(self.resizes[current_frame_index])
                    positions.append(self.positions[current_frame_index])
                    rotations.append(self.rotations[current_frame_index])
    ...
    def _process(self):
        ...
        if self.zoom is not None and self.zoom != 1:
            if PythonValidator.is_number(self.zoom):
                resizes = resizes * self.zoom
            else:
                resizes = [
                    zoom_factor * resize_value
                    for zoom_factor, resize_value in zip(self.zoom, resizes)
                ]

        if self.x_movement is not None and self.x_movement != 0:
            if PythonValidator.is_number(self.x_movement):
                positions = [(position[0] + self.x_movement, position[1]) for position in positions]
            else:
                positions = [
                    (position[0] + x, position[1])
                    for position, x in zip(positions, self.x_movement)
                ]

        if self.y_movement is not None and self.y_movement != 0:
            if PythonValidator.is_number(self.y_movement):
                positions = [(position[0], position[1] + self.y_movement) for position in positions]
            else:
                positions = [
                    (position[0], position[1] + y)
                    for position, y in zip(positions, self.y_movement)
                ]

        if self.rotation is not None and self.rotation != 0:
            if PythonValidator.is_number(self.rotation):
                rotations = [rotation + self.rotation for rotation in rotations]
            else:
                rotations = [
                    rotation_ + rotation
                    for rotation_, rotation in zip(rotations, self.rotation)
                ]
        ...
        video = video.resized(lambda t: self._resizes[video_handler.frame_time_to_frame_index(t, video_handler.fps)])
        video = video.with_position(lambda t: self._positions[video_handler.frame_time_to_frame_index(t, video_handler.fps)])
        video = video.rotated(lambda t: self._rotations[video_handler.frame_time_to_frame_index(t, video_handler.fps)])
