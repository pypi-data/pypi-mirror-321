...
def _forward_base(data, network, device=None, keep_x=False):
...
def _forward_categorical(data, network, device=None, keep_x=False):
...
def _forward_multihead_categorical(data, network, device=None, keep_x=False):
    x = data['x'].to(device)
    y = data['y'].to(device)
    y_hat = network(x)
...
def _forward_autoencode(data, network, device=None):
    x = data['x'].to(device)
    z = network.encoder(x)
