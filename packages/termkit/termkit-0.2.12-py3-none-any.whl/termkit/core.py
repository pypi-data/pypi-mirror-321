    ...
    def __call__(self, *args, **kwargs):
        ...
        if hasattr(arguments, "_TERMKIT_CALLBACKS"):
            for callback in arguments._TERMKIT_CALLBACKS:
                if inspect.iscoroutine(callback.callback):
                    asyncio.run(callback.callback(**filter_args(arguments, callback.callback)))
                else:
                    callback.callback(**filter_args(arguments, callback.callback))

        if hasattr(arguments, "_TERMKIT_COMMAND"):
            if inspect.iscoroutine(arguments._TERMKIT_COMMAND):
                asyncio.run(arguments._TERMKIT_COMMAND(**filter_args(arguments, arguments._TERMKIT_COMMAND)))
            else:
