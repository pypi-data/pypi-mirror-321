...
for algo in os.listdir(path_mask):
 if algo!="truth":
    ...
    for file in os.listdir(path_img):
        iou,dico_objet,df_mask,df_truth=function.calcul_iou(path_img+"/"+file,path_mask+"/"+algo+"/"+file,path_mask+"/truth/"+file)
        list_iou.append(iou)
...
for algo in os.listdir(path_mask):
  if algo!="truth":
    ...
    for file in os.listdir(path_img):
        ...
        iou,dico_objet,df_mask,df_truth=function.calcul_iou(path_img+"/"+file,path_mask+"/"+algo+"/"+file,path_mask+"/truth/"+file,dico_algo_min[algo],dico_algo_max[algo])
        list_iou.append(iou)
        recall,precision,true_pos,false_pos,false_neg=function.recall_precision(threshold,df_mask,df_truth)
        list_precision.append(precision)
        list_recall.append(recall)
        ...
        function.outline_iou(path_img+"/"+file,file,algo,df_mask,threshold,c=0)
        function.outline_iou("./segmentation_quality/outline_iou/"+algo+"/"+file[:-4]+".png",file[:-4]+".png",algo,df_truth,threshold,c=255)
