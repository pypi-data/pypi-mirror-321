...
class ElementFinder:
    ...
    @staticmethod
    def __find_context_menu(driver:AppDriver, element: WebElement, menu_item_name: Optional[str], menu_type: Optional[MenuType]):
        if menu_type and menu_item_name:
            ...
            if element:
                Logger.debug(LogType.FRAME, f"控件[{element.id}/{element.text}]的菜单项为：{menu_item_name}/{menu_type}")
