...
def help():
    ...
    methods_info = {
        ...
        'conv_pool': '16. Архитектура многослойной ИНС для распознавания изображений, сверточные и сжимающие слои в PyTorch',         'cnn_ex': '17. Приемы для глубокого обучения на небольших наборах изображений',
        ...
        'unet': '26. Архитектура U-Net: общие принципы, описание, residual connections и применение в задачах сегментации',           'gradcam': '27. Механизмы внимания в машинном зрении и метод Grad-CAM',
        ...
        'autoencoder': '30. Автоэнкодеры: линейные, нелинейные, латентное пространство, интерполяция и экстраполяция',                 'discr_models': '31. Дискриминативные и генеративные модели: задачи, сравнение, классификация и применение',
        ...
        'cnn_cls': 'Практика CNN классификация',
        'cnn_det': 'Практика. CNN object detection',
        'movie_regression': 'Практика. Регрессия',
        'reg_airline_pass': 'Практика. Регрессия'}
...
2. Градиент в новой точке:
   \[
   v_t = \gamma v_{t-1} + \eta g$theta_{\text{lookahead}})
   \]
...
3. **Policy Gradient**:
   - Находит оптимальную стратегию $\pi(a|s)$, максимизируя награду:
     \[
     J$theta) = \mathbb{E}_{\pi_\theta} \left[ \sum_t R(s_t, a_t) \right]
     \]
...
\[
J$theta) = \mathbb{E}_{\pi_\theta} \left[ \sum_{t=0}^\infty \gamma^t R(s_t, a_t) \right]
\]

#### Обновление параметров:
Используется градиент целевой функции:
\[
\nabla_\theta J$theta) = \mathbb{E}_{\pi_\theta} \left[ \nabla_\theta \log \pi_\theta(a|s) G_t \right]
\]
где:
...
2. Потери оптимизируются с помощью стохастического градиентного спуска:
   \[
   \mathcal{L}$theta) = \left( R + \gamma \max_{a'} Q(s', a'; \theta^-) - Q(s, a; \theta) \right)^2
   \]
...
\[
\mathcal{L}(x) = \underbrace{\mathbb{E}_{z \sim q(z|x)} [\log P(x|z)]}_{\text{Ошибка восстановления}} - \underbrace{D_{\text{KL}}(q(z|x) \| P(z))}_{\text{Регуляризация}}
\]
...
def reg_airline_pass(idx: int = 0):
    if idx == 0:
        code = """
import pandas as pd
import numpy as np
...
def movie_regression(idx: int = 0):
    if idx == 0:
        code = """
import kagglehub
...
"""
        ...
        code = """
ataset = datasets.ImageFolder(root=data_dir, transform=transform)
...
def create_model(num_blocks):
    ...
    layers.append(nn.Linear(32 * (img_size // (2 ** num_blocks)) ** 2, 128))
    layers.append(nn.ReLU())
    layers.append(nn.Linear(128, len(dataset.classes)))
...
def train_model(model, train_loader, val_loader, epochs=5):
    ...
    for epoch in range(epochs):
        ...
        print(f"Epoch [{epoch}/{epochs}, Train Loss = {loss:.2f}, Test Loss = {test_loss:.2f} Test F1-Score = {f1}")
    ...
    return best_f1, val_f1_scores, best_model       
"""

    elif idx == 5:
        code = """
num_blocks_list = [1, 2, 3]
results = {}

for num_blocks in num_blocks_list:
    model = create_model(num_blocks)
    best_f1, val_f1_scores, best_model = train_model(model, train_loader, val_loader)
    results[num_blocks] = (best_f1, val_f1_scores)
    torch.save(best_model, f"best_model_{num_blocks}_blocks.pth")

# 5. Визуализация результатов
plt.figure(figsize=(10, 6))
for num_blocks, (_, val_f1_scores) in results.items():
    plt.plot(range(1, len(val_f1_scores) + 1), val_f1_scores, label=f"{num_blocks} blocks")

plt.title("Validation F1 Score vs. Number of Blocks")
plt.xlabel("Epochs")
...
"""
        ...
        code = """
best_num_blocks = max(results, key=lambda x: results[x][0])
model = create_model(best_num_blocks)
model.load_state_dict(torch.load(f"best_model_{best_num_blocks}_blocks.pth"))
