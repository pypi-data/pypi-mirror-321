...
class Workspace:
    ...
    @property
    def saved_groups(self) -> Dict[str, schema.CobaltDataSubset]:
        ...
        Note that this is a copy of the current collection of groups; modifying
        it will not change the set of groups stored in the Workspace or
        displayed in the UI.
        """
    ...
    def get_graph_level(
        ...
        if isinstance(graph, str):
            graph_data = self.state.graph_repo.get_graph(graph)["graph"]
            graph: HierarchicalPartitionGraph = graph_data["graph"]
    ...
    def find_drifted_groups(
        ...
        group_sizes = [len(g.subset) for g in raw_groups]
        comparison_counts = [subset2.intersection_size(g.subset) for g in raw_groups]
        p_vals = [
