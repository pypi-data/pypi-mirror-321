...
class FlyerAPI:
    ...
    @classmethod
    def check_auth_path(cls, path: str, router) -> bool:
        """检查是否需要鉴权"""
        if not path:
            return True
        white_list = os.getenv("flyer_no_auth_path_prefix", "").split(",")
    ...
    def load_module(cls):
        ...
        for _dir in Path(app_dir).iterdir():
            ...
            try:
                ...
                # 开启 BasicAuth 鉴权
                if int(os.getenv("flyer_auth_enable", "0")) == 1 and cls.check_auth_path(sub_module.router.routes):
