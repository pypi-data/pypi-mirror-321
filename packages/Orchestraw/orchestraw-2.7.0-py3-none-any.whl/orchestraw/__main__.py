            ...
            if opt == "y" or opt == "Y":
                with open(filename+".lrc","w") as f:
                    try:
                        ...
                        f.write(data['tlyric']['lyric'].replace("\n","\n"))
                        print(f"歌词已下载并保存为：{filename}.lrc")
...
def download(music_list):
    ...
    with requests.get(response.json()['data'][0]['url']) as res:
        ...
        if res.status_code == 200:
            ...
            print('音频下载失败')
            sys.exit()
    with requests.get('https://api.csm.sayqz.com/album',params={'id': music_list[order][4]}) as res:
        # 获取封面
        if res.status_code == 200:
            picUrl = res.json()['album']['picUrl']
            picUrlStr = res.json()['album']['picId_str']
            for song in res.json()['songs']:
                if song['id'] == music_list[order][0]:
                    pic = song['al']['pic_str']
                    picUrl = picUrl.replace(picUrlStr,pic)
