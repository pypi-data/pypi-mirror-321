        ...
        if accumulation:
            ...
            def conditional_interpolation(
                ...
                for group_id, group in df.groupby(groups):
                    ...
                    prev_index = nan_indices[0] - 1
                    next_index = nan_indices[-1] + 1
                    # Check bounds
                    if prev_index < 0 or next_index >= len(values):
                        continue  # Skip interpolation if out of bounds
                    # Get values before and after the NaNs
                    prev_value = values.iloc[prev_index]
                    next_value = values.iloc[next_index]
                    # Apply conditions
                    ...
                    df.loc[nan_indices, "value"] = values.interpolate().iloc[
                        nan_indices
                    ]
                return df
            ...
            def interp(series: pd.Series, max_nan: int) -> pd.Series:  # type: ignore
                ...
                groups = (
                    is_nan != is_nan.shift()
                ).cumsum()  # Identify groups of consecutive values
                nan_counts = is_nan.groupby(groups).transform(
                    "sum"
                )  # Count NaNs in each group
                # Mask NaNs where the count exceeds the limit
                mask = ~((nan_counts > max_nan) & is_nan)
                return series.where(mask)

            df["value"] = interp(df["value"], maxMissingCount)
