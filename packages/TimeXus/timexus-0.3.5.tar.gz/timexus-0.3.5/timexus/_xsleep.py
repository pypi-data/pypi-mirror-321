...
class XSleep:
    ...
    def _sleep(seconds):
        ...
        if XSleep._print_awake and XSleep._auto_print_flags.get("_sleep", True):
        ...
        if XSleep._print_awake and seconds == 0:
    ...
    def milliseconds(ms):
        ...
        XSleep._set_auto_print_flag("milliseconds")
    ...
    def seconds(secs):
        ...
        if not isinstance(secs, (int, float)):
            raise ValueError("Seconds must be a number.")
        XSleep._set_auto_print_flag("seconds")
        XSleep._sleep(secs)
        return secs
    ...
    def minutes(mins):
        ...
        XSleep._set_auto_print_flag("minutes")
    ...
    def hours(hrs):
        ...
        XSleep._set_auto_print_flag("hours")
    ...
    def set_print(mode):
        ...
        print(f"Auto-print mode set to: {mode}")

    @staticmethod
    def set_custom_print(message):
        """
        Customizes the automatic message after sleeping (only when sleeping for 0 seconds).
    ...
    def get_random_delay(min_seconds, max_seconds):
        """Returns a random delay between min_seconds and max_seconds."""
        if not isinstance(min_seconds, (int, float)) or not isinstance(max_seconds, (int, float)):
            raise ValueError("Min and max seconds must be numbers.")
        if min_seconds < 0 or max_seconds < 0:
            raise ValueError("Min and max seconds must be non-negative.")
        if min_seconds > max_seconds:
            raise ValueError("Min seconds must be less than or equal to max seconds.")
        
        XSleep._set_auto_print_flag("get_random_delay")

        random_delay = random.uniform(min_seconds, max_seconds)
        
        if XSleep._print_awake and XSleep._auto_print_flags.get("get_random_delay", True):
          print(f"Random delay: {random_delay:.4f} seconds")

        return random_delay

    @staticmethod
    def sleep_random_delay(min_seconds, max_seconds):
        """Sleeps for a random time between min_seconds and max_seconds."""
        XSleep._set_auto_print_flag("sleep_random_delay")

        random_delay = XSleep.get_random_delay(min_seconds, max_seconds)
    ...
    def print_message_with_delay(message, delay_seconds):
        ...
        XSleep._set_auto_print_flag("print_message_with_delay")
    ...
    def capture_screenshot(filename="screenshot.png"):
        ...
        try:
            ...
            XSleep._set_auto_print_flag("capture_screenshot")

            if sys.platform.startswith("win32"):
                import PIL.ImageGrab  # You might need to install Pillow: pip install Pillow
                screenshot = PIL.ImageGrab.grab()
                screenshot.save(filename)
                if XSleep._print_awake and XSleep._auto_print_flags.get("capture_screenshot", True):
    ...
    @staticmethod
    def _set_auto_print_flag(func_name, value=True):
        """
        Sets the auto-print flag for a specific function.
        ...
        """
        XSleep._auto_print_flags[func_name] = value

    @staticmethod
    def set_auto_print_for_function(func_name, mode):
      """
      ...
      """
      if mode == "on":
        XSleep._set_auto_print_flag(func_name, True)
        print(f"Auto-print for '{func_name}' set to: {mode}")
      elif mode == "off":
        XSleep._set_auto_print_flag(func_name, False)
        print(f"Auto-print for '{func_name}' set to: {mode}")
      else:
        raise ValueError("Invalid mode. Use 'on' or 'off'.")
