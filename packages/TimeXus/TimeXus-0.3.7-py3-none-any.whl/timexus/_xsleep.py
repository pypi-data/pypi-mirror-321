...
def _get_sleep_func():
    ...
    if sys.platform.startswith("win32"):
        ...
        try:
            ...
            libc = ctypes.CDLL("/usr/lib/libc.dylib") # Fixed potential issue: Fallback path for libc
...
class XSleep:
    ...
    def set_print(mode):
        ...
        Args:
            mode (bool): True to enable messages, False to disable them.
        """
        if not isinstance(mode, bool):
            raise ValueError("Mode must be a boolean (True or False).")
        
        XSleep._print_awake = mode
        # Enable/Disable auto-print for all functions when set_print is used
        for func_name in XSleep._auto_print_flags:
            XSleep._auto_print_flags[func_name] = mode
    ...
    def get_random_delay(seconds):
        ...
        random_delay = random.randint(0, seconds) # Simplified to return an integer
            ...
            if libc.sysctlbyname(ctypes.c_char_p(b"kern.boottime"), ctypes.byref(boottime), ctypes.byref(size), None, 0) != -1:
                ...
                if sys.platform.startswith("win32"):
                    ...
                    if sys.platform.startswith("darwin"):
                        ...
                        libc = ctypes.CDLL("libc.so.6") # Added fallback for linux
    ...
    def _get_caller_name():
        ...
        """
        return sys._getframe(2).f_code.co_name # Changed to get the caller of the caller (to account for _set_print)
    ...
    def set_print_for_function(func_name, mode):
        ...
        Args:
            ...
            mode (bool): True to enable auto-print for this function, False to disable it.
        """
        if not isinstance(mode, bool):
            raise ValueError("Mode must be a boolean (True or False).")
        XSleep._auto_print_flags[func_name] = mode
