...
class RestClient:
    ...
    def get_job(self, job_id: str | None = None, history: bool = False):
        ...
        try:
            ...
            with self.get_client() as client:
                response = client.get(
                    f"/v0.1/crow-jobs/{job_id}", params={"history": history}
                )
                response.raise_for_status()
                return response.json()
    ...
    def create_crow(
        ...
        try:
            module_name = (
                config.path.name
                if config.path
                else config.functional_environment.__name__
            )
            ...
            files = []
            for file_path in config.path.rglob("*") if config.path else []:
                if any(
...
def get_used_global_imports(
    func, global_imports: dict[str, str], global_scope: dict, visited=None
) -> set[str]:
            ...
            if callable(ref_func):
                used_imports.update(
                    get_used_global_imports(
                        ref_func, global_imports, global_scope, visited
                    )
                )
...
def get_used_modules(env_builder: EnvironmentBuilder, global_scope: dict) -> set[str]:
    ...
    used_imports = get_used_global_imports(
        env_builder.start_fn, global_imports, global_scope
    )
...
def generate_requirements(
    env_builder: EnvironmentBuilder, global_scope: dict
) -> list[str]:
