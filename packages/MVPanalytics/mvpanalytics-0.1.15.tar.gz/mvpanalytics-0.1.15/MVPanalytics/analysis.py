def weightgiver(api_key, host, comp_id, str_id,  perc_min, per, ban=False, max_ban=0.2, prime = False, prime_add = 0.1):
    ...
    """
        ...
        Args:
            ...
            ban: (list\None) - список из id рекламодателей, которые не в приоритете
    ...
    if len(spl)>0:    
        ...
        if ban != False:        
            ...
            if len(ban_perc)>0:
                spl.loc[(spl['affiliate_network_id'].isin(ban))& (spl['perc']>max_ban), 'perc'] = ban_perc - ban_perc +0.2
        ...
        if prime != False:
            ...
            if l_wht>0:
                ...
                if len(n_wht)>0:
                    ...
                    spl.loc[(~spl['affiliate_network_id'].isin(prime)) & (spl['perc']>prime_add/len(n_wht)), 'perc'] = n_wht - prime_add/len(n_wht)
