...
# langchain-openai
def intercept_langchain_openai_llms(agent, mod):
    LangchainCommunityLLMInterceptor(agent, mod.OpenAI).attach(['_agenerate', '_generate', '_astream', '_stream'])


def intercept_langchain_openai_embeddings(agent, mod):
    LangchainCommunityEmbeddingsInterceptor(agent, mod.OpenAIEmbeddings).attach(
        ['embed_query', 'embed_documents', 'aembed_query', 'aembed_documents'])


def intercept_langchain_openai_chat_models(agent, mod):
    LangchainCommunityLLMInterceptor(agent, mod.ChatOpenAI).attach(['_agenerate', '_generate', '_astream', '_stream'])
...
def intercept_langchain_community_llms(agent, mod):
    # Open AI LLM interceptors
    LangchainCommunityLLMInterceptor(agent, mod.OpenAIChat).attach(['_agenerate', '_generate', '_astream', '_stream'])
    LangchainCommunityLLMInterceptor(agent, mod.OpenAI).attach(['_agenerate', '_generate', '_astream', '_stream'])
...
def intercept_langchain_community_embeddings(agent, mod):
    # Open AI embeddings model interceptors
    LangchainCommunityEmbeddingsInterceptor(agent, mod.OpenAIEmbeddings).attach(
        ['embed_query', 'embed_documents', 'aembed_query', 'aembed_documents'])
