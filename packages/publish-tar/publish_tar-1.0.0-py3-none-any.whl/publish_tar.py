...
class TARPublisher:
    def __init__(self, prod_name: str, disk: str = 'D'):
    ...
    def publish_jsons(self, artifact_obj: dict):
        ...
        info = {
            'Build Reason': os.getenv('BUILD_REASON', default_info) if os.getenv('BUILD_REASON', default_info) == 'Schedule' else f'Manual By {os.getenv("BUILD_REQUESTEDFOR", default_info)}',
            ...
            'Pipeline Run': f'https://dev.azure.com/hexagonmi/DE-VirtualManufacturing/_build/results?buildId={os.environ["BUILD_BUILDID"]}',
            'Pipeline Run Datetime': datetime.datetime.now().strftime("%B %d %H:%M:%S"),
            'Xray Execution Link': f'https://hexagon.atlassian.net/browse/{os.environ["XRAY_EXECUTION_ID"]}' if os.getenv("XRAY_EXECUTION_ID", default_info) != default_info else default_info,
        }
...
def main():
    ...
    if prod_name is None or prod_name not in valid_prods:
        ...
        sys.exit(-1)
    TARPublisher(prod_name, disk).publish()
