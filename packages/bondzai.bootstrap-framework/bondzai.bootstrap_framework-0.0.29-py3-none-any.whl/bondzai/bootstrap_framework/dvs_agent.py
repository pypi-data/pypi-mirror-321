    ...
    def load_initial_data(self) -> int:
        ...
        if len(list_datasets) > 0:
            ...
            for ds in list_datasets:
                ...
                if not sourceid:
                    logger.warn(f"Loading features from {ds} fails, source id not found")
                    continue
                ...
                for index, labels in enumerate(dataset["labels"]):
                    if index >= len(dataset["signatures"]):
                        logger.warn(f"Number of labels ({len(dataset['labels'])}) higher than Number of signatures ({len(dataset['signatures'])})")
                        break
                    sigVect = dataset["signatures"][index]
                    groundthruth_for_davinsy = []
                    for labeltype, labelv in labels.items():
                        ...
                        if label_desc:
                            ...
                            if cluster_id:
                                label_value_id = cluster_id

                                # convert:
                                labeltype_rej = labeltype + "_rej"
                                label_type_id, _ = self.config.get_item_by_name("labels", labeltype_rej)
                            ...
                            if label_type_id is not None and label_value_id is not None:
                                try:
                                    groundthruth_for_davinsy.append(int(label_type_id))
                                    groundthruth_for_davinsy.append(int(label_value_id))
                                except Exception as e:
                                    logger.info(f"Invalid ground-truth, label type {labeltype}: {label_type_id}, label value {labelv.get('value','')}: {label_value_id}")
                            else:
                                logger.info(f"Data ignored because label {label_type_id}:{labelv.get('value','')} not in {label_desc.get('values',{})}")

                    #logger.debug(f"try to load_one_raw_data inputVect {sigVect}, expectedOutput {groundthruth_for_davinsy}, source_id {sourceid}")
                    if len(groundthruth_for_davinsy) == 0:
                        logger.debug(f"Data index {index} ignored because empty ground-truth, labels {labels}")
                    else:
                        try:
                            isLoaded = self.load_one_raw_data(is_classification,inputVect=sigVect, expectedOutput=groundthruth_for_davinsy, source_id=sourceid)
                        except Exception as e:
                            logger.error(f"Fail to load_one_raw_data inputVect {sigVect}, expectedOutput {groundthruth_for_davinsy}, source_id {sourceid}")
                            isLoaded = 0
                        ack += int(isLoaded)
                    if ack >= (self.config.get_max_nb_raw_data()-1):
                        logger.warn("DavinSy Internal Storage for raw data might be full")
