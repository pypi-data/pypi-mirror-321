...
from infoapps_mlops_sdk.src.infoapps_mlops_sdk.general_utilities import get_server_url
from infoapps_mlops_sdk.src.infoapps_mlops_sdk.sdk_logging.http_handler import HTTPLogHandler

logger = logging.getLogger("infoapps_mlops_sdk")
...
def init_experiment(experiment_name, platform_type, owner_email=None, source_files=None, **kwargs):
    logger.info("init_experiment called.")

    # Extract the caller's source file details
    caller_frame = inspect.stack()[1]
    source_file_path = caller_frame.filename
    source_file_name = os.path.basename(source_file_path)  # Extract just the file name

    # Define a dictionary mapping for platform initialization
    platform_initializers = {
        PlatformType.HUB: lambda: HubPlatform(**kwargs),
        PlatformType.TENSORFLOW: lambda: TensorflowPlatform(**kwargs),
        PlatformType.PYTORCH_LIGHTNING: lambda: PytorchLightningPlatform(
            experiment_name=experiment_name,
            owner_email=owner_email,
            **kwargs
        ),
        PlatformType.PYTORCH: lambda: PytorchPlatform(
            experiment_name=experiment_name,
            owner_email=owner_email,
            source_file_path=source_file_path,
            source_file_name=source_file_name,
            source_files=source_files,  # Pass source files
            **kwargs
        ),
        PlatformType.MLFLOW: lambda: MLFlowPlatform(**kwargs),
        PlatformType.KERAS: lambda: KerasPlatform(
            experiment_name=experiment_name,
            owner_email=owner_email,
            **kwargs
        ),
    ...
    try:
        initializer = platform_initializers[platform_type]
        return initializer()
    except Exception as e:
        logger.error(f"init_experiment() | {e}")
...
class BasePlatform:
    ...
    def upload_the_source(self, flask_url, source_file_path, source_file_name):
        ...
        try:
            ...
            result = self.send_source_file(source_file_path, source_file_name, flask_url, self.experiment_id,
                                           self.run_id, "source_file")
...
class PytorchPlatform(BasePlatform):
    ...
    def __init__(self, experiment_name="default", owner_email=None, source_file_path=None, source_file_name=None,
        ...
        try:
            ...
            self.server_url = get_server_url(
                ...
                USE_BETA_URL=USE_BETA_URL
            )

            logger.info(f"PytorchPlatform - Server URL set to: {self.server_url}")

            source_path = kwargs.get("source_path", None)
            set_file_name = kwargs.get("set_file_name", None)
            ...
            if response.status_code == 200:
                ...
                logger.info(f"PytorchPlatform - check_or_create() | Experiment ID: {self.experiment_id}, Run ID: {self.run_id}")

                # Upload provided source files
                source_endpoint = f"{self.server_url}/api/experiments/uploadArtifact"
                if source_files:
                    for file in source_files:
                        self.upload_the_source(
                            source_endpoint,
                            file,
                    ...
                    if source_path != None:
                        if source_path.endswith("/"):
                            path_to_source = source_path + set_file_name
                    else:
                        path_to_source = source_file_path

                    # Fallback to the default source file
                    set_file_name = set_file_name if set_file_name else source_file_name
                    ...
                    try:
                        ...
                        logger.error(f"PytorchPlatform | Failed to upload source file: {e}")
                        logger.error(f"PytorchPlatform | source_endpoint: {source_endpoint} | path_to_source: {path_to_source} | set_file_name: {set_file_name}")

                logger.info(f"PytorchPlatform - set_file_name: {set_file_name} | Source file: {source_file_name} | uploaded to: {source_endpoint}")
            else:
                logger.error(f"PytorchPlatform() | Failed to create or retrieve experiment.")
                raise Exception("Failed to create or retrieve experiment.")

        except Exception as e:
            logger.error(f"Error initializing PytorchPlatform: {e}")
            raise e
    ...
    def __setitem__(self, key, value):
        ...
        """
        logger.info(f"Setting metric: {key} = {value}")
    ...
    def __getitem__(self, key):
        ...
        """
        logger.info(f"Getting metric: {key}")
    ...
    def log_metric(self, name, value, step=None):
        ...
        """
        logger.info(f"Logging metric: {name} = {value} (Step: {step})")
        payload = {
    ...
    def track_files(self, name, file_path):
        logger.info(f"Tracking file: {name}")
        try:
            ...
            logger.error(f"Failed to track file {name}: {e}")
    ...
    def done(self):
        """
                ...
                """
        if not self.run_id:
            logger.error("Run ID not set. Unable to stop the run.")
        ...
        if response.status_code == 200:
            ...
            logger.error(f"Failed to stop the run.")
