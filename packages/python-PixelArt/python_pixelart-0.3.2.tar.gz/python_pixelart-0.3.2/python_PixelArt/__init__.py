...
def PixelArt(image_path: str, by: int, output: str = "./PixelArt.png", save_image: bool = True, colors=None, warnings: bool = True):
    ...
    if colors == 16:
        ...
        mode = 256
        colors = []
        for r in [0, 36, 72, 109, 145, 182, 218, 255]:
            for g in [0, 36, 72, 109, 145, 182, 218, 255]:
                for b in [0, 36, 72, 109, 145, 182, 218, 255]:
        ...
        for r in range(32):
            for g in range(64):
                for b in range(32):
                    colors.append(((r * 255) // 31, (g * 255) // 63, (b * 255) // 31, 255))
    ...
    too_small_image_error_message = f"Image trop petite pour y en faire un Pixel Art {str(by)}×{str(by)}.\n\nToo small image to make a Pixel Art = {str(by)}×{str(by)}."
