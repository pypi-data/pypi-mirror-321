...
def import_survey_data(selection, export_csv=False, path=None):
    survey_map = get_survey_map()
...
def import_surveys(selection, export_csv=False):
    survey_map = get_survey_map()
    survey_name = survey_map.get(selection)
    ...
    file_path = ""
    if export_csv:
        owner_email = os.getenv("OWNER_EMAIL").replace("@", "_at_")
        ...
        file_path = os.path.join(
            ...
            owner_email,
            date_stamp,
            f"survey_{survey_name}",
            f"survey_{survey_name}_{date_stamp}.csv"
        )
