...
def map_answers_inR(input_data):
    special_cases = get_special_cases()
    ...
    for idx, row in input_data.iterrows():
        ...
        if answer_id in special_cases:
            input_data.at[idx, 'answer_numeric'] = special_cases[answer_id][0]
            input_data.at[idx, 'answer_text'] = special_cases[answer_id][1]
        else:
            input_data.at[idx, 'answer_numeric'] = mapping_numeric.get(question_id, {}).get(answer_id, pd.NA)
            input_data.at[idx, 'answer_text'] = mapping_text.get(question_id, {}).get(answer_id, pd.NA)
...
def map_items(input_data):
    ...
    def apply_mappings(row):
        if pd.notna(row['answer_numeric']) and pd.notna(row['answer_text']):
            return pd.Series([row['answer_numeric'], row['answer_text']])

        question_id = row['question_concept_id']
        answer_id = row['answer_concept_id']
        numeric = mapping_numeric.get(question_id, {}).get(answer_id, pd.NA)
        text = mapping_text.get(question_id, {}).get(answer_id, pd.NA)
        return pd.Series([numeric, text])

    result = input_data.apply(apply_mappings, axis=1)
    input_data[['answer_numeric', 'answer_text']] = result

    input_data['answer_numeric'] = pd.to_numeric(input_data['answer_numeric'], errors='coerce')

    numeric_mask = pd.isna(input_data['answer_concept_id']) & input_data['answer'].apply(lambda x: str(x).isdigit())
    input_data.loc[numeric_mask, 'answer_numeric'] = input_data.loc[numeric_mask, 'answer'].astype(float)
    input_data.loc[numeric_mask, 'answer_text'] = input_data.loc[numeric_mask, 'answer'].astype(str)

    input_data['answer_numeric'] = input_data.apply(lambda row: get_numeric_value(row), axis=1)
...
def process_items(input_data):
    ...
    converted_data = numeric_conversion(result_df)

    print(f"The number of unique person_ids in the dataset: {converted_data['person_id'].nunique()}")
