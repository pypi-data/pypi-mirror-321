...
def map_items(input_data):
    ...
    data_recoded =pd.DataFrame(input_data)

    data_recoded['answer_numeric'] = pd.to_numeric(data_recoded['answer_numeric'], errors='coerce').astype('float')
...
def process_items(input_data):
    ...
    result_df = pd.concat(results, ignore_index=True)
    data_recoded = pd.DataFrame(result_df)
