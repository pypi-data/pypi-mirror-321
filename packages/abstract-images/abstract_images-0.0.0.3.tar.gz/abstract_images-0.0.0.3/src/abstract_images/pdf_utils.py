    ...
    for i, each in enumerate(pdf_list):
        ...
        for j, image in enumerate(images):
            if paginate:
                ...
                image_output_path = os.path.join(output_folder, f"{file_name}_Page_{j}.{extension}")
    ...
    for file_path in pdf_list:
        with open(file_path, 'rb') as pdf_file:
            ...
            for page_num in range(len(pdf_reader.pages)):
                pdf_writer.add_page(pdf_reader.pages[page_num])
    save_pdf(output_file_path, pdf_writer)
imgDir = '/media/computron/gen1macair/output/imgs'
textDir = "/media/computron/gen1macair/output/text"
image_list = [os.path.join(imgDir,img) for img in os.listdir(imgDir)]
#pdf_to_img_list(pdf_list=['/media/computron/gen1macair/Joe Jamail 4 Trust Documents.pdf'], output_folder=textDir, file_name='Joe Jamail 4 Trust Documents.pdf', paginate=True)
img_to_txt_list(img_list=image_list, output_folder=textDir, file_name='Joe Jamail 4 Trust Documents.pdf', paginate=True, extension="txt")
