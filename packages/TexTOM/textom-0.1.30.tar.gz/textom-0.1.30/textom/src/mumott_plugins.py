...
def mumottize( path, 
    ...
    if 'controt' in geo.scan_mode:
        ...
        import_scan_data(path, sub_data, file,
            geo, q_range, q_index_range, flip_fov=flip_fov,rec_1d=rec_1d,
            check=True)
        # actual import
        ...
        with mp.Pool(processes=n_threads) as pool:
            ...
            parallel_res = pool.starmap(import_scan_data, 
                [(path, sub_data, filename, geo, q_range, q_index_range, flip_fov,rec_1d
                  ) for filename in filelist])
        dat = np.concatenate([p[0] for p in parallel_res], axis=0)
        kappa = np.concatenate([p[1] for p in parallel_res], axis=0)
        omega = np.concatenate([p[2] for p in parallel_res], axis=0)
        airscat = np.concatenate([p[3] for p in parallel_res], axis=0)     
...
def import_scan_data(
    ...
    with h5py.File(os.path.join(path, sub_data, file),'r') as hf:
        ...
        # phi_in = ( hf['azimuthal_units'][0]*np.pi/180 ).astype(data_type)
