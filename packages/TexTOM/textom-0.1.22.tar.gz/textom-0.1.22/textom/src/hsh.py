...
dtype_complex = np.complex64 if data_type==np.float32 else np.complex128
...
def R( n, gl, gr=np.array([0,0,0]) ):
    ...
    UrUl = np.kron( Ur_inv, Ul ).astype(dtype_complex)
    CG = CGn( n ).astype(dtype_complex)
...
def U(j,ome,tta,phi,invert=False):
    ...
    U = wigner_d(j, xi).astype(dtype_complex)
...
def wigner_d(j,tta):
    ...
    Jy = np.zeros((n, n),dtype_complex)#np.zeros([n, n],dtype=complex)
    for a in range(1,n):
        b = n - a - 1
        Jy[a - 1, a] = -X[a]
        Jy[b + 1, b] =  X[a]
...
def CGn(n):
    ...
    """
    l = n/2
    CG = np.zeros( ((n + 1)**2, (n + 1)**2), data_type)
...
def CleGor(j1, j2, J, M):
    ...
    if (J < abs(j1 - j2) or J > j1 + j2 or abs(M) > J):
        ...
        m1 = np.arange(m11,m1n+1,1,data_type)
        m2 = M - m1
