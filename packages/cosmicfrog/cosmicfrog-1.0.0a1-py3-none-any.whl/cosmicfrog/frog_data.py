...
class FrogModel:
    ...
    @check_connection_type("app_key")
    def stop_scenario(self, scenario_name: Optional[str] = '', job_key: Optional[str] = '', workspace: str = "Studio") -> Dict[str, str]:
        """
        ...
        """
        if not workspace:
            return {"status": "error", "message": "Workspace is required. Use Studio if not sure."}

        if not scenario_name and not job_key:
            return {"status": "error", "message": "Scenario name or job key is required."}

        try:
            oc = OptilogicClient(appkey=self._app_key)

            if scenario_name and not job_key:
                tags = f"Scenario={scenario_name}" # this is specified when running a scenario and it can't be overriden

                # Easier to fetch submitted and running jobs rather than fetching all as would have to filter through completed
                submitted_jobs = oc.api.wksp_jobs(wksp=workspace, tags=tags, status="submitted")
                running_jobs = oc.api.wksp_jobs(wksp=workspace, tags=tags, status="running")

                all_jobs = submitted_jobs["jobs"] + running_jobs["jobs"]

                if not all_jobs:
                    return {"status": "success", "message": f"No submitted or running jobs with scenario name {scenario_name}"}

                for job in all_jobs:
                    job_key = job["jobKey"]
                    if job_key:
                        oc.api.wksp_job_stop(wksp=workspace, jobkey=job_key)

                return {"status": "success", "message": f"Stopped {len(all_jobs)} jobs with scenario name {scenario_name}"}

            response = oc.api.wksp_job_stop(wksp=workspace, jobkey=job_key)
    ...
    @check_connection_type("app_key")
    def check_scenario_status(self, scenario_name: Optional[str] = '', job_key: Optional[str]= '', history: str = '7', workspace: str = "Studio") -> Dict[str, str]:
        """
        ...
        try:
            ...
            if scenario_name and not job_key:
                ...
                all_jobs = oc.api.wksp_jobs(wksp=workspace, tags=tags, history=history)

                if not all_jobs:
                    return {"status": "success", "message": f"No jobs with scenario name {scenario_name} in the last {history} days"}

                return {"status": "success", "message": "", "statusCounts": all_jobs["statusCounts"], "jobs": all_jobs["jobs"]}

            job = oc.api.wksp_job_status(jobkey=job_key, wksp=workspace)
    ...
    #@staticmethod
    # def all_runing_scenarios(app_key: Optional[str] = None) -> dict:
    ...
    def create_model(
        ...
        Returns:
            dict: A dictionary containing the status, message, and model_id
        """
        if not name:
        ...
        try:
            ...
            return {"status": "success", "message": f"Model {name} created successfully", "model_id": response.get('storageId', '')}
        except ValueError as ve:
            return {"status": "error", "message": str(ve)}
        except Exception as e:
            return {"status": "error", "message": str(e)}
    ...
    def delete_model(self) -> dict:
        ...
        try:
            oc = OptilogicClient(appkey=self._app_key)
            response = oc.api.storage_delete(name=self.model_name)

            if response.get('result', 'error') == 'error':
                return {"status": "error", "message": response.get('message', '')}

            return {"status": "success", "message": f"Model {self.model_name} deleted successfully"}
        except Exception as e:
            return {"status": "error", "message": str(e)}
