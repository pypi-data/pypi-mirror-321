...
class Assistant(BaseModel):
    ...
    def _initialize_model(self):
        ...
        api_key = getattr(self, 'api_key', None) or os.getenv(f"{self.model.upper()}_API_KEY")
        ...
        llm_provider = self.model.lower()

        if llm_provider not in llm_providers:
            raise ValueError(f"Unsupported LLM provider: {self.model}. Supported providers are: {list(llm_providers.keys())}")
        ...
        model_to_use = getattr(self, 'llm_model', None) or default_model
        ...
        self.llm = llm_class(model=model_to_use, api_key=api_key)
