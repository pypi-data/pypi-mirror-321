...
class CryptoPriceChecker(BaseTool):
    def __init__(
        self,
        default_symbol: str = "BTC-USD",  # Default cryptocurrency
        enable_historical_data: bool = True,  # Enable historical data
        enable_multiple_symbols: bool = True,  # Enable multiple cryptocurrencies
        enable_metrics: bool = True,  # Enable additional metrics (e.g., volume, market cap)
    ):
        """
        Initialize the CryptoPriceChecker tool with customizable options.

        Args:
            default_symbol (str): Default cryptocurrency symbol (e.g., "BTC-USD").
            enable_historical_data (bool): Enable fetching historical data.
            enable_multiple_symbols (bool): Enable fetching data for multiple cryptocurrencies.
