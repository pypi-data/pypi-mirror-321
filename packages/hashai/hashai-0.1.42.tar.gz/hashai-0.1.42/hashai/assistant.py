...
class Assistant(BaseModel):
    ...
    team: Optional[List['Assistant']] = Field(None, description="List of agents in the team.")
    ...
    def collaborate_on_task(self, task: str) -> Dict[str, Any]:
        """
        Collaborate on a task by using the LLM to split it into subtasks and assign them to the appropriate agents.
        Each agent will follow its specific instructions while performing the subtask.
        """
        if not self.team:
            raise ValueError("No team members available for collaboration.")

        # Step 1: Use the LLM to detect tools for the task
        tool_calls = self._detect_tool(task)
        if not tool_calls:
            logger.warning("No tools detected for the task.")
            return {"error": "No tools detected for the task."}

        # Step 2: Assign tasks to agents based on the detected tools
        responses = {}
        for tool_call in tool_calls:
            tool_name = tool_call["tool"]
            tool_input = tool_call["input"]

            # Find the agent that can handle the tool
            assigned_agent = next((agent for agent in self.team if tool_name in [tool.name for tool in agent.tools]), None)
            if not assigned_agent:
                logger.warning(f"No agent found for tool '{tool_name}'. Skipping subtask.")
                continue

            # Step 3: Include the agent's instructions in the subtask prompt
            agent_instructions = "\n".join(assigned_agent.instructions) if assigned_agent.instructions else "No specific instructions."
            subtask_prompt_with_instructions = f"""
            ...
            Please perform the task while strictly following your instructions.
            """

            # Execute the subtask with the agent's instructions
            logger.info(f"Executing subtask: {task} (Assigned Agent: {assigned_agent.name})")
            try:
                response = assigned_agent.print_response(subtask_prompt_with_instructions)
                responses[assigned_agent.name] = response
            except Exception as e:
                logger.error(f"Failed to execute subtask '{task}' with agent '{assigned_agent.name}': {e}")
                responses[assigned_agent.name] = f"Error: {e}"
        ...
        **kwargs,
        ...
        if stream:
            ...
            if self.team:
                ...
                response = self.collaborate_on_task(message)
            else:
    ...
    def _detect_tool(self, message: str) -> Optional[List[Dict[str, Any]]]:
        """
        Use the LLM to detect which tools should be called based on the user's query.
        Returns a list of tools and their inputs.
        """
        if not self.tools:
            logger.warning("No tools available to detect.")
            return None

        # Create a prompt for the LLM to detect multiple tools
        prompt = f"""
        You are an AI assistant that helps users by selecting the most appropriate tools to answer their query. Below is a list of available tools and their functionalities:

        {self._get_tool_descriptions()}

        Based on the user's query, select the most appropriate tools. Respond with a JSON list of tools and their inputs. If no tool is suitable, respond with an empty list.
        ...
        {{
            "tools": [
                {{
                    "tool": "CryptoPriceChecker",
                    "input": {{"query": "Bitcoin"}}
                }},
                {{
                    "tool": "DuckDuckGo",
                    "input": {{"query": "latest news for Bitcoin"}}
                }}
            ]
        }}
        """
        ...
        try:
            ...
            tools_to_call = tool_data.get("tools", [])
    ...
    def _generate_response(self, message: str, markdown: bool = False, **kwargs) -> str:
        """Generate the assistant's response, including tool execution."""
        # Check if the message contains a tool call
        tool_call = self._detect_tool_call(message)
        if tool_call:
            tool_name = tool_call["tool"]
            tool_input = tool_call["input"]
