...
class Assistant(BaseModel):
    ...
    def assign_task_to_team(self, task: str, agent_name: Optional[str] = None) -> Any:
        """
        Assign a task to a specific agent in the team or to the team leader (this assistant).
        """
        if agent_name and self.team:
            for agent in self.team:
                if agent.name == agent_name:
                    return agent.print_response(task)
            raise ValueError(f"Agent with name '{agent_name}' not found in the team.")
        else:
            # If no agent is specified, assign the task to this assistant (team leader)
            return self.print_response(task)

    def collaborate_on_task(self, task: str) -> Dict[str, Any]:
        """
        Collaborate on a task by distributing it among all agents in the team and combining their responses.
        """
        if not self.team:
