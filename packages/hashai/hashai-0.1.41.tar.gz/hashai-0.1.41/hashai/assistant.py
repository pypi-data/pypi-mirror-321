...
class Assistant(BaseModel):
    ...
    def collaborate_on_task(self, task: str) -> Dict[str, Any]:
        ...
        # Step 1: Use the LLM to split the task into subtasks and assign them to agents
        subtask_prompt = f"""
        You are a task coordinator. Your job is to split the following task into subtasks and assign each subtask to the most appropriate agent.
        ...
        {{
            "subtasks": [
                {{
                    ...
                    "assigned_agent": "name of the agent"
                }},
                ...
            ]
        }}
        ...
        # Call the LLM to generate the subtask assignment
        subtask_response = self.llm_instance.generate(prompt=subtask_prompt)
        try:
            subtask_data = json.loads(subtask_response)
            subtasks = subtask_data.get("subtasks", [])
        except json.JSONDecodeError:
            logger.error("Failed to parse subtask assignment from LLM response.")
            return {"error": "Failed to parse subtask assignment."}

        # Step 2: Execute the subtasks and collect the results
        responses = {}
        for subtask in subtasks:
            subtask_description = subtask.get("subtask")
            assigned_agent_name = subtask.get("assigned_agent")

            # Find the assigned agent
            assigned_agent = next((agent for agent in self.team if agent.name == assigned_agent_name), None)
            if not assigned_agent:
                logger.warning(f"Agent '{assigned_agent_name}' not found. Skipping subtask: {subtask_description}")
                continue
            ...
            logger.info(f"Executing subtask: {subtask_description} (Assigned Agent: {assigned_agent_name})")
            try:
                response = assigned_agent.print_response(subtask_prompt_with_instructions)
                responses[assigned_agent_name] = response
            except Exception as e:
                logger.error(f"Failed to execute subtask '{subtask_description}' with agent '{assigned_agent_name}': {e}")
                responses[assigned_agent_name] = f"Error: {e}"
