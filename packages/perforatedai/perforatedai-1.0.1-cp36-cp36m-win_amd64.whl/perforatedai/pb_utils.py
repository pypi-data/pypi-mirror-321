    ...
    if issubclass(type(net),nn.Sequential) or issubclass(type(net),nn.ModuleList):
        for submoduleID, layer in net.named_children():
            ...
            if (type(net.get_submodule(submoduleID)) in PBG.modulesToConvert
                ...
                setattr(net,submoduleID,PB.pb_neuron_layer(net.get_submodule(submoduleID), nameSoFar + '.' + str(submoduleID), pretrainedPB=getPretrainedPBVar(pretrainedPB, submoduleID)))
            else:
                if(net != net.get_submodule(submoduleID)):
                    setattr(net,submoduleID,convertModule(net.get_submodule(submoduleID),  getPretrainedPBVar(pretrainedPB, submoduleID), depth + 1, nameSoFar + '.' + str(submoduleID)))
                #else:
        ...
        for member in allMembers:        
            ...
            if (issubclass(type(getattr(net,member,None)), torch.nn.modules.batchnorm._BatchNorm) or issubclass(type(getattr(net,member,None)), torch.nn.modules.instancenorm._InstanceNorm) or
                ...
                print('potentially found a batchNorm Layer that wont be convereted2, this is not reccomended: %s' % (nameSoFar + '.' + member))
                pdb.set_trace()
            else:
...
def loadNetFromDict(net, stateDict):
    ...
    for module in pbModules:
        ...
        module.setThisInputDimensions(stateDict[moduleName + '.pb.pbValues.0.thisInputDimensions'].tolist())
