...
def load_data(data: pd.DataFrame, input_parameters: list, output_parameters: list,
    ...
    if add_noise:
        ...
        m_train_x, adjusted_cov_train_x = noise_generator(error_rate, train_x)

        m_val_x, adjusted_cov_val_x = noise_generator(error_rate, val_x)
        m_test_x, adjusted_cov_test_x = noise_generator(error_rate, test_x)

        noise_train_x = np.random.multivariate_normal(mean=m_train_x, cov=adjusted_cov_train_x,
                                                      size=train_x.shape[0])
        noise_val_x = np.random.multivariate_normal(mean=m_val_x, cov=adjusted_cov_val_x, size=val_x.shape[0])
        noise_test_x = np.random.multivariate_normal(mean=m_test_x, cov=adjusted_cov_test_x, size=test_x.shape[0])
