...
def start(as_standalone: bool = False) -> None:
    if A.U.for_service(SD, as_standalone=as_standalone):
        ...
        executor: PIHThreadPoolExecutor = PIHThreadPoolExecutor(1)
            ...
            if sc == SC.send_event:
                if subscribtion_result.result:  # type: ignore
                    ...
                    if event == A.CT_E.NEW_FILE_DETECTED:
                        executor.submit(
                            operate_new_file, A.PTH.path(event_parameters[0])
                        )

            return True

        def operate_new_file(file_path: str) -> None:
            if A.PTH.get_extension(file_path) in (
                ...
                if not valenta_source_host_is_available():
                    ...
                    while True:
                        if A.C_R.accessibility_by_smb_port(VALENTA_SOURCE_HOST):
                            sleep(10 * 60)
