...
def compress_image(image: PIL.Image.Image, max_size_bytes: int = 1097152, quality: int = 85) -> tuple[io.BytesIO, int]:
    """
    ...
    Returns:
        Tuple of (BytesIO object containing the compressed image, final quality used)
    """
    output = io.BytesIO()
...
def make_llm_call_gemini(image_path: str, system_message: str, model: str = "gemini-1.5-pro-002", response_schema: dict = None, max_tokens: int = 4000) -> str:
    ...
    try:
        ...
        compressed_image, quality = compress_image(image)
        response = model.generate_content(
            [
                compressed_image,
                system_message
