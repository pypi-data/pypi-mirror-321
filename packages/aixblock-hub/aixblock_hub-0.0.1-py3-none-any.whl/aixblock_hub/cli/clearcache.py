...
from aixblock_hub.hub.constants import  TEMPORARY_FOLDER_NAME
...
class ClearCacheCMD(CLICommand):
    ...
    def __init__(self, args):
        ...
        self.cache_dir = os.getenv(
            'AIXBLOCK_CACHE',
            Path.home().joinpath('.cache', DEFAULT_HUB))
    ...
    def _execute_with_confirmation(self):
        ...
        if self.args.model or self.args.dataset:
            ...
            prompt = prompt + f'entire aixblock cache at {self.cache_dir}, including ALL models and dataset.\n'
            all = True
        user_input = input(
            prompt
