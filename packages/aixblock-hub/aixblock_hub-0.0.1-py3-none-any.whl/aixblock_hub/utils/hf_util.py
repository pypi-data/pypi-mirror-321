...
from aixblock_hub.utils.constant import DEFAULT_MODEL_REVISION, Invoke
...
def _try_login(token: Optional[str] = None):
    ...
    api = HubApi()
    if token is None:
        token = os.environ.get('AIXBLOCK_API_TOKEN')
    if token:
        api.login(token)
...
def _file_exists(
    ...
    *,
    ...
    if repo_type is not None:
        logger.warning(
            'The passed in repo_type will not be used in aixblock. Now only model repo can be queried.'
        )
    _try_login(token)
    from aixblock_hub.hub.api import HubApi
    api = HubApi()
...
def _file_download(repo_id: str,
    ...
    if len(kwargs) > 0:
        logger.warning(
            ...
            'will not be used in aixblock.')
    assert repo_type in (
        None, 'model',
        'dataset'), f'repo_type={repo_type} is not supported in aixblock'
    if repo_type in (None, 'model'):
        from aixblock_hub.hub.file_download import model_file_download as file_download
    else:
        from aixblock_hub.hub.file_download import dataset_file_download as file_download
    _try_login(token)
...
def _patch_pretrained_class():
    ...
    def patch_tokenizer_base():
        """ Monkey patch PreTrainedTokenizerBase.from_pretrained to adapt to aixblock hub.
        """
    ...
    def patch_config_base():
        """ Monkey patch PretrainedConfig.from_pretrained to adapt to aixblock hub.
        """
    ...
    def patch_model_base():
        """ Monkey patch PreTrainedModel.from_pretrained to adapt to aixblock hub.
        """
    ...
    def patch_image_processor_base():
        """ Monkey patch AutoImageProcessorHF.from_pretrained to adapt to aixblock hub.
        """
    ...
    def patch_auto_processor_base():
        """ Monkey patch AutoProcessorHF.from_pretrained to adapt to aixblock hub.
        """
    ...
    def patch_feature_extractor_base():
        """ Monkey patch AutoFeatureExtractorHF.from_pretrained to adapt to aixblock hub.
        """
...
def patch_hub():
    """Patch hf hub, which to make users can download models from aixblock to speed up.
    """
...
def get_wrapped_class(module_class,
                      ...
                      file_filter=None,
                      **kwargs):
    """Get a custom wrapper class for  auto classes to download the models from the aixblock hub
    Args:
