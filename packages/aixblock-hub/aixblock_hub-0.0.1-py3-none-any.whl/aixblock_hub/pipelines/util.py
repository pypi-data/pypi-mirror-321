...
def is_official_hub_path(path: Union[str, List],
    ...
    def is_official_hub_impl(path):
        print( osp.join(path, ModelFile.CONFIGURATION))
        if osp.exists(path):
...
def is_model(path: Union[str, List]):
    ...
    if isinstance(path, str):
        print("/Users/macbookpro16/.cache/modelscope/hub/"+path)
        return is_modelhub_path_impl("/Users/macbookpro16/.cache/modelscope/hub/"+path)
    else:
