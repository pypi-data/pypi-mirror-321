...
from aixblock_hub.hub.utils.utils import get_endpoint
...
class ModelCardCMD(CLICommand):
    ...
    def define_args(parsers: ArgumentParser):
        ...
        parser.add_argument(
            ...
            type=str,
            required=False,
            help='the certification of visit aixblock')
        parser.add_argument(
            '-act',
            '--action',
            type=str,
            ...
            help='the action of api aixblock[create, upload]')
        parser.add_argument(
            '-gid',
            '--group_id',
            type=str,
            default='damo',
            help='the group name of aixblock, eg, damo')
        parser.add_argument(
            '-mid',
            '--model_id',
            type=str,
            required=True,
            help='the model name of aixblock')
        parser.add_argument(
            '-vis',
            '--visibility',
            type=int,
            default=5,
            help=
            'the visibility of aixblock[PRIVATE: 1, INTERNAL:3, PUBLIC:5]')
        parser.add_argument(
            '-lic',
            '--license',
            type=str,
            default='Apache License 2.0',
            help='the license of visit aixblock[Apache License 2.0|'
        ...
        parser.add_argument(
            ...
            type=str,
            default='这是我的第一个模型',
            help='the chinese name of aixblock')
        parser.add_argument(
            '-md',
            '--model_dir',
            type=str,
    ...
    def create_model(self):
        from aixblock_hub.hub.constants import Licenses, ModelVisibility
        visibilities = [
