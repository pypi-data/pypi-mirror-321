...
from aixblock_hub.hub.constants import (API_HTTP_CLIENT_MAX_RETRIES,
                                      ...
                                      AIXBLOCK_CLOUD_ENVIRONMENT,
                                      AIXBLOCK_CLOUD_USERNAME,
                                      AIXBLOCK_REQUEST_ID, ONE_YEAR_SECONDS,
                                      REQUESTS_API_HTTP_METHOD,
                                      DatasetVisibility, Licenses,
                                      ModelVisibility)
from aixblock_hub.hub.errors import (InvalidParameter, NotExistError,
...
from aixblock_hub.hub.git import GitCommandWrapper
from aixblock_hub.hub.repository import Repository
from aixblock_hub.utils.constant import (DEFAULT_DATASET_REVISION,
...
class HubApi:
    ...
    def __init__(self,
        ...
        Args:
            endpoint (str, optional): The aixblock server http|https address. Defaults to None.
        """
        self.endpoint = endpoint if endpoint is not None else get_endpoint()
        self.headers = {'user-agent': aixblockConfig.get_user_agent()}
    ...
    def login(
        ...
        """Login with your SDK access token, which can be obtained from
           https://www.aixblock.io user center.

        Args:
            access_token (str): user access token on aixblock.

        Returns:
            cookies: to authenticate yourself to aixblock open-api
        ...
        if DEFAULT_HUB == 'modelscope':
            path = f'{self.endpoint}/api/v1/login'
        elif  DEFAULT_HUB == 'huggingface':
            path = f'{self.endpoint}/api/login'
        elif  DEFAULT_HUB == 'aixblock':
            path = f'{self.endpoint}/api/current-user/whoami'
        ...
        aixblockConfig.save_token(access_token)
        aixblockConfig.save_cookies(cookies)
        # aixblockConfig.save_user_info(
    ...
    def create_model(self,
        ...
        """Create model repo at aixblock Hub.
        ...
        cookies = aixblockConfig.get_cookies()
        if cookies is None:
        ...
        body = {
            ...
            'TrainId': os.environ.get('aixblock_TRAIN_ID', ''),
        }
        r = self.session.post(
            path,
            json=body,
    ...
    def delete_model(self, model_id: str):
        """Delete model_id from aixblock_hub.
        ...
        if DEFAULT_HUB == 'modelscope':
            ...
            path = f'{self.endpoint}/api/models/{model_id}'
    ...
    def get_model_url(self, model_id: str):
        ...
        if DEFAULT_HUB == 'modelscope':
            ...
            return f'{self.endpoint}/api/models/{model_id}.git'
    ...
    def get_model(
        ...
        """Get model information at aixblock
    ...
    def repo_exists(
        ...
        *,
        ...
        """
        Checks if a repository exists on aixblock
    ...
    def push_model(self,
        ...
        which can be obtained from aixblock's website.
        ...
        Args:
            ...
            visibility(int, optional):
                ...
                not exists in aixblock, this function will create a new model with this
            ...
            license(`str`, defaults to `None`):
                ...
                in aixblock, this function will create a new model with this license
    ...
    def get_model_branches_and_tags_details(
        ...
        if DEFAULT_HUB == 'modelscope':
            ...
            path = f'{self.endpoint}/api/models/{model_id}/revision/main'
        print(cookies)
        print(path)
        r = self.session.get(path, cookies=cookies,
                             headers=self.builder_headers(self.headers))
    ...
    def get_dataset_meta_file_list(self, dataset_name: str, namespace: str, dataset_id: str, revision: str):
        ...
        if file_list is None:
            raise NotExistError(
                f'The aixblock dataset [dataset_name = {dataset_name}, namespace = {namespace}, '
    ...
    def dump_datatype_file(dataset_type: int, meta_cache_dir: str):
        ...
        `aixblock.utils.constant.DatasetFormations`.
        """
    ...
    def get_virgo_meta(self, dataset_id: str, version: int = 1) -> dict:
        ...
        cookies = requests.utils.dict_from_cookiejar(aixblockConfig.get_cookies())
    ...
    def datahub_remote_call(self, url):
        ...
        r = self.session.get(
            url,
            cookies=cookies,
            headers={'user-agent': aixblockConfig.get_user_agent()})
        resp = r.json()
    ...
    def dataset_download_statistics(self, dataset_name: str, namespace: str, use_streaming: bool = False) -> None:
        ...
        if dataset_name and namespace and not is_ci_test and not use_streaming:
            try:
                ...
                user_name = ''
                if AIXBLOCK_CLOUD_ENVIRONMENT in os.environ:
                    channel = os.environ[AIXBLOCK_CLOUD_ENVIRONMENT]
                if AIXBLOCK_CLOUD_USERNAME in os.environ:
                    user_name = os.environ[AIXBLOCK_CLOUD_USERNAME]
    ...
    def builder_headers(self, headers):
        return {AIXBLOCK_REQUEST_ID: str(uuid.uuid4().hex),
                **headers}
...
class aixblockConfig:
    ...
    def make_sure_credential_path_exist():
        os.makedirs(aixblockConfig.path_credential, exist_ok=True)

    @staticmethod
    def save_cookies(cookies: CookieJar):
        aixblockConfig.make_sure_credential_path_exist()
        with open(
                os.path.join(aixblockConfig.path_credential,
                             aixblockConfig.COOKIES_FILE_NAME), 'wb+') as f:
            pickle.dump(cookies, f)

    @staticmethod
    def get_cookies():
        cookies_path = os.path.join(aixblockConfig.path_credential,
                                    aixblockConfig.COOKIES_FILE_NAME)
        if os.path.exists(cookies_path):
            with open(cookies_path, 'rb') as f:
                cookies = pickle.load(f)
                for cookie in cookies:
                    if cookie.is_expired() and not aixblockConfig.cookie_expired_warning:
                        aixblockConfig.cookie_expired_warning = True
                        logger.debug(
                            'Authentication has expired, '
                            'please re-login with aixblock login --token "YOUR_SDK_TOKEN" '
    ...
    def get_user_session_id():
        session_path = os.path.join(aixblockConfig.path_credential,
                                    aixblockConfig.USER_SESSION_ID_FILE_NAME)
        session_id = ''
    ...
    def save_token(token: str):
        ...
        with open(
                os.path.join(aixblockConfig.path_credential,
                             aixblockConfig.GIT_TOKEN_FILE_NAME), 'w+') as f:
            f.write(token)
    ...
    def save_user_info(user_name: str, user_email: str):
        ...
        with open(
                os.path.join(aixblockConfig.path_credential,
                             aixblockConfig.USER_INFO_FILE_NAME), 'w+') as f:
    ...
    def get_user_info() -> Tuple[str, str]:
        try:
            with open(
                    os.path.join(aixblockConfig.path_credential,
                                 aixblockConfig.USER_INFO_FILE_NAME),
                    'r',
                    encoding='utf-8') as f:
                info = f.read()
    ...
    def get_token() -> Optional[str]:
        ...
        try:
            with open(
                    os.path.join(aixblockConfig.path_credential,
                                 aixblockConfig.GIT_TOKEN_FILE_NAME),
                    'r',
                    encoding='utf-8') as f:
                token = f.read()
        except FileNotFoundError:
    ...
    def get_user_agent(user_agent: Union[Dict, str, None] = None, ) -> str:
        ...
        env = 'custom'
        if AIXBLOCK_CLOUD_ENVIRONMENT in os.environ:
            env = os.environ[AIXBLOCK_CLOUD_ENVIRONMENT]
        user_name = 'unknown'
        if AIXBLOCK_CLOUD_USERNAME in os.environ:
            user_name = os.environ[AIXBLOCK_CLOUD_USERNAME]

        from aixblock_hub import __version__
        ua = 'aixblock/%s; python/%s; session_id/%s; platform/%s; processor/%s; env/%s; user/%s' % (
            __version__,
            platform.python_version(),
            aixblockConfig.get_user_session_id(),
            platform.platform(),
            platform.processor(),
            env,
            user_name,
