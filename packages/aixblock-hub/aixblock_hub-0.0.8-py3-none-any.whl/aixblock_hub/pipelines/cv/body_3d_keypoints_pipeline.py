...
from aixblock_hub.models.cv.body_3d_keypoints.canonical_pose.body_3d_pose import \
    KeypointsTypes
...
class Body3DKeypointsPipeline(Pipeline):
    ...
    def __init__(self, model: str, **kwargs):
        ...
        Args:
            model (str): model id on aixblock hub.
        ...
        Example:
            ...
            >>> test_video_url = 'https://app.aixblock.io/test/videos/Walking.54138969.mp4'
    ...
    def read_video_frames(self, video_url: Union[str, cv2.VideoCapture]):
        ...
        if isinstance(video_url, str):
            ...
            if not cap.isOpened():
                raise Exception(
                    'aixblock error: %s cannot be decoded by OpenCV.' %
                    (video_url))
        else:
            cap = video_url
        ...
        if self.fps is None or self.fps <= 0:
            raise Exception('aixblock error: %s cannot get video fps info.' %
                            (video_url))
