...
from aixblock_hub.hub.constants import AIXBLOCK_REQUEST_ID
...
def get_request_id(response: requests.Response):
    if AIXBLOCK_REQUEST_ID in response.request.headers:
        return response.request.headers[AIXBLOCK_REQUEST_ID]
    else:
        return ''
