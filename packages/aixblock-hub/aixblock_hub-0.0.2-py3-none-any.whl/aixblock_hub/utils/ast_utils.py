...
# do not delete
from aixblock_hub.metainfo import (CustomDatasets, Heads, Hooks, LR_Schedulers,
...
# get the path of package 'aixblock_hub'
SKIP_FUNCTION_SCANNING = True
aixblock_PATH = p.resolve().parents[1]
INDEXER_FILE_DIR = get_aixblock_cache_dir()
REGISTER_MODULE = 'register_module'
IGNORED_PACKAGES = ['aixblock_hub', '.']
SCAN_SUB_FOLDERS = [
...
aixblock_PATH_KEY = 'aixblock_path'
VERSION_KEY = 'version'
MD5_KEY = 'md5'
INDEX_KEY = 'index'
...
def get_ast_logger():
    ast_logger = logging.getLogger('aixblock_hub.ast')
...
class FilesAstScanning(object):
    ...
    def parse_import(self, scan_result: dict) -> list:
        ...
        Returns:
            list: a list of package ignored 'aixblock_hub' and relative path import
        """
        output = []
    ...
    def get_files_scan_results(self,
        ...
        for file in self.file_dirs:
            filepath = file[file.rfind('aixblock_hub'):]
...
def _save_index(index, file_path, file_list=None, with_template=False):
    ...
    from aixblock_hub.version import __version__
    index[VERSION_KEY] = __version__
    index[MD5_KEY], index[FILES_MTIME_KEY] = file_scanner.files_mtime_md5(
        file_list=file_list)
    index[aixblock_PATH_KEY] = aixblock_PATH.as_posix()
    json_index = json.dumps(index)
    if with_template:
        json_index = json_index.replace(aixblock_PATH.as_posix(),
                                        TEMPLATE_PATH)
...
def load_index(
    ...
    Returns:
        ...
        index, requirements, files last modified time, aixblock home path,
        version and md5, the detail is shown below example: {
            'index': {
                ('MODELS', 'nlp', 'bert'):{
                    'filepath' : 'path/to/the/registered/model', 'imports':
                    ['os', 'torch', 'typing'] 'module':
                    'aixblock_hub.models.nlp.bert'
                },
                ...
            }, 'requirements': {
                'aixblock_hub.models.nlp.bert': ['os', 'torch', 'typing'],
                'aixblock_hub.models.nlp.structbert': ['os', 'torch', 'typing'],
                ...
            }, 'files_mtime' : {
                '/User/Path/To/Your/aixblock/aixblock/preprocessors/nlp/text_generation_preprocessor.py':
                16554565445, ...
            },'version': '0.2.3', 'md5': '8616924970fe6bc119d1562832625612',
            'aixblock_path': '/User/Path/To/Your/aixblock'
        }
    """
    # env variable override
    cache_dir = os.getenv('AIXBLOCK_CACHE', indexer_file_dir)
    index_file = os.getenv('aixblock_INDEX_FILE', indexer_file)
