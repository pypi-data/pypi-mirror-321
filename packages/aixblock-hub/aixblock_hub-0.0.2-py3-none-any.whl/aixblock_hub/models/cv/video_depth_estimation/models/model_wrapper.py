...
from aixblock_hub.models.cv.video_depth_estimation.utils.load import (
    filter_args, load_class, load_class_args_create, load_network)
from aixblock_hub.models.cv.video_depth_estimation.utils.misc import pcolor
    ...
    if config.name == 'DepthPoseNet':
        model_class = getattr(
            importlib.import_module(
                'aixblock_hub.models.cv.video_depth_estimation.networks.depth_pose.depth_pose_net'
            ), 'DepthPoseNet')
...
def setup_pose_net(config, prepared, **kwargs):
    ...
    pose_net = load_class_args_create(
        config.name,
        paths=[
            'aixblock_hub.models.cv.video_depth_estimation.networks.pose',
        ],
        args={
            **config,
            **kwargs
...
def setup_percep_net(config, prepared, **kwargs):
    ...
    percep_net = load_class_args_create(
        config.name,
        paths=[
            'aixblock_hub.models.cv.video_depth_estimation.networks.layers',
        ],
        args={
            **config,
            **kwargs
...
def setup_model(config, prepared, **kwargs):
    ...
    if config.name == 'SupModelMF':
        model_class = getattr(
            importlib.import_module(
                'aixblock_hub.models.cv.video_depth_estimation.models.sup_model_mf'
            ), 'SupModelMF')
