...
from aixblock_hub.utils.constant import (DEFAULT_MODEL_REVISION, Frameworks,
                                       Invoke, Tasks)
...
class GridVlpPipeline(Pipeline):
    ...
    def __init__(self, model_name_or_path: str, **kwargs):
        ...
        """
        # download model from aixblock to local model dir
        logger.info(f'load checkpoint from aixblock {model_name_or_path}')
...
class GridVlpClassificationPipeline(GridVlpPipeline):
    ...
    ```python
    >>> from aixblock_hub.pipelines.multi_modal.gridvlp_pipeline import \
