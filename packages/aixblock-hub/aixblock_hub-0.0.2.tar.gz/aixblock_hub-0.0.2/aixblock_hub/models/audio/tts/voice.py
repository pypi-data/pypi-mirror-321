...
from aixblock_hub.utils.audio.audio_utils import TtsCustomParams
...
class Voice:
    ...
    def __init__(self,
        ...
        if not os.path.exists(self.am_config_path):
            raise TtsModelConfigurationException(
                'aixblock error: am configuration not found')
        if not os.path.exists(self.voc_config_path):
            raise TtsModelConfigurationException(
                'aixblock error: voc configuration not found')
        if len(self.am_ckpts) == 0:
            raise TtsModelNotExistsException(
                'aixblock error: am model file not found')
        if len(self.voc_ckpts) == 0:
            raise TtsModelNotExistsException(
                'aixblock error: voc model file not found')
    ...
    def train_sambert(self,
        ...
        config['aixblock_version'] = __version__
