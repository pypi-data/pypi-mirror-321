...
from aixblock_hub.hub import snapshot_download
from aixblock_hub.fileio.file import LocalStorage
from aixblock_hub.utils.ast_utils import FilesAstScanning
...
aixblock_FILE_DIR = get_aixblock_cache_dir()
aixblock_DYNAMIC_MODULE = 'aixblock_modules'
BASE_MODULE_DIR = os.path.join(aixblock_FILE_DIR, aixblock_DYNAMIC_MODULE)

PLUGINS_FILENAME = '.aixblock_plugins'
OFFICIAL_PLUGINS = [
    {
        'name': 'adaseq',
        'desc':
        'Provide hundreds of additions NERs algorithms, check: https://github.com/aixblock/AdaSeq',
        'version': '',
        'url': ''
    },
]

LOCAL_PLUGINS_FILENAME = '.aixblock_plugins'
GLOBAL_PLUGINS_FILENAME = os.path.join(Path.home(), '.aixblock', 'plugins')
DEFAULT_PLUGINS = []
...
def import_all_plugins(plugins: List[str] = None) -> List[str]:
    ...
    import_module_and_submodules(
        'aixblock_hub',
        include={
            'aixblock_hub.metrics.builder',
            'aixblock_hub.models.builder',
            'aixblock_hub.pipelines.builder',
            'aixblock_hub.preprocessors.builder',
            'aixblock_hub.trainers.builder',
        },
        exclude={
            'aixblock_hub.metrics.*',
            'aixblock_hub.models.*',
            'aixblock_hub.pipelines.*',
            'aixblock_hub.preprocessors.*',
            'aixblock_hub.trainers.*',
            'aixblock_hub.msdatasets',
            'aixblock_hub.utils',
            'aixblock_hub.exporters',
        })
...
def install_requirements_by_names(plugins: List[str]):
    ...
    if status != 0:
        raise EnvironmentError(
            ...
            f'Please run the command `aixblock plugin install {" ".join(uninstalled_plugins)}` to install them.'
        )
...
class PluginsManager(object):
    ...
    def __init__(self,
                 cache_dir=aixblock_FILE_DIR,
                 plugins_file=PLUGINS_FILENAME):
        cache_dir = os.getenv('AIXBLOCK_CACHE', cache_dir)
        plugins_file = os.getenv('aixblock_PLUGINS_FILE', plugins_file)
    ...
    def pip_command(
        ...
        if command == 'install':
            ...
            command_args.append(
                'https://app.aixblock.io/releases/repo.html'
            )
...
class EnvsManager(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            cache_dir: the system aixblock cache dir
        """
        cache_dir = os.getenv('AIXBLOCK_CACHE', cache_dir)
