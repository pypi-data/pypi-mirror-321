...
from aixblock_hub.models.cv.video_single_object_tracking.config.ostrack import \
    cfg
from aixblock_hub.models.cv.video_single_object_tracking.tracker import (
    OSTrack, ProContEXT)
from aixblock_hub.models.cv.video_single_object_tracking.utils.utils import (
...
class VideoSingleObjectTrackingPipeline(Pipeline):
    ...
    def forward(self, input: Input) -> Dict[str, Any]:
        ...
        if success is False:
            raise Exception(
                'aixblock error: %s can not be decoded by OpenCV.' %
                (self.video_path))

        init_box = self.init_bbox
        frame_h, frame_w = frame.shape[0:2]
        if not check_box(init_box, frame_h, frame_w):
            raise Exception('aixblock error: init_box out of image range ',
                            init_box)
