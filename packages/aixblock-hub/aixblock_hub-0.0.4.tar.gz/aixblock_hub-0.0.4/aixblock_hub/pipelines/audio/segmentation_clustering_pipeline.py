...
class SegmentationClusteringPipeline(Pipeline):
    ...
    def preprocess(self, audio: Union[str, np.ndarray, list]) -> list:
        ...
        assert len(audio.shape) == 1, 'aixblock error: Wrong audio format.'
    ...
    def check_audio_list(self, audio: list):
        ...
        for i in range(len(audio)):
            seg = audio[i]
            assert seg[1] >= seg[0], 'aixblock error: Wrong time stamps.'
            assert isinstance(seg[2],
                              np.ndarray), 'aixblock error: Wrong data type.'
            assert int(seg[1] * self.fs) - int(
                seg[0] * self.fs
            ) == seg[2].shape[
                0], 'aixblock error: audio data in list is inconsistent with time length.'
            if i > 0:
                assert seg[0] >= audio[
                    i - 1][1], 'aixblock error: Wrong time stamps.'
            audio_dur += seg[1] - seg[0]
        assert audio_dur > 5, 'aixblock error: The effective audio duration is too short.'
    ...
    def cut_audio(self, cut_st: float, cut_ed: float,
        ...
        if isinstance(audio, np.ndarray):
            ...
            raise ValueError('aixblock error: Wrong audio format.')
