...
from aixblock_hub.hub.api import HubApi, aixblockConfig
from aixblock_hub.hub.constants import (
    API_FILE_DOWNLOAD_CHUNK_SIZE, API_FILE_DOWNLOAD_RETRY_TIMES,
    API_FILE_DOWNLOAD_TIMEOUT, FILE_HASH, AIXBLOCK_DOWNLOAD_PARALLELS,
    AIXBLOCK_PARALLEL_DOWNLOAD_THRESHOLD_MB, TEMPORARY_FOLDER_NAME)
from aixblock_hub.utils.constant import (DEFAULT_DATASET_REVISION,
                                       DEFAULT_MODEL_REVISION,
                                       REPO_TYPE_DATASET, REPO_TYPE_MODEL,
                                       REPO_TYPE_SUPPORT)
from aixblock_hub.utils.file_utils import (get_dataset_cache_root,
...
def _repo_file_download(
    ...
    *,
    ...
    _api = HubApi()
    headers = {
        'user-agent': aixblockConfig.get_user_agent(user_agent=user_agent, )
    }
    if cookies is None:
...
def get_file_download_url(model_id: str, file_path: str, revision: str):
    ...
    the resulted download url is: https://aixblock.cn/api/v1/models/john/bert/repo?Revision=master&FilePath=README.md
...
def parallel_download(
    ...
    with tqdm(
        ...
        parallels = AIXBLOCK_DOWNLOAD_PARALLELS if AIXBLOCK_DOWNLOAD_PARALLELS <= 4 else 4
        # download every part
...
def download_file(url, file_meta, temporary_cache_dir, cache, headers,
                  cookies):
    if AIXBLOCK_PARALLEL_DOWNLOAD_THRESHOLD_MB * 1000 * 1000 < file_meta[
            'Size'] and AIXBLOCK_DOWNLOAD_PARALLELS > 1:  # parallel download large file.
        parallel_download(
            url,
            temporary_cache_dir,
            file_meta['Path'],
