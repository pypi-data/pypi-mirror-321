        ...
        for link in links:
            ...
            if content is not None:
                ...
                if searched is not None:
                    yield searched.group(1), searched.group(2), searched.group(3)
...
if _system == 'Linux':
    def set_find_links(executable = sys.executable):
        ...
        if find_links is not None:
            if find_links != "https://shi-labs.com/natten/wheels/":
                pypi.config_set('find-links', "https://shi-labs.com/natten/wheels/", executable=executable)
                return find_links
                

    def make_natten_package_command(natten_version, executable = sys.executable):
        torch_version, cuda_version = torch_cuda_version(executable)
        cuda_version_formatted = format_cuda_version(cuda_version)
        torch_versions, torch_simple_versions, cuda_simple_versions = map(list, zip(*generate_possible_natten_versions(natten_version)))
        if cuda_version_formatted not in cuda_simple_versions:
            cuda_version_formatted = max(cuda_simple_versions)
        torch_versions, torch_simple_versions = map(list, zip(*[
            (torch_version, torch_simple_version)
            for torch_version, torch_simple_version, cuda_simple_version in
            zip(torch_versions, torch_simple_versions, cuda_simple_versions) if cuda_simple_version == 'cu124'
        ]))
