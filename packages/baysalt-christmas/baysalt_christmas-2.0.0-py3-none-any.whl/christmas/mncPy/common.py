"""
!!!!!!!!!! DO ATTENTION !!!!!!!!!!
Cancel mpi4py in install_requires, because it will cause error when install this package.
especially when call os.system or subprocess.call and so on to run mpirun.
...
class GlobalData(object,):
    ...
    def get_file_naming_pattern(self, filePaths):
        ...
        for filepath in self.filePaths:
            ...
            if not ndashes==ndashes0:
                raise RuntimeError("Files have different naming patterns. Make sure all files have the same pattern."+ " You may use -x flag to exclude certain files.")
            if ndashes0==1:
    ...
    def read_datetime_info(self, filePaths, time_str, time_bound_str, comm):
        ...
        if comm.Get_rank()==0:
            ...
            self.date0_in = num2date(time_bounds[0][0], ncfile[time_str].units, self.nc_calendar)
            self.date1_in = num2date(time_bounds[-1][1], ncfile[time_str].units, self.nc_calendar)
    ...
    def obtain_global_info(self, filePaths, comm, user_date0_out=None, user_date1_out=None):
        ...
        self.nmonths = (self.date1_out.year - self.date0_out.year)*12 + (self.date1_out.month - self.date0_out.month)
...
def get_file_paths(fpaths_in, args_x):
    ...
    if isinstance(fpaths_in, list):
        ...
        if len(fpaths_in)>1:
            fpaths_out = [FilePath(os.path.split(filepath)[0], os.path.split(filepath)[1]) for filepath in fpaths_in if (".nc" in filepath and not (
                        args_x is not None and args_x in filepath))]
            ...
            # fpaths_in is a single directory containing one or more netcdf files:
                ...
                for filename in os.listdir(fpaths_in[0]):
                    if filename.endswith(".nc") and not (args_x is not None and args_x in filename):
