...
def TrainTicketOCR2Excel(input_path: str, output_excel: str = r'./TrainTicketOCR2Excel.xlsx', img_url: str = None,
    ...
    for ticket in simple_progress(ticket_files):
        ticket_info = poocr.ocr.TrainTicketOCR(img_path=ticket, img_url=img_url, configPath=configPath, id=id, key=key)
        print(ticket_info)
...
def RailwayTicketInfo2Excel(ofd_file_path, output_path=r'./', output_excel='RailwayTicketInfo2Excel.xlsx',
                            configPath=None, id=None, key=None):
    """
    将火车票信息识别为Excel文件。

    该函数接受以下参数：
    - input_path: 输入文件夹路径，包含需要识别的火车票图片。
    - output_path: 输出文件夹路径，默认是当前工作目录。
    - output_excel: Excel文件名，默认是'RailwayTicketInfo2Excel.xlsx'。
    - img_url: 图片URL地址，如果提供，将从此地址下载图片进行识别。
    - configPath: 配置文件路径，用于指定OCR识别的配置。
    - id: API的用户ID。
    - key: API的密钥。

    该函数会遍历输入文件夹中的所有图片，逐一进行识别，并将识别结果保存到Excel
    文件中。

    注意事项：
    - 该函数依赖于poocr库进行OCR识别，需要提前安装好poocr包。
    - 如果输入文件夹为空，或者没有找到任何图片文件，函数不会执行任何操作。
    - 如果输出文件夹不存在，函数会自动创建它。
    - 如果输出Excel文件已经存在，函数会覆盖原有的内容。
    """
    # 获取输入文件夹下的所有图片文件
    pass
    # VerifyOfdVatInvoiceOCR(ofd_file_path=)
