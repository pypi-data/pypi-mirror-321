...
class ConfigBase(pydantic.BaseModel, Mapping, ABC):
    ...
    def parse_args(
        ...
        *,
        ...
        default_file: Path | str | None = None,
        diff_print_mode: DIFF_PRINT_MODE = "tree",
        print_config: bool = True,
        ...
        if default_file is not None:
            console.log(
                "[Expedantic] Default config file is generated at", default_file
            )
            file_dict = nested_args_dict
            # map_to_dump: CommentedMap = yaml.map(**file_dict)
            # items = list(map_to_dump.items())
            # for i in range(len(items)):
            #     k, v = items[i]
            #     if i > 0 and isinstance(items[i - 1][1], dict):
            #         map_to_dump.yaml_set_comment_before_after_key(k, before="\n")
