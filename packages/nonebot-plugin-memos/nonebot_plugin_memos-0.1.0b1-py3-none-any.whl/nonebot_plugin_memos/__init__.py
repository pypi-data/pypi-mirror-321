...
from nonebot_plugin_memos.cache import get_client
...
async def bind_handler(
    ...
    memo = await sqlSession.scalar(
        select(Memo)
        .where(Memo.user_id == session.id1)
        .where(Memo.session_persist_id == session_persist_id)
    )

    setting = await sqlSession.scalar(
        select(Setting).where(Setting.user_id == session.id1)
    )
...
async def default_handler(
    memo_id: Match[int], session: EventSession, sqlSession: async_scoped_session
):
    setting = await sqlSession.scalar(
        select(Setting).where(Setting.user_id == session.id1)
    )
    ...
    memo = await sqlSession.scalar(
        select(Memo).where(Memo.id == memo_id.result).where(Memo.user_id == session.id1)
    )
...
async def create_handler(
    content: Match[str], session: EventSession, sqlSession: async_scoped_session
):
    setting = await sqlSession.scalar(
        select(Setting).where(Setting.user_id == session.id1)
    )
    ...
    memo = await sqlSession.scalar(
        select(Memo)
        .where(Memo.id == setting.default_memo)
        .where(Memo.user_id == session.id1)
    )
