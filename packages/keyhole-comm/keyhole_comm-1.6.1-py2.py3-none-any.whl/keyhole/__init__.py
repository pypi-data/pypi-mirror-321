...
class Keyhole( object ):
	...
	Send a message using the `<<` operator. It will be automatically
	encoded to `bytes` and a newline will be appended if there wasn't
	one already::
	
	    k << "hello"     # sends b'hello\n'
	
	The return value will provide information from the first line of
	any reply that occurs within the timeout period (decoded into a
	dict, if the line was a valid dict literal) along with timing
	information.
	
	If the microcontroller has been programmed using the Keyhole
	library for the Arduino IDE, and it exposes variables via the
	keyhole, then these variables can be accessed as attributes::
	
	    print(k.foo)
	    k.foo = 123
	
	To close the connection, `k<<KEOF` or `k<<None` or `k._close()`
	will all work (this will also happen automatically when the `k`
	is garbage-collected).	
	"""
	def __init__( self, port, timeout=0.1, encoding='UTF8', **kwargs ):
		"""
		Use the same constructor args as `serial.SerialBase`.
		"""
