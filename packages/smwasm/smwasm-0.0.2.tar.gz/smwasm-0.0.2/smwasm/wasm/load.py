...
class WasmLoad:
    ...
    def load_wasm(self):
        ...
        for one in imps:
            ...
            if name == "hostputmemory":
                self.add_func(
                    FuncType([ValType.i32(), ValType.i32()], []), self.sm_hostputmemory
                )
            elif name == "hostcallsm":
                self.add_func(
                    FuncType([ValType.i32()], [ValType.i32()]), self.sm_callsm
                )
            elif name == "hostdebug":
                self.add_func(
                    FuncType([ValType.i32(), ValType.i32()], []), self.sm_hostdebug
                )
            elif name == "hostgetms":
        ...
        for one in dtRet:
            smh.register(dtRet[one], self.wasm_path, self.call_wasm)

        print(
            "+++ %s --- %s [%s-%s] --- loaded ---"
            % (self.sn, self.wasm_path, num, num_set)
        )
    ...
    def sm_emscripten_notify_memory_growth(self):
        print(
            "+++ %s +++ sm --- %s ---"
            % (self.sn, inspect.currentframe().f_code.co_name)
        )
