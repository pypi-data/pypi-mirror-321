...
def generate_markdown(repository_path, csl_path, target_path, book):
    ...
    for index, previous_chapter, current_chapter, next_chapter in neighborhood(
        ...
        extra_args = [
            ...
            f"--bibliography={textes_path / chapter_id }/{chapter_id}.bib",
            "--citeproc",
            "--wrap=preserve",
        ]
        if csl_path is not None:
            extra_args.append(f"--csl={csl_path}")
        # Convert the markdown to include citations from bib files.
...
def generate_pdf(repository_path, template_path, target_path, book):
    ...
    latex_content = pypandoc.convert_text(md_content, "latex", format="markdown")
