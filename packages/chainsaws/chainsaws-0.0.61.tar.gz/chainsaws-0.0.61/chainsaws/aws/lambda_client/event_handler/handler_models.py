...
class LambdaEvent(BaseModel):
    ...
    def is_api_gateway_event(cls, event: dict[str, Any]) -> bool:
        ...
        Returns:
            bool: True if the event is from API Gateway (REST or HTTP)
        """
        request_context = event.get("requestContext", {})

        # HTTP API (v2)
        is_http_api = bool(
            request_context.get("apiId") and
            request_context.get("http") and
        ...
        is_rest_api = bool(
            request_context.get("apiId") and
            request_context.get("httpMethod") and
...
class LambdaResponse:
    ...
    def create(
        ...
        status_code: int = 200,
        content_type: str = "application/json",
        serialize: bool = True  # API Gateway를 통한 호출인 경우 True
    ) -> dict:
