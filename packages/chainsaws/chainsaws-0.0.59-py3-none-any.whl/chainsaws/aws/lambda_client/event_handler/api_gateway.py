...
class BaseResolver(Generic[T]):
    ...
    def include_router(
        ...
        Args:
            ...
            prefix: Optional URL prefix to prepend to router's routes
            tags: Optional list of tags to add to router's routes
        """
        if prefix:
            router.base_path = prefix.rstrip('/')
        if tags:
    ...
    def _normalize_path(self, path: str) -> str:
        ...
        Returns:
            Normalized path with base path prefix
        """
        path = path.lstrip('/')  # 시작의 '/' 제거
...
class Router(BaseResolver[T]):
    ...
    def _get_full_path(self, path: str) -> str:
        """Get full path including parent's base path if exists."""
        if self.parent and self.parent.base_path:
            return f"{self.parent.base_path}{self._normalize_path(path)}"
