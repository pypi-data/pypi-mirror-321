...
def initialize_agents(
    ...
    try:
        ...
        logger.info(
            f"Agents initialized successfully: {[agent.agent_name for agent in agents]}"
        )
        return agents
...
async def execute_task(
    ...
    Tuple[Any, Optional["SwarmRouter"], str], None
]:  # Changed the return type here
    logger.info(
        f"Executing task: {task} with swarm type: {swarm_type}"
    )
    try:
        if not task:
            logger.error("Task description is missing.")
            yield "Please provide a task description.", gr.update(
                visible=True
            ), ""
            return
        if not agent_keys:
            logger.error("No agents selected.")
            yield "Please select at least one agent.", gr.update(
                visible=True
            ), ""
            return
        if not provider:
            logger.error("Provider is missing.")
            yield "Please select a provider.", gr.update(
                visible=True
            ), ""
            return
        if not model_name:
            logger.error("Model is missing.")
            yield "Please select a model.", gr.update(
                visible=True
            ), ""
            return
        if not api_key:
            logger.error("API Key is missing.")
            yield "Please enter an API Key.", gr.update(
                visible=True
            ), ""
            return
        ...
        try:
            ...
            logger.error(
                f"Error initializing agents: {e}", exc_info=True
            )
            yield f"Error initializing agents: {e}", gr.update(
                visible=True
            ), ""
            return
        ...
        if swarm_type == "AgentRearrange":
            if not flow:
                logger.error(
                    "Flow configuration is missing for AgentRearrange."
                )
                yield "Flow configuration is required for AgentRearrange", gr.update(
                    visible=True
                ), ""
                return
            ...
            router_kwargs["output_type"] = (
                "string"  # Changed output type here
            )
        ...
        if swarm_type == "MixtureOfAgents":
            if len(agents) < 2:
                ...
                yield "MixtureOfAgents requires at least 2 agents", gr.update(
                    visible=True
                ), ""
                return
        ...
        if swarm_type == "SequentialWorkflow":
            if len(agents) < 2:
                ...
                yield "SequentialWorkflow requires at least 2 agents", gr.update(
                    visible=True
                ), ""
                return
        ...
        try:
            ...
            if swarm_type == "AgentRearrange":
                ...
                logger.info(
                    "AgentRearrange task executed successfully."
                )
                yield result, None, ""
                return
            ...
            if swarm_type == "ConcurrentWorkflow":
                ...
                logger.info(
                    "ConcurrentWorkflow task executed successfully."
                )
                yield result, None, ""
                return
                ...
                if isinstance(result, dict):
                    ...
                    logger.error(
                        "Auto Swarm returned an unexpected type"
                    )
                    yield "Error : Auto Swarm returned an unexpected type", gr.update(
                        visible=True
                    ), ""
                    return
                ...
                logger.info(
                    f"{swarm_type} task executed successfully."
                )
                yield result, None, ""
                return
            ...
            yield f"Task execution timed out after {timeout} seconds", gr.update(
                visible=True
            ), ""
            return
        except (
            litellm.exceptions.APIError
        ) as e:  # Catch litellm APIError
            logger.error(f"LiteLLM API Error: {e}", exc_info=True)
            yield f"LiteLLM API Error: {e}", gr.update(
                visible=True
            ), ""
            return
        except (
            litellm.exceptions.AuthenticationError
        ) as e:  # Catch litellm AuthenticationError
            logger.error(
                f"LiteLLM Authentication Error: {e}", exc_info=True
            )
            yield f"LiteLLM Authentication Error: {e}", gr.update(
                visible=True
            ), ""
            return
        except Exception as e:
            logger.error(f"Error executing task: {e}", exc_info=True)
            yield f"Error executing task: {e}", gr.update(
                visible=True
            ), ""
            return
        ...
        yield f"An unexpected error occurred: {e}", gr.update(
            visible=True
        ), ""
        return
    finally:
        logger.info(
            f"Task execution finished for: {task} with swarm type: {swarm_type}"
        )


def format_output(
    data: Optional[str], swarm_type: str, error_display=None
) -> str:
    if data is None:
...
def parse_mixture_of_agents_data(
    data: dict, error_display=None
) -> str:
    """Parses the MixtureOfAgents output data and formats it for display."""
    logger.info(
        "Parsing MixtureOfAgents data within Auto Swarm output..."
    )

    try:
        output = ""
        if "InputConfig" in data and isinstance(
            data["InputConfig"], dict
        ):
        ...
        logger.info(
            "MixtureOfAgents data parsed successfully within Auto Swarm."
        )
        return output
...
def parse_auto_swarm_output(
    data: Optional[str], error_display=None
) -> str:
    """Parses the auto swarm output string and formats it for display."""
    logger.info("Parsing Auto Swarm output...")
    if data is None:
        logger.error(
            "No data provided for parsing Auto Swarm output."
        )
    ...
    try:
        ...
        if "input" not in parsed_data or not isinstance(
            parsed_data.get("input"), dict
        ):
            errors.append(
        ...
        if (
            ...
            if parsed_data["outputs"][0].get("agent_name") == "auto":
                mixture_data = parsed_data["outputs"][0].get(
                    "steps", []
                )
                if (
                    mixture_data
                    and isinstance(mixture_data[0], dict)
                    and "content" in mixture_data[0]
                ):
                    try:
                        mixture_content = json.loads(
                            mixture_data[0]["content"]
                        )
                        output += parse_mixture_of_agents_data(
                            mixture_content
                        )
                    except json.JSONDecodeError as e:
                        logger.error(
                            f"Error decoding nested MixtureOfAgents data: {e}",
                            exc_info=True,
                        )
                ...
                for i, agent_output in enumerate(
                    ...
                    if not isinstance(agent_output, dict):
                        errors.append(
                            f"Error: Agent output at index {i} is not a dictionary"
                        )
                        continue
                    ...
                    output += (
                        f"Run {(3-i)} (Agent: `{agent_name}`)\n\n"
                    )

                    # Iterate over steps
                    for j, step in enumerate(
                        agent_output["steps"], start=3
                    ):
                        if not isinstance(step, dict):
                            errors.append(
                                f"Error: step at index {j} is not a dictionary at {i} agent output."
                            )
                            continue
                        if step is None:
                            errors.append(
                                f"Error: step at index {j} is None at {i} agent output"
                            )
                            continue

                        if "role" not in step:
                            errors.append(
                                f"Error: 'role' key missing at step {j} at {i} agent output."
                            )
                            continue

                        if "content" not in step:
                            errors.append(
                                f"Error: 'content' key missing at step {j} at {i} agent output."
                            )
                            continue

                        if (
                            step["role"].strip() != "System:"
            ...
            logger.error(
                "Error: 'outputs' data is not in the expected format."
            )
            return (
                "Error: 'outputs' data is not in the expected format."
            )
        ...
        logger.error(
            f"Error during parsing Auto Swarm output: {e}",
            exc_info=True,
        )
...
def parse_agent_rearrange_output(
    data: Optional[str], error_display=None
) -> str:
    """
    ...
    if data is None:
        logger.error(
            "No data provided for parsing AgentRearrange output."
        )
        return "Error: No data provided for parsing."
    ...
    try:
        ...
        if errors:
            logger.error(
                f"Errors found while parsing AgentRearrange output: {errors}"
            )
            return "\n".join(errors)
        ...
        logger.error(
            f"Error during parsing AgentRearrange output: {e}",
            exc_info=True,
        )
        return f"Error during parsing: json.JSONDecodeError {e}"
...
def parse_mixture_of_agents_output(
    data: Optional[str], error_display=None
) -> str:
    ...
    if data is None:
        logger.error(
            "No data provided for parsing MixtureOfAgents output."
        )
        return "Error: No data provided for parsing."
    ...
    try:
        ...
        if "InputConfig" not in parsed_data or not isinstance(
            parsed_data["InputConfig"], dict
        ):
            logger.error(
                "Error: 'InputConfig' data is missing or not a dictionary."
            )
            return "Error: 'InputConfig' data is missing or not a dictionary."

        if "name" not in parsed_data["InputConfig"]:
            logger.error(
                "Error: 'name' key is missing in 'InputConfig'."
            )
            return "Error: 'name' key is missing in 'InputConfig'."
        if "description" not in parsed_data["InputConfig"]:
            logger.error(
                "Error: 'description' key is missing in 'InputConfig'."
            )
            return "Error: 'description' key is missing in 'InputConfig'."

        if "agents" not in parsed_data[
            "InputConfig"
        ] or not isinstance(
            parsed_data["InputConfig"]["agents"], list
        ):
            logger.error(
                "Error: 'agents' key is missing in 'InputConfig' or not a list."
            )
        ...
        for agent in parsed_data["InputConfig"]["agents"]:
            if not isinstance(agent, dict):
                logger.error(
                    "Error: agent is not a dict in InputConfig agents"
                )
                return (
                    "Error: agent is not a dict in InputConfig agents"
                )
            if "agent_name" not in agent:
                logger.error(
                    "Error: 'agent_name' key is missing in agents."
                )
                return "Error: 'agent_name' key is missing in agents."

            if "system_prompt" not in agent:
                logger.error(
                    "Error: 'system_prompt' key is missing in agents."
                )
                return (
                    "Error: 'system_prompt' key is missing in agents."
                )
        ...
        if (
            "normal_agent_outputs" not in parsed_data
            or not isinstance(
                parsed_data["normal_agent_outputs"], list
            )
        ):
            logger.error(
                "Error: 'normal_agent_outputs' key is missing or not a list."
            )
            return "Error: 'normal_agent_outputs' key is missing or not a list."

        for i, agent_output in enumerate(
            parsed_data["normal_agent_outputs"], start=3
        ):
            if not isinstance(agent_output, dict):
                logger.error(
                    f"Error: agent output at index {i} is not a dictionary."
        ...
        logger.error(
            f"Error during parsing MixtureOfAgents output: {e}",
            exc_info=True,
        )
        return f"Error during parsing json.JSONDecodeError : {e}"
...
def parse_sequential_workflow_output(
    data: Optional[str], error_display=None
) -> str:
    ...
    if data is None:
        logger.error(
            "No data provided for parsing SequentialWorkflow output."
        )
        return "Error: No data provided for parsing."
    ...
    try:
        ...
        logger.error(
            f"Error during parsing SequentialWorkflow output: {e}",
            exc_info=True,
        )
        return f"Error during parsing json.JSONDecodeError : {e}"
...
def parse_spreadsheet_swarm_output(
    file_path: str, error_display=None
) -> str:
    """Parses the SpreadSheetSwarm output CSV file and formats it for display."""
    logger.info("Parsing SpreadSheetSwarm output...")
    if not file_path:
        logger.error(
            "No file path provided for parsing SpreadSheetSwarm output."
        )
    ...
    try:
        ...
        logger.error(
            f"Error during parsing SpreadSheetSwarm output: {e}",
            exc_info=True,
        )
...
def parse_json_output(data: str, error_display=None) -> str:
    ...
    try:
        ...
        logger.error(
            f"Error during parsing JSON output: {e}", exc_info=True
        )
...
def create_app():
    ...
    for provider in providers:
        filtered_models[provider] = models_by_provider.get(
            provider, []
        )
        ...
        with gr.Row():
            with gr.Column(scale=4):  # Left column (80% width)
                        ...
                        with gr.Column(scale=1):
                            ...
                            with gr.Accordion(
                                ...
                                with gr.Row():
                                    ...
                                    with gr.Column():
                                        new_agent_prompt_input = gr.Textbox(
                ...
                async def run_task_wrapper(
                    ...
                    try:
                        # Update status
                        yield "Processing...", "Running task...", "", gr.update(
                            visible=False
                        ), gr.update(
                            visible=False
                        )
                        ...
                        agents = initialize_agents(
                            ...
                            api_keys.get(
                                provider
                            ),  # Access API key from the dictionary
                            temperature,
                            max_tokens,
                        )
                        print(
                        ...
                        async for (
                            ...
                            if error:
                                yield f"Error: {error}", f"Error: {error}", "", gr.update(
                                    visible=True
                                ), gr.update(
                                    visible=True
                                )
                            ...
                            if result is not None:
                                formatted_output = format_output(
                                    result, swarm_type, error_display
                                )
                                yield formatted_output, "Completed", api_key, gr.update(
                                    visible=False
                                ), gr.update(
                                    visible=False
                                )
                        ...
                        yield f"Error: {str(e)}", f"Error: {str(e)}", "", gr.update(
                            visible=True
                        ), gr.update(
                            visible=True
                        )
