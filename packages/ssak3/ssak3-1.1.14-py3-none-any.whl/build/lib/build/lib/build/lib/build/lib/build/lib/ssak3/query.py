...
# --------------------------------------------------------------------------------------------------------------
            ...
            # cf = get_config_path(env_ctx['domain'])
            # q = await query.getkv(cf, env_ctx['domain'], env_ctx['name'], query.QUERY)
...
async def getkv(_config, _domain, _name, _opt):
    ...
    print(f'jojox : {query_domain}{SX_EXT_YAML}')


    # 없으면..
    if not begintrx:
        print(f'jo1')
        return get(_domain, _name, _opt)
