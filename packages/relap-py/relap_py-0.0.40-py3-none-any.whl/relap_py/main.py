...
def plot_variables(variable: list, variables_df, cases_df, ylabel=''):
    ...
    if type(cases_df) == dict :
        message('Remember that cases_df should be an Ndarray containing the data one the particular case: cases_df["base"]')
        return
    variables = []
    for var in variable:
    ...
    if len(variables) == 0:
        message('no variables to be plotted')
        return
    print(67*'-')
    print('making plot for', variables[0] + '_' + variables[-1])
    ...
    print("plot has been saved as:", 
        figures_loc() + variables[0] +'_'+ variables[-1] +'.'+ figures_type())
    print(67*'-')
    if show_plot() : plt.show()
