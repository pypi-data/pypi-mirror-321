...
evaluation_case = client.evaluations.retrieve(
    "REPLACE_ME",
)
...
async def main() -> None:
    evaluation_case = await client.evaluations.retrieve(
        "REPLACE_ME",
    )
    print(evaluation_case.data)
...
client.with_options(max_retries=5).evaluations.retrieve(
    "REPLACE_ME",
)
```
...
client.with_options(timeout=5.0).evaluations.retrieve(
    "REPLACE_ME",
)
```
...
evaluation = response.parse()  # get the object that `evaluations.retrieve()` would have returned
