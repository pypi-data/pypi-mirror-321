...
def _parse_kml_to_geodataframe(kml_file: Path) -> GeoDataFrame:
    kml_obj = KML.parse(kml_file, strict=False)
    ...
    for feature in kml_obj.features:
        for placemark in feature.features:
            if hasattr(placemark, "geometry"):
                ...
                if len(placemark.styles) > 0:
                    for style in placemark.styles[0].styles:
                        color = style.color or color

                colors.append(f"#{color}")
                names.append(placemark.name)

    gdf = GeoDataFrame({"geometry": geoms, "color": colors, "name": names})
    gdf = gdf.explode()
...
def kml_to_geotiff(kml_file: Path, output_file: Path):
    """
    ...
    _geodataframe_to_geotiff(gdf, output_file)
