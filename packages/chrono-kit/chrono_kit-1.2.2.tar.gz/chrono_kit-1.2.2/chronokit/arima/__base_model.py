...
class ARIMAProcess(TraditionalTimeSeriesModel):
    ...
    def __init__(self, data, initialization_method, **kwargs):
        ...
        if initialization_method != "known":
            self.initializer = SARIMAInitializer(self, initialization_method)
    ...
    def calc_iv_weights(self, phi, theta):
        ...
        for i in range(3*(len(phi) + len(theta))):
            p = 0
            ...
            if i < len(phi):
                p += phi[i]
            if abs(p) < 1e-6 and i >= len(phi) + len(theta):
                break
            pi.append(p)
    ...
    def invertibility_condition(self, ma_weights):
        ...
        #TODO: Why are roots are empty
        try:
            if min(abs(poly_roots)) < 1:
                return False
        except: #Roots are empty
            return True
    ...
    def check_kwargs(self, kwargs: dict):
        ...
        for k,v in kwargs.items():
            #Implement this exception "better"
            if k == "verbose":
                valid_kwargs[k] = v
                continue
            try:
