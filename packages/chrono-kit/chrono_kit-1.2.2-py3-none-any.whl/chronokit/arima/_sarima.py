...
class SARIMA(ARIMAProcess):
    def __init__(
        ...
        ** seasonal_theta (array_like): Parameters for Seasonal MA component;
            must be of length Q
        ** verbose (bool): Control verbosity during parameter estimation. Default False.
    ...
    def fit(self, verbose=False):
        ...
        if self.initializer is not None:
            init_params = self.initializer.initialize_parameters()
        ...
        #self.parameters = np.concatenate((v.numpy() for v in self.params.values()))

        self.cov_matrix = np.linalg.pinv(self.hessian)
        ##TODO: Why does sometimes (very rare, but important) 
        # diagonal of cov_matrix is negative
        standard_errors = np.sqrt(np.diag(np.abs(self.cov_matrix)))
        self.z_scores = self.model_parameters / standard_errors
        self.p_values = 2*(1 - norm.cdf(abs(self.z_scores)))
