...
class Grid(GridCore, Interpolation, Prediction, Focal, Zonal):
    ...
    def density(self, *points: Tuple[float, float]):
        if not points:
            points = tuple((p.x, p.y) for p in self.to_points() if p.value)
        return density(self.extent, self.crs, self.cell_size, *points)
    ...
    def interp_idw(
        ...
        if not points:
            points = tuple(p for p in self.to_points() if p.value)
        return idw(self.extent, self.crs, cell_size or self.cell_size, *points)
    ...
    def value_at(self, x: float, y: float) -> float:
        r = int((x - self.xmin) / self.cell_size.x)
        c = int((self.ymax - y) / self.cell_size.y)
        if r < 0 or r >= self.width or c < 0 or c >= self.height:
            return float(np.nan)
        return float(self.data[c, r])
...
def _coords(grid: Grid):
    x, y = np.meshgrid(
        np.linspace(grid.xmin, grid.xmax, grid.width),
        np.linspace(grid.ymax, grid.ymin, grid.height),
    )
...
def density(
    ...
    *points: Tuple[float, float],
    ...
    if len(points) == 0:
        raise ValueError("Density function requires at least one point.")

    g = grid(np.nan, extent, crs, cell_size)
    kernel_density = KernelDensity()
    kernel_density.fit(np.array(points))
    return g.local(
        np.exp(kernel_density.score_samples(_coords(g))).reshape(g.height, g.width)
    )
...
def distance(
    ...
    *points: Tuple[float, float],
    ...
    g = grid(np.nan, extent, crs, cell_size)
    distances = cdist(_coords(g), np.array(points))
    return g.local(distances.min(axis=1).reshape(g.height, g.width))
...
def idw(
    ...
    *points: Tuple[float, float, float],
    ...
    distances = np.sqrt((xi[:, None] - x) ** 2 + (yi[:, None] - y) ** 2)
    distances[distances == 0] = 1e-10
    weights = 1 / distances**2
    interpolated_values = np.dot(weights / weights.sum(axis=1, keepdims=True), values)
    return g.local(interpolated_values.reshape(g.height, g.width))
