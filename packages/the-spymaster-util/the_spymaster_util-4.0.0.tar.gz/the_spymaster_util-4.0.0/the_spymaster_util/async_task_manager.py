...
class AsyncTaskManager(ContextManager):
    ...
    def __init__(
        ...
        kill_on_complete: bool = True,
    ):
        super().__init__()
        self.name = name or "Task manager"
        self.collect_results = collect_results
        self.iter_timeout = iter_timeout
        self.kill_on_complete = kill_on_complete
    ...
    def __next__(self):
        if self.is_work_done and self.is_empty:
            if self.kill_on_complete:
                self.kill()
            raise StopIteration()
        return self.get_result(timeout=self.iter_timeout)

    def __enter__(self):
        self.kill_on_complete = False
        return self
    ...
    def add_task(self, func: Callable, args: Optional[Sequence] = None, kwargs: Optional[Mapping] = None):
        if self.is_closed:
            return
    ...
    def stop_workers(self, amount: Optional[int] = None):
        if amount is None:
