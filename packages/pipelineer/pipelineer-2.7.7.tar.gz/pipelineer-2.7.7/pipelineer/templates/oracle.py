                    ...
                    "query": {
                        "query": f"""
                            CALL `{bigquery_project_id}.monitoreo.sp_calcular_auditoria`(
                                '{{{{ task_instance.xcom_pull(task_ids="oracle_to_bq_{table_name}.merge_table_{table_name}", key="return_value") }}}}',
                                '{table_name}', 
                                '{tabla_destino}',
                                '{tabla_staging}'
                            )
