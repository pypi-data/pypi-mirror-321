...
def with_knex(func):
    def wrapper(*args, **kwargs):
        ...
        logger.info(f"Checking for knex in args: {args}, kwargs: {kwargs}")
        any_knex = any([isinstance(arg, Knex) for arg in args])
        if any_knex:
            logger.info("knex found in args args: {args}, kwargs: {kwargs}")
            return func(*args, **kwargs)
        else:
            logger.info("knex not found in args args: {args}, kwargs: {kwargs}")
            varnames = [
            ...
            if "knex" not in kwargs:
                logger.info("knex not found in kwargs args: {args}, kwargs: {kwargs}")
