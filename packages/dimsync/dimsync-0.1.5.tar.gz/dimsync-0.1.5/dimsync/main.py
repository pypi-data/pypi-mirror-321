...
class Docker(object):
    ...
    def pull(self, **kwargs):
        ...
        pull_layers = {}
        pbar = tqdm(total=100, unit='B', unit_scale=True, desc='Pulling', dynamic_ncols=True)
        ...
        data = self._image_info(images[0])
        if len(data) == 0:
    ...
    def push(self, **kwargs):
        ...
        push_layers = {}
        pbar = tqdm(total=100, unit='B', unit_scale=True, desc='Pushing', dynamic_ncols=True)
        for output in push_stream:
            ...
            complete = sum([1 for v in push_layers.values() if v.get('status') in ['Already exists', 'Pull complete']])
...
def _sync(args):
    ...
    try:
        ...
        for image in docker.images():
            ...
            docker.push(**{**mirror, 'debug': config.debug()})
            docker.rmi(**mirror)
            hist = hist or {}
