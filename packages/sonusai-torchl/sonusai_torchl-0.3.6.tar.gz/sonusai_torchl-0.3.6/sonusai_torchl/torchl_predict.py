        ...
        for ndi in range(num_dev):  # iterate over list devices (num of batch groups)
            ...
            for bi in range(num_batches):  # iterate over list of batches per dev
                ...
                for di in range(bsz):
                    ...
                    output_name = join(
                        trainer.default_root_dir, trainer.predict_dataloaders.dataset.mixdb.mixtures[gid].name + ".h5"
...
def main() -> None:
    ...
    if mixdb is not None:  # mixdb input, already loaded
        ...
        if wavdbg:
            ...
            for idx, key in enumerate(mixdb.truth_configs):
                if mixdb.truth_configs[key].function == "target_f":
                    logger.info("Found target_f truth type, will write target audio files.")
                    targetf_key = key
