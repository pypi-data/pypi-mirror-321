...
class GpioCommandCodes(Enum):
    ...
    """
    GPIO_CONFIGURE_PIN              = makeCommandCode(GROUP_GPIO, CommandRole.GENERIC.value, CommandType.REQUEST_RESPONSE.value, 1)
    GPIO_DIGITAL_WRITE              = makeCommandCode(GROUP_GPIO, CommandRole.GENERIC.value, CommandType.REQUEST_RESPONSE.value, 2)
    GPIO_DIGITAL_READ               = makeCommandCode(GROUP_GPIO, CommandRole.GENERIC.value, CommandType.REQUEST_RESPONSE.value, 3)
    GPIO_SET_INTERRUPT              = makeCommandCode(GROUP_GPIO, CommandRole.GENERIC.value, CommandType.REQUEST_RESPONSE.value, 4)
    GPIO_DISABLE_INTERRUPT          = makeCommandCode(GROUP_GPIO, CommandRole.GENERIC.value, CommandType.REQUEST_RESPONSE.value, 5)
    GPIO_INTERRUPT_NOTIFICATION     = makeCommandCode(GROUP_GPIO, CommandRole.GENERIC.value, CommandType.NOTIFICATION.value, 1)
...
class GpioResultCodes(Enum):
    ...
    """
    GPIO_NOT_CONFIGURED         = makeResultCode(GROUP_GPIO, 1)
    GPIO_WRONG_CONFIGURATION    = makeResultCode(GROUP_GPIO, 2)
