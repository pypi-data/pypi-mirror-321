...
def _handle_query_job_result(query_job: Union[bigquery.LoadJob, bigquery.QueryJob, bigquery.CopyJob, bigquery.ExtractJob],
    ...
    log_by_lvl(
        ...
        debug_msg=result.get_status_report(),
        logger=logger
    )
    if pipelinemon:
        pipelinemon.add_log(StructLog(
            ...
            description=result.get_status_report()
        ))
    ...
    try:
        ...
        if table_exists:
            if replace_if_exists:
                ...
                log_by_lvl(debug_msg=result.get_status_report(), info_msg=msg, logger=logger, print_out=print_out)
...
def merge_into_bigquery_table_extended(
    ...
    try:
        ...
        if not create_result.is_success:
            ...
            result.add_issue(f"Failed to create temp table: {create_result.get_status_report()}")
            result.final()
            return result
        ...
        if temp_table_name:
            try:
                ...
                if pipelinemon:
                    ...
                    pipelinemon.add_log(StructLog(level=LogLevel.INFO, action=Action.PERSIST_DELETE_TABLE_TEMP, resource=DataResource.DB_BIGQUERY, progress_status=ProgressStatus.DONE, description=result.get_status_report()))
                ...
                if pipelinemon:
                    pipelinemon.add_log(StructLog(level=LogLevel.WARNING, action=Action.PERSIST_DELETE_TABLE_TEMP, resource=DataResource.DB_BIGQUERY, progress_status=ProgressStatus.FAILED, description=result.get_status_report()))
        result.final()
    return result
