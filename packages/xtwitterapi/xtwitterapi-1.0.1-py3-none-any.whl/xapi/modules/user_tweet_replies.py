...
class UserTweetsAndReplies(BaseXModule):
    def __init__(self, session: AsyncSession) -> None:
        super().__init__(session)
        self.endpoint = "https://x.com/i/api/graphql/bt4TKuFz4T7Ckk-VvQVSow/UserTweetsAndReplies"
        self.method = "GET"
        self.action_type = ActionType.USER_TWEETS_AND_REPLIES
    ...
    async def _format_query(self, **kwargs: dict[str, Any]) -> QueryItem:
        ...
        querystring = {
            "variables": f'{{"userId":"{user_id}","count":20,"cursor":"{cursor}",'
            '"includePromotedContent":true,"withCommunity":true,"withVoice":true,"withV2Timeline":true}',
