...
def categorize_dataframes_by_type(df, table_info_gpt):
    ...
    str_types = ["nvarchar", "varchar", "text", "string", "char", "str", "object", "object_", "varchar2"]
    numeric_types = ["int", "float", "decimal", "double", "numeric", "number", "float64", "int64"]
    date_types = ["datetime", "date", "timestamp", "datetime64", "timedelta", "datetime64[ns]", "datetime(time_unit='ns', time_zone=none)", "Datetime(time_unit='ns', time_zone='UTC')", "timestamp(6) with time zone"]
    
    # Validar tipos string
    for row in range(len(table_info_gpt_str_df)):
        if table_info_gpt_str_df['datatype'][row].lower() not in str_types:
    ...
    for row in range(len(table_info_gpt_int_df)):
        if table_info_gpt_int_df['datatype'][row].lower() not in numeric_types:
    ...
    for row in range(len(table_info_gpt_date_df)):
        if table_info_gpt_date_df['datatype'][row].lower() not in date_types:
