...
class PPL_Convertion_Tool:
    ...
    def gen_target_file(self):
        ...
        cmd = [
            ...
            self.opt_level, "--o {}".format(self.target_path)
        ]
    ...
    def gen_test_file(self):
        ...
        cmd = [
            ...
            "--chip {}".format(self.chip), self.opt_level,
    ...
    def compare(self):
        ...
        assert (not len(files) % 2)
        mem_num = len(files) // 2
        inp_data = {}
        ref_data = {}
        tpu_data = {}
        for i in range(mem_num):
            inp_data_file = os.path.join(
                self.data_path, self.out_name + "_fp32_" + str(i) + ".in")
            ref_data_file = os.path.join(
                self.data_path, self.out_name + "_ref_" + str(i) + ".out")
            tar_data_file = os.path.join(
                self.data_path, self.out_name + "_tar_" + str(i) + ".out")
            assert (os.path.exists(inp_data_file))
            assert (os.path.exists(ref_data_file))
            assert (os.path.exists(tar_data_file))
            inp_data[str(i)] = np.fromfile(inp_data_file, dtype=np.float32)
            ref_data[str(i)] = np.fromfile(ref_data_file, dtype=np.float32)
        ...
        np.savez(inp_npz, **inp_data)
        np.savez(ref_npz, **ref_data)
