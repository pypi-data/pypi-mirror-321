...
from ..runtime.jit import (JITFunction, version_key, data_path)
...
def validate(path, arch, function_name, mode, only_emit_kernel, debug, desc=False):
    ...
    if mode == "cmodel":
        if arch == "bm1684x" or arch == "bm1690" or arch == "bm1688" or arch == "sg2262":
            if arch == "bm1684x":
...
def compile(fn, **kwargs):
    ...
    if only_emit_kernel:
         key = f"{fn.__name__}-{fn.src}-{''.join(signature.values())}-{constants}-{debug}-{mode}-{arch}-{grid_size}-{grid_0}-{grid_1}-{grid_2}"
    else:
        #if use different scalar, trigger to generate again
        key = f"{fn.__name__}-{fn.src}-{''.join(signature.values())}-{scalar_value}-{constants}-{debug}-{mode}-{arch}-{grid_size}-{grid_0}-{grid_1}-{grid_2}"
