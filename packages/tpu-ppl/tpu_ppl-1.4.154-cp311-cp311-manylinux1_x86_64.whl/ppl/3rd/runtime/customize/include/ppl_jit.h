  ...
  int jitCompile(const std::string &file_name, const std::string &func_name,
    ...
    if (!v && !state.count(key)) {
      ...
      if (ret == 0) {
        ...
        generateFlag(key, false);
      return ret;
    }
    return 0;
  }

  bool isCompiled(std::size_t key) const {
    return state.count(key) && state.at(key);
  }
...
private:
  ...
  std::map<std::size_t, std::size_t> state;
  ...
  int runJitCompile(const std::string &file_name, const std::string &func_name,
    ...
    // << " > /dev/null 2>&1";
    std::string output;
    int ret = execCompileCommand(cmd.str(), output);
    ret >>= 8;
    if (ret) {
      fs::remove_all(path);
