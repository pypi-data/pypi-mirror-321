...
class gigaspot_connection:
    ...
    def generate_headers(self, base={}):
        if type(base) != dict:
            base = {}
        base["Content-Type"] = "NXj2bHUXHwzvd5-Lm6UfvgGtnNwaHxLu"
                                    ...
                                    async with aiohttp.ClientSession() as cdn_sesion:
                                        async with cdn_sesion.get(
                                            ...
                                            if cdn_res.status == 200:
                                                ...
                                                return gigaspot_snapshot(False, self.last_v1_snapshot, extrct_gs_shapshot_time(self.last_v1_snapshot_url), self.my_user_id)
                                else:
                                    return gigaspot_snapshot(False, self.last_v1_snapshot, extrct_gs_shapshot_time(self.last_v1_snapshot_url), self.my_user_id)
                        clore_res.raise_for_status()
                        return gigaspot_snapshot(True, {}, None, None)
                    ...
                    async with session.post(
                        ...
                        if clore_res.status == 429:
                            ...
                            if "failed_to_oc_servers" in data:
                                failed_to_rent =list(set(data["failed_to_oc_servers"] + data["failed_to_rent"]))
                                rented_ids = []
                                for order in orders:
    ...
    async def edit_orders(self, changes: List[dict], retries=3):
        ...
        if not all(isinstance(item, int) for item in changes):
        ...
        for attempt in range(retries):
            ...
            if attempt + 1 < retries:
                ...
                await asyncio.sleep(delay)
        return edit_result(True, None, None)
