...
class RequestMessageChat(NamedTuple):
    """
    Represents the data payload for a C{CHAT} channel C{MESSAGE} request.
    """

    sender: str
    """The sender of the C{CHAT} channel message."""

    receiver: str
    """The receiver of the C{CHAT} channel message."""

    text: str
    """The text content of the C{CHAT} channel message."""

RequestMessage = Union[RequestMessageSlack, RequestMessageEmail, RequestMessageSms, RequestMessageRest, RequestMessageChat]
"""Request message alias."""


class RequestDataMessage(NamedTuple):
    """
    Represents the data payload for sending a C{MESSAGE} request: C{slack}, C{email}, C{SMS}, C{rest} or C{chat}.
    """
...
class ResponseMessageChat(NamedTuple):
    """
    Represents the data payload for the response to a C{CHAT} channel C{MESSAGE} request.
    """
    ...
    def make(
        ...
        """Factory method to create a `ResponseDataChat` object."""
        return cls(sender, receiver, text)



ResponseMessage = Union[ResponseMessageSlack, ResponseMessageEmail, ResponseMessageSms, ResponseMessageRest, ResponseMessageChat]
"""Response message alias."""


class ResponseDataMessage(NamedTuple):
    """
    Represents the data payload for the response to a C{MESSAGE} request (C{slack}, C{email}, C{SMS}, C{rest} or C{chat}).
    """
...
def _deserialize_message(src: Any, channel_type: ChannelType, skip_unknown_fields: bool) -> RequestMessage:
    match channel_type:
        ...
        case ChannelType.CHAT:
            _check_keys('chat_message_data', src, skip_unknown_fields, {'sender', 'receiver', 'text'})
            return _deserialize_tuple(RequestMessageChat, src, skip_unknown_fields)
