...
from core.types import RestoreConfig, StepResult
from core.vss import VSSManager
from utils.compute import ComputeClient
from utils.ui import UIManager
from utils.winrm import WinRMManager

class SnapManager:
    """Manages VSS restore operations."""
    ...
    def __init__(self, config: RestoreConfig):
        ...
        self.disk_name = f"restored-disk-{suffix}"
        self.temp_vm_name = f"vss-temp-{suffix}"
        self.final_vm_name = f"vss-restored-{suffix}"
        
        # Define restore stages
        self.stages = {
    ...
    def _make_bootable(self) -> StepResult:
        ...
        try:
            ...
            self.vss.winrm = WinRMManager(external_ip, ui_manager=self.ui)
    ...
    def _create_restored_vm(self) -> StepResult:
        ...
        try:
            # Create the restored VM using the bootable disk
    ...
    def run_stage(self, stage_id: str, stage: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            # Show stage before running steps
            if self.ui:
