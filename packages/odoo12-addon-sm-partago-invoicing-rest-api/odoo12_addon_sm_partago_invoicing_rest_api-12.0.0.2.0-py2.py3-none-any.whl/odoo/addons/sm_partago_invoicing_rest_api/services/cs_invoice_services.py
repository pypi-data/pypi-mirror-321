...
class CsInvoiceService(Component):
    ...
    def create(self, **params):
        ...
        public_url, pdf_url = self._generate_invoice_urls(invoice)
        return Response(
            json.dumps({
    ...
    def _generate_invoice_urls(self, invoice):
        ...
        public_url = base_url + f"?model=account.invoice&res_id={invoice.id}&access_token={invoice.access_token}"
        pdf_url = base_url + f"?access_token={invoice.access_token}&report_type=pdf&download=true"
        return public_url, pdf_url
