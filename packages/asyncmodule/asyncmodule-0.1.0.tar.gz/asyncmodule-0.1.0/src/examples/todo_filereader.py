...
class FileReader(module.Module):
    ...
    def get_hostname(self):
        """Return the hostname (without domain) of the machine the script is running on"""
    ...
    def prepare_shutdown1(self):
        super().prepare_shutdown1()  # set state to "becoming inactive" to that we know we're shutting down
        logger.debug('Stopping child thread')

    def prepare_shutdown2(self):
        logger.debug('Waiting for child thread to terminate...')
        self._thread.join()
        logger.debug('Child thread ended')
        self.deactivate()
        super().prepare_shutdown2()
    ...
    def exec_task(self, task, **args):
        ...
        return self._function_references['exec_task_withlock'](task, **args)
