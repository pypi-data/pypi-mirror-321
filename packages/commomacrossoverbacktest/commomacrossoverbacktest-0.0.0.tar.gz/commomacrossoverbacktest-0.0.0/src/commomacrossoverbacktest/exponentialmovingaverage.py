    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for ticker, group in grouped:
            ...
            for i in range(1, len(group)):
                # Buy signal: EMA_Short crosses above both EMA_Medium and EMA_Long
                if (
                ...
                # Sell signal: EMA_Short crosses below both EMA_Medium and EMA_Long
                elif (
                    group['EMA_Short'].iloc[i] < group['EMA_Medium'].iloc[i] < group['EMA_Long'].iloc[i] and
                    group['EMA_Short'].iloc[i - 1] >= group['EMA_Medium'].iloc[i - 1] and
                    position != 'Short'
        ...
        # Add column 'Position' (Buy ou Sell)
