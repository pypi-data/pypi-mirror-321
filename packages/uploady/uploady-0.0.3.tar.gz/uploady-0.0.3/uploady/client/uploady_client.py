...
from uploady.utils import grouper, async_to_sync, get_environment_integer
        ...
        async with helpers._FileStream(file, file_size=file_size) as stream:
            ...
            for part_index in range(part_count):
                ...
                self.loop.create_task(
                    ...
                    name=f"uploady-file-{part_index}"
                )
            # Wait for all tasks to finish
            await asyncio.wait([
                task for task in asyncio.all_tasks() if task.get_name().startswith(f"uploady-file-")
            ])
        if is_big:
