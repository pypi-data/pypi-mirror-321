...
from uploady.client.telegram_manager_client import USER_MAX_FILE_SIZE
from uploady.exceptions import TelegramInvalidFile
from uploady.upload_files import get_file_attributes, RecursiveFiles, NoDirectoriesFiles, NoLargeFiles, \
...
class TestGetFileAttributes(unittest.TestCase):
    ...
    @patch('uploady.upload_files.video_metadata')
...
class TestRecursiveFiles(unittest.TestCase):
    @patch('uploady.upload_files.scantree', return_value=[])
    @patch('uploady.upload_files.os.path.isdir', return_value=False)
    def test_one_file(self, m1, m2):
        self.assertEqual(list(RecursiveFiles(MagicMock(), ['foo'])), ['foo'])

    @patch('uploady.upload_files.scantree')
    @patch('uploady.upload_files.os.path.isdir', return_value=True)
...
class TestNoLargeFiles(unittest.TestCase):
    @patch('uploady.upload_files.os.path.getsize', return_value=USER_MAX_FILE_SIZE - 1)
    @patch('uploady.upload_files.File')
...
class TestSplitFiles(unittest.TestCase):
    ...
    @patch('uploady.upload_files.os.path.getsize', return_value=USER_MAX_FILE_SIZE + 1000)
    @patch('uploady.upload_files.SplitFile.__init__', return_value=None)
    @patch('uploady.upload_files.SplitFile.seek')
