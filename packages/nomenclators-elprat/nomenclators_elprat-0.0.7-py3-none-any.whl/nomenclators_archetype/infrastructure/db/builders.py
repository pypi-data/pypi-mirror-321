...
class QueryBuilderImpl(QueryBuilder):
    ...
    def apply_filter_query(self, query):
        ...
        super().apply_filter_query(query)
    ...
    def apply_group_query(self, _query=None):
        ...
        if self._session is not None:
            ...
            if self._grouping_by_id_name is None:
                raise ValueError(
                    f"Identifier name should defined for {self._model.__name__}.")
        ...
        if self._pageable.sort:
            for (sort_field_name, sort_orientation) in self._pageable.sort:
                ...
                try:
                    ...
                    if column is None:
                        raise ValueError(
                            f"For persistence class {self._model.__name__}, invalid column name: {sort_field_name}")
                    ...
                    raise ValueError(
                        f"For persistence class {self._model.__name__}, error accessing attribute '{sort_field_name}'") from e

        if self._pageable.pagination:
            query = query.limit(self._pageable.size).offset(
                (self._pageable.page - 1) * self._pageable.size)
