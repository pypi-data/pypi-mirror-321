...
from trl import DPOConfig, FPOTrainer, is_peft_available
...
@require_torch_gpu
class FPOTrainerSlowTester(unittest.TestCase):
    def setUp(self):
    ...
    def test_dpo_bare_model(self, model_id, loss_type, pre_compute_logits):
        """
        A test that tests the simple usage of `FPOTrainer` using a bare model in full precision.
        """
    ...
    def test_dpo_peft_model(self, model_id, loss_type, pre_compute_logits, gradient_checkpointing_kwargs):
        """
        A test that tests the simple usage of `FPOTrainer` using a peft model in full precision + different scenarios of gradient checkpointing.
        """
        model = AutoModelForCausalLM.from_pretrained(model_id)
        tokenizer = AutoTokenizer.from_pretrained(model_id)
    ...
    def test_dpo_peft_model_qlora(self, model_id, loss_type, pre_compute_logits, gradient_checkpointing_kwargs):
        """
        A test that tests the simple usage of `FPOTrainer` using QLoRA + different scenarios of gradient checkpointing.
        """
