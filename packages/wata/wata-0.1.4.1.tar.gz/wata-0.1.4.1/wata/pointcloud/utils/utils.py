...
def save_pcd_from_structured_points(structured_points, save_path, type='binary_compressed'):
    points = structured_array_to_array(structured_points)
    fields = []
    npdtype = []
    for line in list(structured_points.dtype.descr):
        fields.append(line[0])
        npdtype.append(( pcd_type_to_numpy_type((line[1][1].upper(), int(line[1][2]))) ))
