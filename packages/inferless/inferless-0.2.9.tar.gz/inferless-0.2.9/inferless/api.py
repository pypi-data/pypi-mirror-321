...
class InferlessOpenAIClient:
    ...
    def call_infer(self, data: dict, callback: Optional[Any] = None):
        """
        Function to call openai format Inferless API

        :param data: Model Input Data as a dictionary
        :param callback: Callback function to be called after the response is received
        """
        try:
            json_stringfied_data = json.dumps(data)
            input_data = {
                "inputs": [
                    {
                        "name": "message",
            ...
            response = requests.post(self.base_url, json=input_data, headers=self.headers)
            if response.status_code != 200:
                raise Exception(
