...
def test_main_delete_token(monkeypatch):
    ...
    with patch("src.audio_scribe.transcriber.TokenManager") as mock_tm, \
         patch("src.audio_scribe.transcriber.DependencyManager") as mock_dm:
...
def test_main_no_token(monkeypatch):
    ...
    with patch("src.audio_scribe.transcriber.DependencyManager") as mock_dm, \
         patch("src.audio_scribe.transcriber.get_token") as mock_get_token:
...
def test_main_model_init_failure(monkeypatch):
    ...
    with patch("src.audio_scribe.transcriber.DependencyManager") as mock_dm, \
         patch("src.audio_scribe.transcriber.get_token") as mock_get_token, \
         patch("src.audio_scribe.transcriber.TranscriptionPipeline") as mock_pipeline:
...
def test_main_interactive_audio_input(monkeypatch):
    ...
    with patch("src.audio_scribe.transcriber.DependencyManager") as mock_dm, \
        ...
        pipeline_instance.process_file.assert_called_once_with(Path("/path/to/audio.wav"))

def test_main_show_warnings(monkeypatch):
    """Test --show-warnings flag behavior."""
    with patch("src.audio_scribe.transcriber.warnings") as mock_warnings, \
         patch("src.audio_scribe.transcriber.DependencyManager") as mock_dm:
