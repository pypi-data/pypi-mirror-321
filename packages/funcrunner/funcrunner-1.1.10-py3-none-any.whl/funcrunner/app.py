...
class FuncRunnerApp:
    ...
    def _fetch_run_data(self, message: Message):
        url = f"{self.proxy_host}/v1/threads/{message.body.thread_id}/runs/{message.body.run_id}"
        self.logger.debug(f"Fetching run data from URL: {url}", message_id=message.id, run_id=message.body.run_id,
    ...
    def _process_queue_message(self, message: Message) -> Optional[RunResult]:
        ...
        if not tool_calls:
            self.logger.info(f"No tool calls found for run", run_id=message.body.run_id,
                             correlation_id=message.correlation_id)

        function_executions = self._process_tool_calls(tool_calls)

        result = RunResult(run_id=message.body.run_id, thread_id=message.body.thread_id)
