...
async def common_parameters():
    ...
    logger.info(f'CUSTOMER_TENANTS_JSON: {customer_tenants_json}')

    if not customer_tenants_json:
        logger.error("Error: CUSTOMER_TENANTS_JSON environment variable is not set.")
        exit(1)

    # Strip surrounding quotes if present
    if customer_tenants_json.startswith('"') and customer_tenants_json.endswith('"'):
        customer_tenants_json = customer_tenants_json[1:-1]
        logger.info(f'Stripped quotes: {customer_tenants_json}')
