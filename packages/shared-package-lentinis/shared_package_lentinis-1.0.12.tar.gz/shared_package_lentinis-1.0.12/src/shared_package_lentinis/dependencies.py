...
async def common_parameters():
    ...
    try:
        ...
        sys.exit(1)
    # customer_tenants= await parse_customer_tenants()
    # Extract tenant_ids from customer_tenants
    # tenant_ids = [tenant['tenantId'] for tenant in customer_tenants if 'tenantId' in tenant]
...
async def parse_customer_tenants():
    # Retrieve the environment variable
    customer_tenants_env = os.getenv('CUSTOMER_TENANTS_JSON')
    
    if not customer_tenants_env:
        logger.error("Error: CUSTOMER_TENANTS_JSON environment variable is not set.")
        return []
    
    logger.info(f'CUSTOMER_TENANTS_JSON (raw): {customer_tenants_env}')
    
    # Check for surrounding quotes and strip them
    if (customer_tenants_env.startswith('"') and customer_tenants_env.endswith('"')) or \
       (customer_tenants_env.startswith("'") and customer_tenants_env.endswith("'")):
        customer_tenants_env = customer_tenants_env[1:-1]
        logger.info(f'Stripped surrounding quotes: {customer_tenants_env}')
    
    # Replace escaped double quotes with actual double quotes
    customer_tenants_env = customer_tenants_env.replace('\\"', '"').replace("\\'", "'")
    logger.info(f'Processed CUSTOMER_TENANTS_JSON: {customer_tenants_env}')
    
    # Parse JSON
    try:
        customer_tenants = json.loads(customer_tenants_env)
        if not isinstance(customer_tenants, list):
            raise ValueError("CUSTOMER_TENANTS_JSON is not a list.")
        logger.info(f'Parsed CUSTOMER_TENANTS_JSON successfully: {customer_tenants}')
        return customer_tenants
    except (json.JSONDecodeError, ValueError) as error:
        logger.error(f'Failed to parse CUSTOMER_TENANTS_JSON: {error}')
        return []
...
#             logger.error("Error: No valid tenant IDs found in CUSTOMER_TENANTS_JSON.")
#         else:
#             logger.info(f'Extracted tenant_ids: {tenant_ids}')
#             print("Extracted Tenant IDs:", tenant_ids)
