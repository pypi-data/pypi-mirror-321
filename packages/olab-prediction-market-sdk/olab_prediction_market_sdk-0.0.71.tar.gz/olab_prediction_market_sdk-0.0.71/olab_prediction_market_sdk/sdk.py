    ...
    def place_order(self, data: PlaceOrderDataInput, exchange_addr='', chain_id=8453, currency_addr='', check_approval=False):
        ...
        if(data.side == BUY):
            ...
            if(data.makerAmountInBaseToken):
                makerAmount = data.makerAmountInBaseToken * data.price * currency['decimal']
            # e.g. usdc
            if(data.makerAmountInQuoteToken):
                makerAmount = data.makerAmountInQuoteToken * currency['decimal']
        if(data.side == SELL):
            # e.g. yes/no
            if(data.makerAmountInBaseToken):
                makerAmount = data.makerAmountInBaseToken * currency['decimal']
            # e.g. usdc
            if(data.makerAmountInQuoteToken):
                makerAmount = data.makerAmountInQuoteToken / data.price * currency['decimal']
