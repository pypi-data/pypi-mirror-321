...
from blissdata.redis_engine.exceptions import EndOfStream, IndexNoMoreThereError


def event_stream_pair(data_store, name, dtype, shape=()):
    recipe = Stream.recipe(name, dtype, shape)
    model = data_store._stream_model(encoding=recipe.encoder.info(), info=recipe.info)
    event_rw_stream = EventStream.create(data_store, recipe.name, model)
    event_ro_stream = EventStream.open(data_store, recipe.name, model)
...
def test_cursor_batching(data_store, last_only, batches, seal, block, group):
    recipe = Stream.recipe("my_stream", np.int64)
    model = data_store._stream_model(encoding=recipe.encoder.info(), info=recipe.info)
    stream = Stream(EventStream.create(data_store, recipe.name, model))
