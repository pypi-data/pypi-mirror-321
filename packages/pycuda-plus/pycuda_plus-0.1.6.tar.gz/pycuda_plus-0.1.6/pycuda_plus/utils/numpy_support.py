...
class NumpyHelper:
    ...
    def elementwise_operation(self, device_array1, device_array2, operation):
        """
        Perform an element-wise operation on two device arrays.

        Args:
            device_array1 (pycuda.gpuarray.GPUArray): First device array.
            device_array2 (pycuda.gpuarray.GPUArray): Second device array.
            operation (str): Operation to perform ('add', 'sub', 'mul', 'div').

        Returns:
            pycuda.gpuarray.GPUArray: Result of the element-wise operation.
        """
        if operation == 'add':
            return device_array1 + device_array2
        elif operation == 'sub':
            return device_array1 - device_array2
        elif operation == 'mul':
            return device_array1 * device_array2
