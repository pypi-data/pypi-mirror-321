...
Example:
    ...
    def _build_query(self, tag_str: str, **kwargs) -> str:
        ...
        (
          node{tag_str}{self._format_area_filter(**kwargs)};
          way{tag_str}{self._format_area_filter(**kwargs)};
          relation{tag_str}{self._format_area_filter(**kwargs)};
        );
        out body;
        >;
        out skel qt;
    ...
    def _format_area_filter(self, **kwargs) -> str:
        """Format area filter based on search mode"""
        if self.mode == 'radius':
            return f"(around:{kwargs['radius']},{kwargs['lat']},{kwargs['lon']})"
        elif self.mode == 'bbox':
            return f"({kwargs['min_lat']},{kwargs['min_lon']},{kwargs['max_lat']},{kwargs['max_lon']})"
        return ""
    ...
    def search(self, 
        ...
        try:
            with tqdm(total=100, disable=not self.show_progress) as pbar:
                ...
                pbar.update(50)
                found_elements = self._process_results(result)
                pbar.update(50)
                return found_elements
        except Exception as e:
