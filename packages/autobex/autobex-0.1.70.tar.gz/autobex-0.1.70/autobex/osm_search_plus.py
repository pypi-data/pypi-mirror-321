        ...
        try:
            for tag in self.tags:
                ...
                if '=' in tag:
                    ...
                    # Match tag as key with any value after =
                    tag_filters.append(f'["{tag_key}"~".*"]')
                    # Match tag as value for any key
                    tag_filters.append(f'[~".*"~"{tag_key}"]')
                    # Match tag as value after any key with colon
                    tag_filters.append(f'[~".*:"~"{tag_key}"]')

                if show_logs:
                    self.logger.debug(f"Tag filters: {tag_filters}")

                # Build the complete query with all tag filters
                node_queries = []
                way_queries = []
                relation_queries = []
                for tag_filter in tag_filters:
                    node_queries.append(f"node{tag_filter}{area_filter};")
                    way_queries.append(f"way{tag_filter}{area_filter};")
                    relation_queries.append(f"relation{tag_filter}{area_filter};")
