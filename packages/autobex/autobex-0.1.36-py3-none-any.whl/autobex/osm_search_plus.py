...
class OSMSearchPlus:
    ...
    def _build_query(self, tag_str: str, **kwargs) -> str:
        ...
        query = f"""
        [out:json][timeout:{self.api_timeout}][maxsize:2048];
        ("""
        ...
        if self.mode == 'radius':
            ...
            # Split bbox into larger chunks
            lat_span = abs(kwargs['max_lat'] - kwargs['min_lat'])
            lon_span = abs(kwargs['max_lon'] - kwargs['min_lon'])
            if lat_span * lon_span > 0.25:  # Area threshold
                mid_lat = (kwargs['min_lat'] + kwargs['max_lat']) / 2
                mid_lon = (kwargs['min_lon'] + kwargs['max_lon']) / 2
                query += f"""
                node[{tag_str}]({mid_lat-0.25},{mid_lon-0.25},{mid_lat+0.25},{mid_lon+0.25});
                way[{tag_str}]({mid_lat-0.25},{mid_lon-0.25},{mid_lat+0.25},{mid_lon+0.25});
                """
            else:
                query += f"""
...
Example:
    ...
    def search(self, 
        ...
        if self.show_logs:
            ...
            self.logger.info(f"Show logs: {self.show_logs}, Show progress: {self.show_progress}")
        ...
        if params.get('radius'):
            ...
            if self.show_logs:
                self.logger.info(f"Converted radius to meters: {params['radius']}")

        # Build tag string
        tag_str = ''.join(f'["{k}"="{v}"]' for k, v in self.tags.items())
        if self.show_logs:
            self.logger.info(f"Using tags: {self.tags}")
        ...
        try:
            with tqdm(total=100, disable=not self.show_progress) as pbar:
                ...
                tag_batches = [self.tags[i:i+3] for i in range(0, len(self.tags), 3)]
                total_batches = len(tag_batches)
                
                for batch_idx, tag_batch in enumerate(tag_batches):
                    if self.show_logs:
                        self.logger.info(f"Processing tag batch {batch_idx+1}/{total_batches}")
                    
                    # Build tag string for this batch
                    tag_str = ''.join(f'["{k}"="{v}"]' for k, v in tag_batch.items())
                    
                    try:
                        if self.mode == 'radius':
                            self._process_radius_search(params, tag_str, all_elements, pbar)
                        elif self.mode == 'bbox':
                            self._process_bbox_search(params, tag_str, all_elements, pbar)
                        elif self.mode == 'polygon':
                            self._process_polygon_search(params, tag_str, all_elements, pbar)
                            
                    except Exception as e:
                        if self.show_logs:
                            self.logger.warning(f"Batch {batch_idx+1} failed: {str(e)}")
                        continue
                    
                    pbar.update(100 // total_batches)
                
                # Remove duplicates
                if self.show_logs:
                    self.logger.info(f"Found {len(all_elements)} elements before deduplication")
                
                unique_elements = self._deduplicate_results(all_elements)
