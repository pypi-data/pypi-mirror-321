...
class OSMSearchPlus:
    def __init__(self, logger=None):
        ...
        self.usgs_api = 'https://nationalmap.gov/epqs/pqs.php'  # USGS Elevation Point Query Service also provides land ownership
        self.timeout = 60
    ...
    def _check_land_ownership(self, lat: float, lon: float) -> bool:
        """
        Check if a location is on public land using USGS API.
        Returns True if public, False if private.
        """
        try:
            params = {
                'x': lon,        # USGS uses longitude first
                'y': lat,        # Then latitude
                'units': 'Feet', # Required parameter
                'output': 'json'
            }
            ...
            # Check if the location is on federal/public land
            # USGS returns landowner information in the response
            if 'owner' in data:
                owner = data['owner'].lower()
                return any(public_owner in owner for public_owner in [
                    'federal', 'national', 'state', 'county', 'city', 
                    'municipal', 'public', 'blm', 'usfs', 'nps'
                ])
            return False  # If no owner info, assume private

        except Exception as e:
            self.logger.warning(f"Failed to check land ownership: {e}")
            # If API fails, try to use OSM tags as fallback
            if self.tags.get('access') == 'public' or self.tags.get('ownership') == 'public':
                return True
            if self.tags.get('access') == 'private' or self.tags.get('ownership') == 'private':
                return False
            return None  # Unknown if can't determine
