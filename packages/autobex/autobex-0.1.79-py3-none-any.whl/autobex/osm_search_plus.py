...
class LocationResult:
    ...
    def all_tags(self, include_empty: bool = False) -> str:
        ...
        # Add distance information first, with None checks
        if self.distance is not None:
            lines.append(f"Direct distance: {self.distance/1609.34:.2f} miles")
        if self.road_distance is not None:
            lines.append(f"Distance to nearest road: {self.road_distance/1609.34:.2f} miles")
    ...
    def __str__(self) -> str:
        ...
        if self.distance is not None:
            parts.append(f"Distance: {self.distance/1609.34:.2f} miles")
        if self.road_distance is not None:
            parts.append(f"Road distance: {self.road_distance/1609.34:.2f} miles")
        return " | ".join(parts)
...
class OSMSearchPlus:
    ...
    def get_nearest_road_distance(self, lat: float, lon: float) -> Optional[float]:
        ...
        try:
            # Query for nearby roads
            query = f"""
            [out:json][timeout:30];
            (
              way["highway"]["highway"!="path"]["highway"!="footway"]["highway"!="cycleway"]
                (around:1000,{lat},{lon});
            );
            out body;
            >;
    ...
    def _point_to_line_distance(self, point: Tuple[float, float], 
        ...
        if (None in point or None in line_start or None in line_end or
            not all(isinstance(x, (int, float)) for x in point + line_start + line_end)):
            return None
            
        try:
            # Convert to x,y coordinates (approximate for small distances)
            lat, lon = point
            ...
            x = (lon - lon1) * meters_per_degree * np.cos(np.radians(lat))
            y = (lat - lat1) * meters_per_degree
            
            dx = (lon2 - lon1) * meters_per_degree * np.cos(np.radians(lat))
            ...
            t = max(0, min(1, (x*dx + y*dy) / (line_length*line_length)))
