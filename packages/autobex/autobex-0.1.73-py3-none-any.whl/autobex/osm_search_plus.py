        ...
        try:
            for tag in self.tags:
                ...
                if '=' in tag:
                    ...
                    tag_filters.append(f'["{tag_key}"="{tag_value}"]')
                else:
                    # Simple tag (e.g., ruins)
                    tag_key = tag.replace('"', '\\"')  # Escape quotes
                    # Match tag as key with any value
                    tag_filters.append(f'["{tag_key}"]')
                    # Match tag as value for any key
                    # Create regex pattern from all loaded tags that don't contain =
                    key_patterns = [t.split('=')[0] for t in self.tags if '=' not in t]
                    if key_patterns:
                        pattern = '|'.join(key_patterns)
                        tag_filters.append(f'[~"^({pattern})$"="{tag_key}"]')

                # Build a single query for all types
                query = f"""
                [out:json][timeout:{self.timeout}];
                (
                  node{tag_filters[0]}{area_filter};
                  way{tag_filters[0]}{area_filter};
                  relation{tag_filters[0]}{area_filter};
                  {';'.join(f'''
                  node{filter}{area_filter};
                  way{filter}{area_filter};
                  relation{filter}{area_filter}''' for filter in tag_filters[1:])}
                );
                out body;
                >;
                out skel qt;
                ...
                # Cache relation tags to apply to members
                relation_tags = {}  # {member_id: {tags}}
                for element in data.get('elements', []):
                    if element['type'] == 'relation' and 'tags' in element:
                        for member in element.get('members', []):
                            if member['ref'] not in relation_tags:
                                relation_tags[member['ref']] = {}
                            relation_tags[member['ref']].update(element['tags'])
                ...
                for element in data.get('elements', []):
                    ...
                    if element['type'] == 'node':
                        ...
                        # Get center of relation members
                        member_coords = []
                        for member in element.get('members', []):
                            if member['type'] == 'node' and member['ref'] in node_cache:
                                member_coords.append(node_cache[member['ref']])
                        if not member_coords:
                            continue
                        result_lat = sum(n[0] for n in member_coords) / len(member_coords)
                        result_lon = sum(n[1] for n in member_coords) / len(member_coords)
                    else:
                        continue
                    ...
                    # Merge relation tags with element tags
                    merged_tags = element['tags'].copy()
                    if element['id'] in relation_tags:
                        merged_tags.update(relation_tags[element['id']])

                    # Create result object
                    result = LocationResult(
                        name=self.get_location_name(result_lat, result_lon, merged_tags),
                        latitude=result_lat,
