...
class OSMSearchPlus:
    ...
    def _parse_dms_coordinate(self, coord_str: str) -> float:
        """
        Parse DMS (Degrees, Minutes, Seconds) coordinate string to decimal degrees.
        Handles formats like: 41°28'50.4"N or 71°23'35.5"W
        """
        try:
    ...
    def convert_coordinates(self, lat: str, lon: str) -> Tuple[float, float]:
        """
        Convert coordinate strings to decimal degrees.
        Handles both decimal degrees and DMS formats.
        
        Examples:
            - DMS: "41°28'50.4"N", "71°23'35.5"W"
            - Decimal: "41.2345", "-71.2345"
        """
        try:
            # Try parsing as float first (decimal degrees)
            lat_dec = float(lat)
            lon_dec = float(lon)
        except ValueError:
            # If that fails, try parsing as DMS
            try:
                lat_dec = self._parse_dms_coordinate(lat)
                lon_dec = self._parse_dms_coordinate(lon)
            except Exception as e:
                raise OSMSearchError(f"Invalid coordinates. Must be decimal degrees or DMS format: {str(e)}")
        
        # Validate coordinates
        if not (-90 <= lat_dec <= 90) or not (-180 <= lon_dec <= 180):
            raise OSMSearchError(f"Coordinates out of range: {lat_dec}, {lon_dec}")
    ...
    def search(self, lat: Union[str, float, None] = None, 
        ...
        if lat is not None and lon is not None:
            if isinstance(lat, str) or isinstance(lon, str):
                lat, lon = self.convert_coordinates(str(lat), str(lon))
                
        # Convert polygon coordinates if provided
        if polygon_coords:
            converted_polygon = []
            for p_lat, p_lon in polygon_coords:
                if isinstance(p_lat, str) or isinstance(p_lon, str):
                    conv_lat, conv_lon = self.convert_coordinates(str(p_lat), str(p_lon))
                    converted_polygon.append((conv_lat, conv_lon))
                else:
