        ...
        try:
            with tqdm(total=100, disable=not self.show_progress) as pbar:
                ...
                if self.mode == 'radius':
                    ...
                    if self.show_logs:
                        self.logger.info(f"Splitting radius search into {chunks} chunks")
                    
                    for i in range(chunks):
                        current_radius = (radius_meters / chunks) * (i + 1)
                        chunk_params = {**params, 'radius': min(current_radius, radius_meters)}
                        if self.show_logs:
                            self.logger.info(f"Processing chunk {i+1}/{chunks} with radius {chunk_params['radius']}")
                    ...
                    if self.show_logs:
                        self.logger.info(f"Splitting bbox search into {total_chunks} chunks")
                    
                    for i, chunk_params in enumerate(chunk_params_list):
                        if self.show_logs:
                            self.logger.info(f"Processing bbox chunk {i+1}/{total_chunks}: {chunk_params}")
                    ...
                    if self.show_logs:
                        self.logger.info(f"Splitting polygon search into {total_chunks} chunks")
                    
                    for i, chunk_coords in enumerate(chunk_coords_list):
                        chunk_params = {**params, 'polygon_coords': chunk_coords}
                        if self.show_logs:
                            self.logger.info(f"Processing polygon chunk {i+1}/{total_chunks}")
                        self._execute_chunk(chunk_params, tag_str, all_elements, pbar, i, total_chunks)
