...
Example:
    ...
    def _build_query(self, tag_str: str, **kwargs) -> str:
        ...
        if self.mode == 'radius':
            ...
            lat_diff = abs(kwargs['max_lat'] - kwargs['min_lat'])
            lon_diff = abs(kwargs['max_lon'] - kwargs['min_lon'])
            if lat_diff * lon_diff > 0.1:  # Area too large
                area_filter = f"(area:3600000000)->"  # Use area filter
            else:
                area_filter = f"({kwargs['min_lat']},{kwargs['min_lon']},{kwargs['max_lat']},{kwargs['max_lon']})"
                
        elif self.mode == 'polygon':
            # Convert polygon to bbox for initial filter
            lats = [p[0] for p in kwargs['polygon_coords']]
            lons = [p[1] for p in kwargs['polygon_coords']]
            area_filter = f"({min(lats)},{min(lons)},{max(lats)},{max(lons)})"
        else:
            area_filter = ""
        ...
        try:
            with tqdm(total=100, disable=not self.show_progress) as pbar:
                ...
                if self.mode == 'radius':
                    ...
                    # Split large bbox into chunks
                    lat_diff = abs(params['max_lat'] - params['min_lat'])
                    lon_diff = abs(params['max_lon'] - params['min_lon'])
                    if lat_diff * lon_diff > 0.1:
                        chunks_lat = max(1, int(lat_diff / 0.1))
                        chunks_lon = max(1, int(lon_diff / 0.1))
                        total_chunks = chunks_lat * chunks_lon
                        
                        for i in range(chunks_lat):
                            for j in range(chunks_lon):
                                min_lat = params['min_lat'] + (lat_diff * i / chunks_lat)
                                max_lat = params['min_lat'] + (lat_diff * (i + 1) / chunks_lat)
                                min_lon = params['min_lon'] + (lon_diff * j / chunks_lon)
                                max_lon = params['min_lon'] + (lon_diff * (j + 1) / chunks_lon)
                                
                                chunk_params = {**params, 'min_lat': min_lat, 'min_lon': min_lon, 
                                              'max_lat': max_lat, 'max_lon': max_lon}
                                query = self._build_query(tag_str, **chunk_params)
                                result = self.api.query(query)
                                elements = self._process_results(result)
                                all_elements.extend(elements)
                    ...
                    # Use bbox for initial query, then filter by polygon
                    query = self._build_query(tag_str, **params)
                    result = self.api.query(query)
                    elements = self._process_results(result)
                    
                    # Post-process to filter by polygon
                    polygon = Polygon(params['polygon_coords'])
                    all_elements = [
                        elem for elem in elements
                        if (elem['type'] == 'node' and 
                            polygon.contains(Point(elem['lon'], elem['lat']))) or
                        (elem['type'] == 'way' and 
                         polygon.intersects(LineString([(n[1], n[0]) for n in elem['nodes']])))
                    ]
                    pbar.update(100)
