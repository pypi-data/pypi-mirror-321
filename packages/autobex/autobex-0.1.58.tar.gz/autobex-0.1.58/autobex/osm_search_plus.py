...
class OSMSearchPlus:
    def __init__(self, logger=None):
        ...
        self.padus_api = 'https://services.arcgis.com/v01gqwM5QqNysAAi/arcgis/rest/services/PADUS3_0Combined_Proclamation_Marine_Fee_Designation_Easement/FeatureServer/0/query'
        self.timeout = 60
    ...
    def _check_land_ownership(self, lat: float, lon: float, tags: Dict[str, str]) -> bool:
        """
        Check if a location is on public land using USGS Protected Areas Database.
        Returns True if public, False if private, None if uncertain.
        """
        try:
            # Build a small buffer around the point (10 meters)
            buffer = 0.0001  # approximately 10 meters in decimal degrees
            params = {
                'geometry': f"{lon},{lat}",  # ESRI REST uses lon,lat
                'geometryType': 'esriGeometryPoint',
                'inSR': '4326',  # WGS84
                'spatialRel': 'esriSpatialRelIntersects',
                'outFields': 'Own_Type,Mang_Type,Des_Tp,GAP_Sts,Loc_Nm',
                'returnGeometry': 'false',
                'f': 'json'
            }
            
            response = requests.get(self.padus_api, params=params, timeout=self.timeout)
            response.raise_for_status()
            data = response.json()
            ...
            if 'features' in data and data['features']:
                ...
                owner_type = attributes.get('Own_Type', '').lower()
                manager_type = attributes.get('Mang_Type', '').lower()
                designation = attributes.get('Des_Tp', '').lower()
                gap_status = attributes.get('GAP_Sts')  # 1-4, where 1-3 are protected
                
                # Public land indicators
                public_owners = ['fed', 'state', 'local', 'native', 'tribal', 'territory']
                public_designations = ['wilderness', 'park', 'monument', 'recreation', 'conservation', 'wildlife', 'forest']
                
                is_public = (
                    any(owner in owner_type for owner in public_owners) or
                    any(owner in manager_type for owner in public_owners) or
                    any(desig in designation for desig in public_designations) or
                    (gap_status and 1 <= gap_status <= 3)
                )
                
                return is_public
            
            return False  # If no features found, assume private

        except Exception as e:
            self.logger.warning(f"Failed to check PAD-US land ownership: {e}")
