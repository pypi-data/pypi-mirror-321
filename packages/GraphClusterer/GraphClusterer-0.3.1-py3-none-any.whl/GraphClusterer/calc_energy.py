...
def load_embedding(ds_name, embeddings_dir='data/embeddings', _is_first_run=False):
    ...
    if embedding_path is None or not os.path.exists(embedding_path):  # If the embedding file does not exist, create it.
        ...
        path = input(f"Enter the path to the dataset file for {ds_name}: ") if not _is_first_run \
            else "data/3D printing_papers.csv"  # Default dataset file
