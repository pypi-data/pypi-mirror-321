    ...
    def resolve_import(self, base_path: str | None = None) -> ImportResolution[TSFile] | None:
        ...
        if "." not in import_name:
            ...
            for p_path in possible_paths:
                if os.path.exists(os.path.join(import_source, p_path)):
    ...
    def from_export_statement(cls, source_node: TSNode, file_node_id: NodeId, G: CodebaseGraph, parent: TSImportStatement) -> list[TSImport]:
        ...
        if export_clause := next((child for child in export_statement_node.named_children if child.type == "export_clause"), None):
            ...
            if namespace_export := next((child for child in export_statement_node.named_children if child.type == "namespace_export"), None):
                ...
                imp = cls(
                    ts_node=export_statement_node, file_node_id=file_node_id, G=G, parent=parent, module_node=source_node, name_node=namespace_export, alias_node=alias, import_type=ImportType.WILDCARD
                )
                imports.append(imp)
            else:
