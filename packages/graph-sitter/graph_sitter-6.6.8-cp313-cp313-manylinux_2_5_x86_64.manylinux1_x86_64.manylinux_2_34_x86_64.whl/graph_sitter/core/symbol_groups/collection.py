        ...
        if insert_byte < self._container_end_byte - self._bracket_size:
            ...
            # Case 3: A spread element was deleted and we must respect that
                ...
                if (relative_byte := remaining.rfind(self._delimiter)) != -1:
                    ...
                    element_deleted = self.transaction_manager.get_transactions_at_range(self.filepath, delim_byte, self.start_byte + len(remaining), TransactionPriority.Remove, combined=True)
                    delimeter_deleted = self.transaction_manager.get_transactions_at_range(self.filepath, delim_byte - len(self._delimiter), delim_byte, TransactionPriority.Remove, combined=True)
