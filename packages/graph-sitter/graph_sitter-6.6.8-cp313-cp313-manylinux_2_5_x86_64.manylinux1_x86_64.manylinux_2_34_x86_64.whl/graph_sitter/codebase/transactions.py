...
class RemoveTransaction(Transaction):
    ...
    def __init__(self, start_byte: int, end_byte: int, file_path: str, file: "File", priority: int = 0, exec_func: Callable[[], None] | None = None) -> None:
        super().__init__(start_byte, end_byte, file_path, priority=priority)
        self.file = file
...
class InsertTransaction(Transaction):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(insert_byte, insert_byte, file_path, priority=priority, new_content=new_content)
...
class EditTransaction(Transaction):
    ...
    def __init__(
        ...
        super().__init__(start_byte, end_byte, file_path, priority=priority, new_content=new_content)
        self.file = file
    ...
    def break_down(self) -> list[InsertTransaction] | None:
        ...
        if old and old in new:
            ...
            ret = []
            if suffix:
                ret.append(InsertTransaction(self.end_byte, self.file_path, self.file, suffix.decode("utf-8"), priority=self.priority))
            if prefix:
                ret.append(InsertTransaction(self.start_byte, self.file_path, self.file, prefix.decode("utf-8"), priority=self.priority))
            return ret
        return None
...
class FileRenameTransaction(Transaction):
    ...
    def __init__(
        ...
        super().__init__(0, 0, file_path, priority=priority, new_content=new_file_path)
