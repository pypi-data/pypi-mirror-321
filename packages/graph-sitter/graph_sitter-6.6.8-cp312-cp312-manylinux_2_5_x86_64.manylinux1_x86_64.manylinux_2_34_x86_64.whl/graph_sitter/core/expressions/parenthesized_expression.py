    ...
    def unwrap(self, node: Expression | None = None):
        ...
        if node is None:
            remaining = list(
                ...
                if not self.transaction_manager.get_transactions_at_range(self.filepath, start_byte=child.start_byte, end_byte=child.end_byte, transaction_order=TransactionPriority.Remove)
            )
            if len(remaining) == 1:
                node = remaining[0]
            else:
