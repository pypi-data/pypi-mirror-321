...
class TSConfig:
        ...
        if references is not None:
            for reference in references:
                if ref_path := reference.get("path", None):
                    abs_ref_path = self._relative_to_absolute_directory_path(ref_path)
                    if directory := self.config_file.G.get_directory(abs_ref_path):
    ...
    def _precompute_import_aliases(self):
        ...
        for pattern, relative_paths in self._self_paths.items():
            ...
            for relative_path in relative_paths:
                ...
                formatted_relative_path = self._relative_to_absolute_directory_path(cleaned_relative_path)
    ...
    def _parse_parent_config_path(self, config_filepath: String | None) -> str | None:
    ...
    def _relative_to_absolute_directory_path(self, relative_path: str) -> str:
        ...
        # For example "@src/tsconfig.json" could be resolved to "base/src/tsconfig.json"
        config_file_dir = os.path.dirname(self.config_file.filepath)  # the directory of the config file
        absolute_path = os.path.join(config_file_dir, relative_path.strip('"'))  # absolute path of the relative path
        normalized_path = os.path.normpath(absolute_path)  # normalized absolute path of the import. removes redundant separators and './' or '../' segments.
    ...
    def resolve_base_url(self, import_path: str) -> str:
        ...
        if self._self_base_url:
            if not import_path.startswith(self._self_base_url):
                ...
                import_path = self._relative_to_absolute_directory_path(import_path)
            return import_path
    ...
    def reference_import_aliases(self) -> dict[str, list[str]]:
        ...
        """
        return self._reference_import_aliases
