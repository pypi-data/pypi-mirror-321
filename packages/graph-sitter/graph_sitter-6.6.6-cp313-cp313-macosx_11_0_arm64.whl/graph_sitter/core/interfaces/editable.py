...
from codegen.utils.string_utils import truncate_line
...
from graph_sitter.utils import descendant_for_byte_range, find_all_descendants, find_first_ancestor, find_index
...
class Editable(JSONable, Generic[Parent]):
    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        ...
        if self.file:
            yield from style_editable(self.ts_node, self.filepath, self.file.ts_node)
    ...
    def remove_byte_range(self, start_byte: int, end_byte: int) -> None:
        ...
        t = RemoveTransaction(start_byte, end_byte, self.filepath, self.file)
    ...
    def _remove(self, delete_formatting: bool = True, priority: int = 0, dedupe: bool = True) -> None:
        ...
        # Delete the node
        t = RemoveTransaction(removed_start_byte, removed_end_byte, self.filepath, self.file, priority=priority, exec_func=exec_func)
        ...
        if delete_formatting and siblings and len(siblings) > 1:
            ...
            all_previous_deleted = all(
                self.transaction_manager.get_transactions_at_range(self.filepath, start_byte=siblings[i].start_byte, end_byte=siblings[i].end_byte, transaction_order=TransactionPriority.Remove)
    ...
    def commit(self) -> None:
        ...
        Returns:
            None
        """
        self.G.commit_transactions(files={self.filepath})
