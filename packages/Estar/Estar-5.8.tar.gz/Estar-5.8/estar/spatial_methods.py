...
def runoverfile(hsfolder,obsfolder,obstaxafile,sig=30,subregionfile=None,RefRangeDistSmooth=50,WE=0.7,Wdens=0.7,bydeclust=40,typerange = "PseudoRange",NaNvalue=None,savefigfolder=None,outputtype="CR",plot=False,Ksig=None,bynx=10,comp1percent=50):
    ...
    if plot==True and savefigfolder is not None:
    ...
    for obsfile,hsfile in zip(os.listdir(obsfolder),os.listdir(hsfolder)):
        ...
        if os.path.exists(savefigfolder+"/continuous/"+obsfile[:-4]+".tif")==True:
        ...
        density = Pxy( Obs=Obs, background=HS, bynx=bynx,comp1percent=comp1percent,sigdefault=sig,plot=plot)
