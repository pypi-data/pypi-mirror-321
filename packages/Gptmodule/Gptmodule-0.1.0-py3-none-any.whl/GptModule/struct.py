...
class Dataset():
    ...
    def dataset_loadqa(self,file_name,first_n=None,start_from=0,name="WICKED4950/MentalHeathEsther",login=True,local=True):
      if local:
          import json
    ...
    def mask_qu_multiple(self,tokenizer ,conversations ,len=1024):
        all_input_ids = []
        all_attention_masks = []
        all_labels = []

        for conversation in conversations:
            # Tokenize the conversation and extract input_ids and attention_mask
        ...
        return self.create_tf_dataset(inputs=tf.concat(all_input_ids, axis=0),attention_mask=tf.concat(all_attention_masks, axis=0),labels=tf.concat(all_labels, axis=0))
    ...
    def __call__(self,file_name,tokenizer,first_n=None,start_from=0,name="WICKED4950/MentalHeathEsther",local=False,answer_only=True,batch_size=8,maskq=False):
        ...
        try:
            self.tf_dataset = tf.data.experimental.load(self.dt_save_path)
            self.return_tf()
        except:
            ...
            if maskq:
                ...
                self.mask_qu_multiple(tokenizer=tokenizer,len=512)
            tf.data.experimental.save(self.tf_dataset, self.dt_save_path)
...
class ModelGPT():
    ...
    @error_handler
    def train_model(self, dataset_, epochs=1, lr=5e-5, decay_rate=1.00, decay_steps=1000,loss_avg=1500,distr=True,change_batch=500,split_training=False):
        ...
        with strategy.scope():
            # Distribute dataset
            dist_dataset = strategy.experimental_distribute_dataset(dataset_)
            optimizer = tf.keras.optimizers.Adam(learning_rate=lr_schedule)
            for epoch in range(epochs):
                all_loss = []
                progress_bar = tqdm(dist_dataset, desc=f"Epoch {epoch + 1}", unit="batch")
                i = 0
                j = 0
                for batch in progress_bar:
                    ...
                    losses = all_loss[-loss_avg:]
                    memory_used = psutil.virtual_memory().used / (1024 ** 3)  # in GB
                    cpu_percent = psutil.cpu_percent(interval=0)  # in percentage
                    progress_bar.set_postfix(loss=(sum(losses)/len(losses)), memory=f"{memory_used:.2f} GB", cpu=f"{cpu_percent}%",inst_loss=(losses[-1]))
    ...
    def push_model(self,name,username="WICKED4950"):
        from huggingface_hub import login, create_repo, upload_file
        repo_url = create_repo(name, token="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")
        print(f"Repository created at: {repo_url}")
        self.upload(f"{username}/{name}")
    ...
    def generate_text(self, prompt, sos="<|SOH|>", eos="<|SOB|>", recall_sent=4,sys_prom=None,show_rawinp=False,ret_sq=1,temperature=0.5,top_k=2):
        ...
        raw_responses = self.raw_pred(input_text, max_length=256,ret_sq=ret_sq,temperature=temperature,top_k=top_k)
