try:
    ...
    import psutil
    import shutil
    from struct import Dataset, ModelGPT, Voice
except Exception as e:
    raise ImportError(f"Some of the modules are not imported correctly!\n{e}")


dataset = Dataset()
model = ModelGPT(mix_pre=False,use_default=True)
...
def reinitmodel(mix_pre=False,use_default=True):
    global model
    del model
...
def train(epochs=1, lr=5e-5, decay_rate=1.00, decay_steps=1000,loss_avg=1500,distr=True,change_batch=500,split_training=False):
    if dataset.return_tf() == None:
        raise ValueError("You have to first load and process the data to train the model!")
    else:
        model.train_model(dataset.return_tf(),epochs,lr,decay_rate,decay_rate,loss_avg,distr,change_batch,split_training)
        print("training done!")
